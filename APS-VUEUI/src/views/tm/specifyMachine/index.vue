<template>
  <basic-container>
    <page-table
      tableRef="tmSpecifyMachineMainTable"
      :calcHeight="true"
      v-loading="loading"
      :columns="columns"
    ...
    >
      <template slot="header">
        <el-button
          type="primary"
          plain
          v-hasPermi="['tm:tmSpecifyMachine:edit']"
          @click="handleAdd"
        >{{ $t("ui.frame.btn.add") }}</el-button>
        <el-button
          type="danger"
          v-hasPermi="['tm:tmSpecifyMachine:remove']"
          :disabled="selection.length == 0"
          @click="handleDeleteAll"
        >{{ $t("ui.frame.btn.delete") }}</el-button>
        <el-button
          v-hasPermi="['tm:tmSpecifyMachine:import']"
          @click="$refs.tltUpload.handleImport()"
        >{{ $t("ui.frame.btn.import") }}</el-button>
        <el-button
          @click="handleExport"
          v-hasPermi="['tm:tmSpecifyMachine:export']"
    ...
    <tlt-upload-form
      ref="tltUpload"
      :updateSupport="true"
      downloadUrl="/tm/tmSpecifyMachine/importTemplate"
      uploadUrl="/tm/tmSpecifyMachine/importData"
...
//interface
import {listTmSpecifyMachine, removeTmSpecifyMachine, saveTmSpecifyMachine} from "@/api/tm/specifyMachine";
//components
                ...
                onChange={(val) => {
                  ...
                  this.$confirm(confirmMsg, { type: "warning" }).then(
                    async () => {
                      try {
                        this.loading = true;
                        const data = await saveTmSpecifyMachine({
                          ...row,
                          enableStatus: val,
                        });
  ...
  methods: {
    ...
    handleDelete(row) {
      this.$confirm(this.$t("common.confirm.delete"), {
        ...
        const ids = row.id;
        removeTmSpecifyMachine({ ids }).then((data) => {
          this.$modal.msgSuccess(data.msg);
    ...
    handleExport() {
      downloadLink("/tm/tmSpecifyMachine/export", this.formatParams(false));
    },
    ...
    async getList() {
      try {
        this.loading = true;
        const data = await listTmSpecifyMachine(this.formatParams());
        this.data = data.rows;
