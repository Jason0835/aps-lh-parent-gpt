...
export default {
  ...
  computed: {
    ...
    title: function () {
      return (
        ...
        this.$t("ui.data.column.tm.mouthPlate.modelName")
      );
    },
    columns() {
      return [
        {
          ...
          required: true,
          disabled: true,
          placeholder: this.$t("ui.data.placeholder.tm.mouthPlate.factoryCode"),
        },
        {
          prop: "mouthPlateCode",
  ...
  methods: {
    ...
    async loadMachines() {
      try {
        const res = await listTmMachineInfo({ pageSize: 9999 });
