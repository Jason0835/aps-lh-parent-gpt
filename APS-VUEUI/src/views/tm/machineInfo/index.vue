<template>
  <basic-container>
    <page-table
      tableRef="treadMachineInfoMainTable"
      :calcHeight="true"
      v-loading="loading"
      :columns="columns"
    ...
    >
      <template slot="header">
        <el-button
          type="primary"
          plain
          v-hasPermi="['tm:tmMachineInfo:edit']"
          @click="handleAdd"
          >{{ $t("ui.frame.btn.add") }}</el-button
        >
        <el-button
          type="danger"
          v-hasPermi="['tm:tmMachineInfo:remove']"
          :disabled="selection.length == 0"
          @click="handleDeleteAll"
          >{{ $t("ui.frame.btn.delete") }}</el-button
        >
        <el-button
          v-hasPermi="['tm:tmMachineInfo:import']"
          @click="$refs.tltUpload.handleImport()"
          >{{ $t("ui.frame.btn.import") }}</el-button
        >
        <el-button
          @click="handleExport"
          v-hasPermi="['tm:tmMachineInfo:export']"
    ...
    <tlt-upload-form
      ref="tltUpload"
      :updateSupport="true"
      downloadUrl="/tm/tmMachineInfo/importTemplate"
      uploadUrl="/tm/tmMachineInfo/importData"
  ...
  methods: {
    ...
    handleDelete(row) {
      this.$confirm(this.$t("common.confirm.delete"), {
        ...
        const ids = row.id;
        removeTmMachineInfo({ ids }).then((data) => {
          this.$modal.msgSuccess(data.msg);
    ...
    handleExport() {
      downloadLink("/tm/tmMachineInfo/export", this.formatParams(false));
    },
    ...
    async getList() {
      try {
        this.loading = true;
        const data = await listTmMachineInfo(this.formatParams());
        this.data = data.rows;
