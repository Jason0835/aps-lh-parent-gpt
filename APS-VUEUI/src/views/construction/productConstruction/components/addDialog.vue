...
export default {
    ...
    columns() {
      return [
        ...
        },
        {
          ...
          span: 24,
          type: "select",
          dictData: this.embryoVersionsList,
          labelKey: "embryoVersion",
          valueKey: "embryoVersion",
          // disabled: true,
        },
      ];
    },
  ...
  watch: {
    "form.embryoCode": function (val) {
      if (val) {
        this.getEmbryoVersions();
      } else {
        this.embryoVersionsList = [];
      }
    },
  },
  ...
  methods: {
    ...
    async getEmbryoVersions() {
      try {
        const res = await getEmbryoVersions({
          embryoCode: this.form.embryoCode,
        });
        this.embryoVersionsList = res;
      } catch (error) {
        console.error(error);
      }
    },
