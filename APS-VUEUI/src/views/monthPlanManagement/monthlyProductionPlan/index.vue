...
<template>
  <basic-container>
    <page-table
      tableRef="MonthlyProductionPlanMainTable"
      :calcHeight="true"
      v-loading="loading"
      :columns="columns"
    ...
    >
      <template slot="header">
        ...
        <el-button type="warning" @click="handleExportExpectedExcessArrears">{{
          $t("ui.data.column.mdmMonthProdPlan.expectedExport")
        }}</el-button>
        <el-button type="warning" @click="handleExportExcessArrears">{{
          $t("ui.data.column.mdmMonthProdPlan.overProdExport")
        }}</el-button>
        <el-button type="primary" @click="handleIssuePlan">{{
          $t("ui.data.column.mdmMonthProdPlan.issuePlan")
        }}</el-button>
        <el-button
          type="primary"
          @click="
...
export default {
 name: "MonthlyProductionPlan",
  ...
  dicts: ["quality_level", "STORAGE_LOCATION"],
  data() {
    return {
      dailyVisible: false,
      importDefaultValue: {
  ...
  computed: {
    ...mapState({
      machines: (state) => state.molding.machines,
    }),
    columns() {
      let columns = [
        ...
        },
        {
          ...
          render: ({ row }) => {
            let value = this.isEmpty()
              ? this.$t("ui.data.column.productConstruction.noVersion")
              : row.bomDataVersion;
            return value;
            // return (
            //   <el-link
        ...
        },
        {
          ...
          formatter: (row, column, value) => {
            return this.selectDictLabel(this.dict.type.quality_level, value);
          },
        },
        {
        ...
        {
          label: this.$t(
            "ui.data.column.mdmMonthProdPlan.theoryProductionPlan"
          ),
        ...
        {
          label: this.$t(
            "ui.data.column.mdmMonthProdPlan.expectedExcessArrears"
          ),
        ...
        {
          label: this.$t(
            "ui.data.column.mdmMonthProdPlan.actualOverProduction"
          ),
