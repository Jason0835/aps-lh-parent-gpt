<template>
  <basic-container>
    <page-table tableRef="machineMaintenanceMainTable" :calcHeight="true" v-loading="loading" :columns="columns" :searchColumns="searchColumns" :data="data" :page="page" :search="search" @refresh="getList" @search="handleSearch" @pageChange="handlePageChange" @sort-change="handleSortChange" @selection-change="handleSelectionChange" :showSummary="false" :selectArea="false">
      <template slot="header">
        <el-button type="primary" plain v-hasPermi="['cd90:machineMaintenance:add']" @click="handleAdd">{{ $t("ui.frame.btn.add") }}</el-button>
        <el-button v-hasPermi="['cd90:machineMaintenance:edit']" @click="handleBatchEdit" :disabled="selection.length !== 1">{{ $t("ui.frame.btn.update") }}</el-button>
        <el-button type="danger" v-hasPermi="['cd90:machineMaintenance:remove']" :disabled="selection.length === 0" @click="handleBatchDelete">{{ $t("ui.frame.btn.delete") }}</el-button>
        <el-button v-hasPermi="['cd90:machineMaintenance:import']" @click="$refs.tltUpload.handleImport()">{{ $t("ui.frame.btn.import") }}</el-button>
        <el-button @click="handleExport" v-hasPermi="['cd90:machineMaintenance:export']">{{ $t("ui.frame.btn.export") }}</el-button>
      </template>
    </page-table>
    <tlt-upload-form ref="tltUpload" :updateSupport="true" downloadUrl="/cd90/cd90MachineMaintenance/importTemplate" uploadUrl="/cd90/cd90MachineMaintenance/importData" @uploadSuccess="getList" labelWidth="0" :columns="importColumns" />
  ...
  computed: {
    columns() {
      return [
        ...
        { prop: "factoryCode", align: "center", halign: "center", label: this.$t("ui.data.column.cd90MachineMaintenancePlan.factoryCode"), minWidth: 120, formatter: (row, column, value) => this.selectDictLabel(this.dict.type.biz_factory_name, value) },
        { prop: "machineCode", align: "center", halign: "center", label: this.$t("ui.data.column.cd90MachineMaintenancePlan.machineCode"), minWidth: 140 },
        { prop: "downtimeDate", align: "center", halign: "center", label: this.$t("ui.data.column.cd90MachineMaintenancePlan.downtimeDate"), minWidth: 120 },
        { prop: "downtimeStartTime", align: "center", halign: "center", label: this.$t("ui.data.column.cd90MachineMaintenancePlan.downtimeStartTime"), minWidth: 160 },
        { prop: "downtimeEndTime", align: "center", halign: "center", label: this.$t("ui.data.column.cd90MachineMaintenancePlan.downtimeEndTime"), minWidth: 160 },
    ...
    searchColumns() {
      return [
        { label: this.$t("ui.data.column.cd90MachineMaintenancePlan.factoryCode"), prop: "factoryCode", type: "select", dictData: this.dict.type.biz_factory_name, filterable: true, listeners: { change: (factoryCode) => this.loadMachineOptions(factoryCode) } },
