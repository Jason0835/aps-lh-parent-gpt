...
export default {
  ...
  computed: {
    ...
    columns() {
      return [
        { prop: "factoryCode", label: this.$t("ui.data.column.cd90MachineMaintenancePlan.factoryCode"), type: "select", dictData: this.parentDict.type.biz_factory_name, filterable: true },
        { prop: "machineCode", label: this.$t("ui.data.column.cd90MachineMaintenancePlan.machineCode"), maxlength: 30 },
        { prop: "downtimeDate", label: this.$t("ui.data.column.cd90MachineMaintenancePlan.downtimeDate"), type: "date" },
        { prop: "downtimeStartTime", label: this.$t("ui.data.column.cd90MachineMaintenancePlan.downtimeStartTime"), type: "datetime" },
        { prop: "downtimeEndTime", label: this.$t("ui.data.column.cd90MachineMaintenancePlan.downtimeEndTime"), type: "datetime" },
        { prop: "downtimeHours", label: this.$t("ui.data.column.cd90MachineMaintenancePlan.downtimeHours"), type: "number" },
        { prop: "remark", label: this.$t("ui.common.column.remark"), type: "textarea", rows: 3, maxlength: 300 },
      ];
    },
  },
  methods: {
    async save(params) { this.loading = true; try { const res = this.isEdit ? await updateMachineMaintenancePlan(params) : await addMachineMaintenancePlan(params); this.$modal.msgSuccess(res.msg); this.$emit("success"); this.hide(); } finally { this.loading = false; } },
