<template>
  <el-dialog
    :title="$t('ui.data.column.cd90ScheduleResult.changeQty')"
  ...
  >
    <el-form ref="form" v-loading="loading" :model="form" :rules="rules" label-width="100px">
      ...
      <el-form-item :label="$t('ui.data.column.cd90ScheduleResult.changeQtyEditableShift')">
        <el-table :data="shiftRows" border size="small" style="width:100%">
          <el-table-column :label="$t('ui.data.column.cd90ScheduleResult.changeQtyEditableShift')" width="100" align="center">
          ...
          <el-table-column :label="$t('ui.data.column.cd90ScheduleResult.originalPlanQty')" width="130" align="right">
            <template slot-scope="scope">
              <span>{{ formatQty(scope.row.originalPlanQty) }}</span>
            </template>
          </el-table-column>
          <el-table-column :label="$t('ui.data.column.cd90ScheduleResult.finishQty')" width="120" align="right">
            <template slot-scope="scope">
              <span>{{ formatQty(scope.row.finishQty) }}</span>
            </template>
          </el-table-column>
          <el-table-column :label="$t('ui.data.column.cd90ScheduleResult.produceOrder')" width="100" align="center">
            <template slot-scope="scope">
              <span>{{ scope.row.produceOrder || '-' }}</span>
            </template>
          </el-table-column>
          <el-table-column :label="$t('ui.data.column.cd90ScheduleResult.targetPlanQty')" width="180" align="center">
            <template slot-scope="scope">
              <el-input-number
...
<script>
import { shiftDates, validateChangeQty, changeQty } from '@/api/cd90/scheduleResult'
...
export default {
  ...
  methods: {
    ...
    async loadShiftDates() {
      ...
      const currentRow = rows.find(item => item.currentShift) || rows.find(item => item.changeQtyEditable)
      this.currentClassField = currentRow ? currentRow.classField : ''
      this.shiftRows = this.buildShiftRows(this.currentRow, rows)
      this.resetReadonlyTargets()
    },
    ...
    changeQtyShiftStatusText(row) {
      if (row && row.currentShift) {
        return this.$t('ui.data.column.cd90ScheduleResult.changeQtyCurrentShift')
      }
      return this.isChangeQtyClass(row)
        ? this.$t('ui.data.column.cd90ScheduleResult.changeQtyEditable')
        : this.$t('ui.data.column.cd90ScheduleResult.changeQtyReadonly')
    },
    resetReadonlyTargets() {
      this.shiftRows.forEach(row => {
        if (!this.isChangeQtyClass(row)) {
          row.targetPlanQty = row.originalPlanQty
        } else if (row.targetPlanQty == null) {
          row.targetPlanQty = row.originalPlanQty
        }
      })
    },
    handleConfirm() {
      this.$refs.form.validate(async valid => {
        ...
        if (!this.shiftRows.some(row => this.isChangeQtyClass(row))) {
          this.$modal.msgWarning(this.$t('ui.data.column.cd90ScheduleResult.changeQtyNoPlan'))
          return
        }
        if (this.shiftRows.some(row => this.isChangeQtyClass(row) && row.targetPlanQty == null)) {
          this.$modal.msgWarning(this.$t('ui.data.column.cd90ScheduleResult.changeQtyTargetRequired'))
          return
        }
        if (this.shiftRows.some(row => this.isChangeQtyClass(row) && Number(row.targetPlanQty) < row.finishQty)) {
          this.$modal.msgWarning(this.$t('ui.data.column.cd90ScheduleResult.changeQtyLessThanFinish'))
          return
        }
        if (!this.shiftRows.some(row => this.isChangeQtyClass(row) && Number(row.targetPlanQty) !== row.originalPlanQty)) {
          this.$modal.msgWarning(this.$t('ui.data.column.cd90ScheduleResult.changeQtyNeedOne'))
          return
        }
        await this.submit()
      })
    ...
    buildRequest() {
      ...
      this.shiftRows.forEach((row, index) => {
        params[CLASS_PLAN_FIELDS[index]] = this.isChangeQtyClass(row) ? Number(row.targetPlanQty) : null
      })
      return params
    },
    async submit() {
      ...
      try {
        ...
        if (this.handleBatchCheckResponse(response)) {
          return
        }
        if (response.needConfirm) {
          ...
          try {
            await this.$confirm(
              ...
              this.$t('ui.data.column.cd90ScheduleResult.changeQtyConfirmTitle'),
              {
    ...
    buildCarryoverConfirmHtml(details, fallbackMsg) {
      ...
      carryoverDetails.forEach(item => {
        const targetClass = item.targetClassField || this.$t('ui.data.column.cd90ScheduleResult.carryoverWindowEnd')
        ...
        html += '，' + this.$t('ui.data.column.cd90ScheduleResult.carryoverQty') + ' ' + this.escapeHtml(item.carryoverQty)
    ...
    showBatchCheckAlert(data, fallbackMsg) {
      ...
      html += '</div>'
      this.$alert(html, this.$t('ui.data.column.cd90ScheduleResult.batchCheckTitle'), {
        dangerouslyUseHTMLString: true,
    ...
    formatQty(value) {
      return value || value === 0 ? Number(value).toFixed(3).replace(/\.0+$/, '').replace(/(\.\d*?)0+$/, '$1') : '-'
    },
    escapeHtml(value) {
      const characters = { '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;', "'": '&#39;' }
