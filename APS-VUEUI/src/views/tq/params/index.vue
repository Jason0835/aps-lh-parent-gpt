<template>
  <basic-container>
    ...
    >
      <template slot="header">
        <el-button
          type="primary"
          plain
          v-hasPermi="['tq:params:add']"
          @click="handleAdd"
          >{{ $t("ui.frame.btn.add") }}</el-button
        >
        <el-button
          type="danger"
          v-hasPermi="['tq:params:remove']"
          :disabled="selection.length == 0"
          @click="handleDeleteAll"
          >{{ $t("ui.frame.btn.delete") }}</el-button
        >
        <el-button
          v-hasPermi="['tq:params:import']"
          @click="$refs.tltUpload.handleImport()"
          >{{ $t("ui.frame.btn.import") }}</el-button
        >
        <el-button
          @click="handleExport"
          v-hasPermi="['tq:params:export']"
    ...
    <tlt-upload-form
      ref="tltUpload"
      :updateSupport="true"
      downloadUrl="/tq/params/importTemplate"
      uploadUrl="/tq/params/importData"
...
//interface
import {listParams, removeParams, saveParams} from "@/api/tq/params";
//components
...
export default {
    ...
    columns() {
      let columns = [
        ...
        },
        {
          prop: "paramGroup",
          halign: "center",
          label: this.$t("ui.data.column.tq.params.paramGroup"),
          formatter: (row, column, value) => {
            const map = { GLOBAL: "全局参数", SHIFT: "班次参数", MACHINE: "机台参数", BEAD: "胎圈参数" };
        ...
        },
        {
          prop: "valueType",
          halign: "center",
          label: this.$t("ui.data.column.tq.params.valueType"),
          formatter: (row, column, value) => {
        ...
        },
        {
          prop: "enableStatus",
          halign: "center",
          label: this.$t("ui.data.column.tq.params.enableStatus"),
          render: ({ row }) => {
            return (
              <el-switch
                active-value="1"
  ...
  methods: {
    ...
    handleExport() {
      downloadLink("/tq/params/export", this.formatParams(false));
    },
