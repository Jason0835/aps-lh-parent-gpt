<template>
  <basic-container>
    <page-table
      ...
      :selectArea="false"
      :showSummary="false"
      tableRef="tcGlueMachineRealMainTable"
    ...
    >
      <template slot="header">
        <el-button
          v-hasPermi="['tc:tcGlueMachineReal:edit']"
          plain
          type="primary"
          @click="handleAdd"
        >{{ $t("ui.frame.btn.add") }}</el-button>
        <el-button
          v-hasPermi="['tc:tcGlueMachineReal:remove']"
          :disabled="selection.length == 0"
          type="danger"
          @click="handleDeleteAll"
        >{{ $t("ui.frame.btn.delete") }}</el-button>
        <el-button
          v-hasPermi="['tc:tcGlueMachineReal:import']"
          @click="$refs.tltUpload.handleImport()"
        >{{ $t("ui.frame.btn.import") }}</el-button>
        <el-button
          v-hasPermi="['tc:tcGlueMachineReal:export']"
          @click="handleExport"
    ...
    <tlt-upload-form
      ...
      :updateSupport="true"
      downloadUrl="/tc/tcGlueMachineReal/importTemplate"
      labelWidth="0"
      uploadUrl="/tc/tcGlueMachineReal/importData"
...
import {listGlueMachineReal, removeGlueMachineReal, saveGlueMachineReal} from "@/api/tc/glueMachineReal";
import tltUpload from "@/components/tltUpload/tltUpload.vue";
import TltUploadForm from "@/views/components/tltUploadForm.vue";
import infoDialog from "./components/infoDialog.vue";

export default {
  name: "/tc/tcGlueMachineReal",
  components: {
    tltUpload,
    infoDialog,
    TltUploadForm,
    ...
    columns() {
      return [
        ...
        },
        {
          prop: "machineName",
          halign: "center",
          label: this.$t("ui.data.column.tcGlueMachineReal.machineName"),
        },
        {
          prop: "machineCode",
          halign: "center",
  ...
  methods: {
    ...
    handleDelete(row) {
      this.$confirm(this.$t("common.confirm.delete"), {
        ...
        const ids = row.id;
        removeGlueMachineReal({ ids }).then((data) => {
          this.$modal.msgSuccess(data.msg);
    ...
    handleExport() {
      downloadLink("/tc/tcGlueMachineReal/export", this.formatParams(false));
    },
    ...
    async getList() {
      try {
        this.loading = true;
        const data = await listGlueMachineReal(this.formatParams());
        this.data = data.rows;
