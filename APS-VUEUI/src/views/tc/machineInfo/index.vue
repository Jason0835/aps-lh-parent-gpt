<template>
  <basic-container>
    <page-table
      ...
      :showSummary="false"
      tableRef="tcMachineInfoMainTable"
    ...
    >
      <template slot="header">
        <el-button
          v-hasPermi="['tc:tcMachineInfo:edit']"
          plain
          type="primary"
          @click="handleAdd"
        ...
        >
        <el-button
          v-hasPermi="['tc:tcMachineInfo:remove']"
        ...
        >
        <el-button
          v-hasPermi="['tc:tcMachineInfo:import']"
          @click="$refs.tltUpload.handleImport()"
          >{{ $t("ui.frame.btn.import") }}</el-button
        >
        <el-button
          v-hasPermi="['tc:tcMachineInfo:export']"
          @click="handleExport"
    ...
    <tlt-upload-form
      ...
      :updateSupport="true"
      downloadUrl="/tc/tcMachineInfo/importTemplate"
      labelWidth="0"
      uploadUrl="/tc/tcMachineInfo/importData"
...
//interface
import {listTcMachineInfo, removeTcMachineInfo, saveTcMachineInfo} from "@/api/tc/machineInfo";
//components
                ...
                onChange={(val) => {
                  ...
                  this.$confirm(confirmMsg, { type: "warning" }).then(
                    async () => {
                      try {
                        this.loading = true;
                        const data = await saveTcMachineInfo({
                          ...row,
                          machineStatus: val,
                        });
  ...
  methods: {
    ...
    handleDelete(row) {
      this.$confirm(this.$t("common.confirm.delete"), {
        ...
        const ids = row.id;
        removeTcMachineInfo({ ids }).then((data) => {
          this.$modal.msgSuccess(data.msg);
    ...
    handleExport() {
      downloadLink("/tc/tcMachineInfo/export", this.formatParams(false));
    },
    ...
    async getList() {
      try {
        this.loading = true;
        const data = await listTcMachineInfo(this.formatParams());
        this.data = data.rows;
