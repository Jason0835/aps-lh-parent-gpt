...
<template>
  <basic-container>
    <page-table
      tableRef="cut90ScheduleMainTable"
      :calcHeight="true"
      v-loading="loading"
      :columns="columns"
    ...
    >
      <template slot="header">
        <el-button
          type="warning"
          v-hasPermi="['cd90:cd90ScheduleResult:autoPlan']"
          @click="handleAutoPlan"
          >{{ $t("ui.data.column.scheduleResult.autoPlan") }}</el-button
        >
        <el-button
          type="warning"
          v-hasPermi="['cd90:cd90ScheduleResult:add']"
          @click="handleAdd"
        ...
        <el-button
          ...
          v-hasPermi="['cd90:cd90ScheduleResult:edit']"
          >{{ $t("ui.frame.btn.modify") }}</el-button
        >
        <el-button
          type="danger"
          :disabled="selection.length === 0"
          v-hasPermi="['cd90:cd90ScheduleResult:remove']"
          @click="handleDelete"
          >{{ $t("ui.frame.btn.delete") }}</el-button
        >
        <el-button
          v-hasPermi="['cd90:cd90ScheduleResult:changeMachine']"
          type="primary"
          :disabled="selection.length === 0"
        ...
        >
        <el-button
          v-hasPermi="['cd90:cd90ScheduleResult:changePlan']"
          type="primary"
          @click="handleChangePlan"
          >{{ $t("ui.data.column.scheduleResult.changePlan") }}</el-button
        >
        <el-button
          v-hasPermi="['cd90:cd90ScheduleResult:balance']"
          type="primary"
          @click="handleBalance"
          >{{ $t("ui.data.column.scheduleResult.balance") }}</el-button
        >
        <el-button
          v-hasPermi="['cd90:cd90ScheduleResult:combinationMiddleAndNight']"
          type="primary"
          :disabled="selection.length == 0"
        ...
        <el-button
          v-hasPermi="['cd90:cd90ScheduleResult:publish']"
          type="primary"
          :disabled="selection.length === 0"
          @click="handlePublish"
        ...
        <el-dropdown>
          ...
          <el-dropdown-menu slot="dropdown">
            ...
            <el-dropdown-item v-hasPermi="['cd90:finishQty:import']">
              <el-button
                type="primary"
                class="more-btn"
    ...
    <tlt-upload-form
      ref="tltUploadForm"
      title="导入纤维直裁排程结果数据"
      downloadUrl="/cd90/cd90ScheduleResult/importTemplate"
      uploadUrl="/cd90/cd90ScheduleResult/importScheduleData"
    ...
    <tlt-upload-form
      ref="tltUploadForm2"
      title="导入纤维直裁排程结果数据"
      uploadUrl="/cd90/cd90ScheduleResult/importScheduleData2"
      @uploadSuccess="getList"
      :columns="[
        {
          label: '排程日期',
    ...
    <tlt-upload-form
      ref="tltUploadForm3"
      title="导入纤维直裁排程结果数据"
      uploadUrl="/cd90/cd90ScheduleResult/importFinishQty"
      downloadUrl="/cd90/cd90ScheduleResult/importFinishQtyTemplate"
      @uploadSuccess="getList"
      :columns="[
        {
          label: '排程日期',
...
export default {
        ...
        { type: "selection", fixed: "left" },
        {
          ...
          children: [
            ...
            ,
            {
              ...
              align: "center",
              //  sortable: "custom",
              label: this.$t("ui.data.column.scheduleResult.edgeGlue.br"),
            },
            ,
            {
              prop: "machineId",
  ...
  methods: {
    ...
    handleExportUiExcel() {
      downloadLink("/cd90/cd90ScheduleResult/export", this.formatParams(false));
    },
