...
<template>
  <basic-container>
    <page-table
      tableRef="cut90FixedPointMachineMainTable"
      :calcHeight="true"
      v-loading="loading"
      :columns="columns"
    ...
    >
      <template slot="header">
        <el-button
          type="primary"
          plain
          v-hasPermi="['cd90:specifyMachine:add']"
          @click="handleAdd"
          >{{ $t("ui.frame.btn.add") }}</el-button
        >
        <!-- <el-button
          type="warning"
          v-hasPermi="['cd90:specifyMachine:edit']"
          @click="handleEdit(selection[0])"
          >{{ $t("ui.frame.btn.modify") }}</el-button
        > -->
        <el-button
          v-hasPermi="['cd90:specifyMachine:import']"
          @click="$refs.tltUpload.handleImport()"
          >{{ $t("ui.frame.btn.import") }}</el-button
        >
        <el-button
          @click="handleExport"
          v-hasPermi="['cd90:specifyMachine:export']"
    ...
    <tlt-upload
      ref="tltUpload"
      downloadUrl="/cd90/specifyMachine/importTemplate"
      uploadUrl="/cd90/specifyMachine/importData"
...
export default {
    ...
    columns() {
      let columns = [
        { type: "selection", fixed: "left" },
        {
          ...
          align: "center",
          halign: "center",
          label: this.$t("ui.common.column.lb.clothCode"),
          // sortable: "custom",
        },
        {
          prop: "machineName",
          ...
          render: ({ row }) => {
            return (
              <div>
                ...
                <el-button
                  v-hasPermi={["cd90:specifyMachine:remove"]}
                  class="minus"
                  type="danger"
  ...
  methods: {
    ...
    handleExport() {
      downloadLink("/cd90/specifyMachine/export", this.formatParams(false));
    },
