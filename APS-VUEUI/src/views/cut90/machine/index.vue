...
<template>
  <basic-container>
    <page-table
      tableRef="cut90MachineMainTable"
      :calcHeight="true"
      v-loading="loading"
      :columns="columns"
    ...
    >
      <template slot="header">
        ...
        > -->
        <el-button
          v-hasPermi="['cd90:machine:import']"
          @click="() => $refs.tltUploadForm.handleImport(importDefaultValue)"
          >{{ $t("ui.frame.btn.import") }}</el-button
        >
        <el-button v-hasPermi="['cd90:machine:export']" @click="handleExport">{{
    ...
    <tlt-upload-form
      ref="tltUploadForm"
      title="导入机台信息数据"
      downloadUrl="/cd90/machine/importTemplate"
      uploadUrl="/cd90/machine/importData"
  ...
  computed: {
    columns() {
      return [
        ...
        {
          label: this.$t("ui.data.column.machine.isStickFilm"),
          prop: "isStickFilm",
          minWidth: 100,
          // sortable: "custom",
          render: ({ row }) => {
            return this.selectDictLabel(this.dict.type.IS_SUPPORTED, row.isStickFilm);
          },
        },
        {
          label: this.$t("ui.data.column.machine.clothWithMax"),
          prop: "clothWithMax",
          minWidth: 100,
          // sortable: "custom",
          type: "number",
        ...
        {
          label: this.$t("ui.data.column.machine.clothWithMin"),
          prop: "clothWithMin",
          minWidth: 100,
          // sortable: "custom",
          type: "number",
