...
<script>
import { addGdyyShiftConfig, updateGdyyShiftConfig } from "@/api/gdyy/gdyyShiftConfig";
          ...
          requiredInput,
          {
            validator: (rule, value, callback) => {
              if (value === undefined || value === null || value === "" || Number(value) <= 0) {
                callback(new Error(this.$t("ui.data.alert.gdyyShiftConfig.shiftHoursPositive")));
              } else {
                callback();
              }
            },
    ...
    async save(params) {
      ...
      try {
        const res = this.isEdit
          ? await updateGdyyShiftConfig(params)
          : await addGdyyShiftConfig(params);
