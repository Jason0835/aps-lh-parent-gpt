...
import { saveScheduleResult } from "@/api/gsq/scheduleResult";
...
export default {
  ...
  computed: {
    ...
    title() {
      return this.$t("ui.data.column.gsqScheduleResult.editTitle");
    },
    columns() {
      return [
        {
    ...
    visible(val) {
      if (val && this.row) {
        this.form = {
          ...
          twiningDiscCode: this.row.twiningDiscCode,
      ...
      saveScheduleResult(this.form)
        .then((res) => {
          ...
          this.$modal.msgSuccess(
            tip || this.$t("ui.data.column.gsqScheduleResult.editSuccess")
          );
          this.$emit("refresh");
          this.hide();
        })
