  ...
  <basic-container>
    ...
    >
      <template slot="header">
        ...
        >
        <el-button
          v-hasPermi="['gsq:machine:import']"
          @click="() => $refs.tltUploadForm.handleImport(importDefaultValue)"
          >{{ $t("ui.frame.btn.import") }}</el-button
        >
        <el-button v-hasPermi="['gsq:machine:export']" @click="handleExport">{{
    ...
    <tlt-upload-form
      ref="tltUploadForm"
      title="导入钢丝圈机台信息数据"
      downloadUrl="/gsq/machine/importTemplate"
      uploadUrl="/gsq/machine/importData"
...
export default {
  ...
  methods: {
    ...
    handleExport() {
      this.$confirm(this.$t("确定导出所有钢丝圈机台信息？"), {
        type: "warning",
      }).then(() => {
        try {
          this.loading = true;
