...
import { autoScheduleResult } from "@/api/xwyy/xwyyScheduleResult";
...
export default {
  ...
  computed: {
    title() {
      return this.$t("ui.data.column.xwyyScheduleResult.autoScheduleTitle");
    },
    columns() {
      return [
        {
  ...
  methods: {
    ...
    async save(params) {
      ...
      try {
        ...
        if (needConfirm) {
          this.loading = false;
          try {
            const confirmText = msg || this.$t("ui.data.column.xwyyScheduleResult.autoScheduleConfirm");
    ...
    showBatchCheckAlert(data, fallbackMsg) {
      ...
      this.$alert(html, this.$t("ui.data.column.cxScheduleResult.scheduleFailed"), {
        ...
        type: 'error',
        customClass: 'xwyy-auto-schedule-batch-check',
...
<style>
.xwyy-auto-schedule-batch-check {
...
.xwyy-auto-schedule-batch-check .el-message-box__content {
  flex: 1;
  overflow: hidden;
  display: flex;
  flex-direction: column;
}
.xwyy-auto-schedule-batch-check .el-message-box__message {
  flex: 1;
  overflow-y: auto;
