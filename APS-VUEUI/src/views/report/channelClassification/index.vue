...
<template>
  <basic-container>
    <page-table
      tableRef="ChannelClassificationMainTable"
      :calcHeight="true"
      v-loading="loading"
      :columns="columns"
    ...
    >
      <template slot="header">
        <el-button
          @click="handleExport"
          v-hasPermi="['report:channelClassification:export']"
...
export default {
  ...
  methods: {
    ...
    handleExport() {
      exportChannelClassification(this.formatParams(false));
    },
    ...
    async getList() {
      try {
        this.loading = true;
        const res = await listChannelClassification(this.formatParams());
        // console.log()
