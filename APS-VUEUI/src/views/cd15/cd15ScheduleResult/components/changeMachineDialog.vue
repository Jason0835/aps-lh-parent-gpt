  ...
  methods: {
        ...
        if (response.needConfirm) {
          ...
          try {
            await this.$confirm(
              ...
              this.$t('ui.data.column.cd15ScheduleResult.transferCarryoverTitle'),
              {
    ...
    buildCarryoverConfirmHtml(details, fallbackMsg) {
      ...
      ;(details || []).forEach(item => {
        const targetClass = item.targetClassField || this.$t('ui.data.column.cd15ScheduleResult.transferWindowEnd')
        ...
        html += '<br/>' + this.$t('ui.data.column.cd15ScheduleResult.transferCarryoverQty') + ' '
