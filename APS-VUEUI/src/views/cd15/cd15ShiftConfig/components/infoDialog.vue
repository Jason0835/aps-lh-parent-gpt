...
<script>
import { addCd15ShiftConfig, updateCd15ShiftConfig } from "@/api/cd15/cd15ShiftConfig";
          ...
          requiredInput,
          {
            validator: (rule, value, callback) => {
              if (value === undefined || value === null || value === "" || Number(value) <= 0) {
                callback(new Error(this.$t("ui.data.alert.cd15ShiftConfig.shiftHoursPositive")));
              } else {
                callback();
              }
            },
    ...
    async save(params) {
      ...
      try {
        const res = this.isEdit
          ? await updateCd15ShiftConfig(params)
          : await addCd15ShiftConfig(params);
