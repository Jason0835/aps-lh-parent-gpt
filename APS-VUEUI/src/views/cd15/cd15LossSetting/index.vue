  ...
  <basic-container>
    ...
    >
      <template slot="header">
        <el-button type="primary" plain v-hasPermi="['cd15:lossSetting:add']" @click="handleAdd">{{ $t("ui.frame.btn.add") }}</el-button>
        <el-button v-hasPermi="['cd15:lossSetting:edit']" @click="handleBatchEdit" :disabled="selection.length !== 1">{{ $t("ui.frame.btn.update") }}</el-button>
        <el-button type="danger" v-hasPermi="['cd15:lossSetting:remove']" :disabled="selection.length === 0" @click="handleBatchDelete">{{ $t("ui.frame.btn.delete") }}</el-button>
        <el-button v-hasPermi="['cd15:lossSetting:import']" @click="$refs.tltUpload.handleImport()">{{ $t("ui.frame.btn.import") }}</el-button>
        <el-button @click="handleExport" v-hasPermi="['cd15:lossSetting:export']">{{ $t("ui.frame.btn.export") }}</el-button>
      </template>
    </page-table>
    <tlt-upload-form ref="tltUpload" :updateSupport="true" downloadUrl="/cd15/cd15LossSetting/importTemplate" uploadUrl="/cd15/cd15LossSetting/importData" @uploadSuccess="getList" labelWidth="0" :columns="importColumns" />
    <info-dialog ref="infoRef" :steel-strip-options="steelStripOptions" @success="getList" />
  </basic-container>
</template>

<script>
import { listLossSetting, delLossSetting, exportLossSetting } from "@/api/cd15/lossSetting";
...
export default {
  ...
  data() {
    return {
      ...
      loading: false, data: [], selection: [], steelStripOptions: [], machineOptions: [],
  ...
  computed: {
    columns() {
      return [
        ...
        { prop: "factoryCode", align: "center", halign: "center", label: this.$t("ui.data.column.cd15LossSetting.factoryCode"), minWidth: 120, formatter: (row, column, value) => this.selectDictLabel(this.dict.type.biz_factory_name, value) },
        { prop: "steelStripCode", align: "center", halign: "center", label: this.$t("ui.data.column.cd15LossSetting.steelStripCode"), minWidth: 140 },
        { prop: "machineCode", align: "center", halign: "center", label: this.$t("ui.data.column.cd15LossSetting.machineCode"), minWidth: 140 },
        { prop: "lossRate", align: "center", halign: "center", label: this.$t("ui.data.column.cd15LossSetting.lossRate"), minWidth: 130 },
        ...
        { align: "center", halign: "center", label: this.$t("ui.data.btn.option"), prop: "option", minWidth: 160, width: 160, fixed: "right",
          render: ({ row }) => { return (<div><el-button v-hasPermi={["cd15:lossSetting:edit"]} class="minus" type="success" onClick={() => this.handleEdit(row)}>{this.$t("ui.frame.btn.update")}</el-button><el-button v-hasPermi={["cd15:lossSetting:remove"]} class="minus" type="danger" onClick={() => this.handleDelete(row)}>{this.$t("ui.frame.btn.delete")}</el-button></div>); } },
      ];
    },
    searchColumns() {
      return [
  ...
  created() { this.getList(); this.loadSteelStripOptions(); this.loadMachineOptions(); },
};
</script>
