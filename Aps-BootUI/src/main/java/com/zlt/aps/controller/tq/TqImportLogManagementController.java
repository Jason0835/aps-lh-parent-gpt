...
import com.zlt.aps.tq.api.service.ITqImportLogManagementService;
 ...
 */
@Controller
@RequestMapping("/tq/importLogManagement")
@Api(tags = {"工序导入日志管理接口"})
public class TqImportLogManagementController extends BaseController {
    private final String prefix = "tq/importLogManagement";

    @Autowired
    private ITqImportLogManagementService iTqImportLogManagementService;

    @Autowired
    private IApsFileService iApsFileService;

    @RequiresPermissions("tq:importLogManagement:view")
    @GetMapping()
     ...
     * 查询工序导入日志管理列表
     */
    @RequiresPermissions("tq:importLogManagement:list")
    @ApiOperation("查询工序导入日志管理列表")
    @PostMapping("/list")
    @ResponseBody
    public TableDataInfo list(TqImportLogManagementDto dto) {
        return iTqImportLogManagementService.list(dto);
    }
    ...
    public String getImportLogManagement(@PathVariable("id") Long id, ModelMap mmap) {
        mmap.put("Params", iTqImportLogManagementService.getImportLogManagement(id));
        return prefix + "/edit";
    }
    ...
    @ResponseBody
    public TableDataInfo detailList(TqImportErrorLogManagementDto dto) {
        return iTqImportLogManagementService.getImportErrorLogManagement(dto);
    }
