...
public class Cd90StockUIController extends BaseUIController<Cd90Stock> {
    @Resource
    private ICd90StockRemoteService remote;

    @ApiOperation("查询列表")
    @RequiresPermissions("cd90:stock:list")
    @PostMapping("/list")
    @ResponseBody
    public TableDataInfo list(Cd90Stock q) {
    ...
    public AjaxResult add(@RequestBody Cd90Stock e) {
        if (UserConstants.NOT_UNIQUE.equals(remote.checkUnique(e)))
            return AjaxResult.error(I18nUtil.getMessage("ui.data.column.cd90Stock.checkUnique"));
        return remote.add(e);
    }
    ...
    public String getFunctionName() {
        return I18nUtil.getMessage("ui.data.column.cd90Stock.modelName");
    }

    @ApiOperation("下载导入模板")
    @Override
    public AjaxResult importTemplate(HttpServletResponse r) throws IOException {
        ExcelUtil<Cd90Stock> u = new ExcelUtil<>(Cd90Stock.class);
    ...
    @ResponseBody
    @Override
    public void export(HttpServletResponse r, Cd90Stock e) throws IOException {
