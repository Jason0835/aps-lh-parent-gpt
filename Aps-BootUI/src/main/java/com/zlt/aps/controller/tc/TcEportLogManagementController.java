...
import com.zlt.aps.tc.api.service.ITcExportLogManagementService;
 ...
 */
@Controller
@RequestMapping("/tc/exportLogManagement")
@Api(tags = {"工序导出日志管理接口"})
public class TcEportLogManagementController extends BaseController {
    private final String prefix = "tc/exportLogManagement";

    @Autowired
    private ITcExportLogManagementService iTcExportLogManagementService;

    @Autowired
    private IApsFileService iApsFileService;


    @RequiresPermissions("tc:exportLogManagement:view")
    @GetMapping()
     ...
     * 查询工序导出日志管理列表
     */
    @RequiresPermissions("tc:exportLogManagement:list")
    @ApiOperation("查询工序导出日志管理列表")
    @PostMapping("/list")
    @ResponseBody
    public TableDataInfo list(TcExportLogManagementDto dto) {
        return iTcExportLogManagementService.list(dto);
    }
    ...
    public String getExportLogManagement(@PathVariable("id") Long id, ModelMap mmap) {
        mmap.put("Params", iTcExportLogManagementService.getExportLogManagement(id));
        return prefix + "/edit";
    }
