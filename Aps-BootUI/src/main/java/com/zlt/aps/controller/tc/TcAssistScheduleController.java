...
import com.zlt.aps.tc.api.service.ITcAssistScheduleService;
...
public class TcAssistScheduleController extends BaseController {
    ...
    @Autowired
    private ITcAssistScheduleService iTcAssistScheduleService;
    @Autowired
    private IExportLogService iExportLogService;

    private final String prefix = "tc/assistSchedule";

    /**
     * 跳转至主页面
     */
    @RequiresPermissions("tc:assistSchedule:view")
    @GetMapping()
     ...
     * 根据条件查询胎侧外协排程结果列表
     */
    @ApiOperation("根据条件查询胎侧外协排程结果列表")
    @RequiresPermissions("tc:assistSchedule:list")
    @PostMapping("/list")
    @ResponseBody
    public TableDataInfo list(TcAssistSchedule entity) {
        ...
        return iTcAssistScheduleService.list(entity);
    }

    /**
     * 导出胎侧外协排程结果
     */
    @ApiOperation("导出胎侧外协排程结果")
    @RequiresPermissions("tc:assistSchedule:export")
    @GetMapping("/export")
    @ResponseBody
    public void export(HttpServletResponse response, TcAssistSchedule assistSchedule) throws Exception {
        //若是没传日期则默认查询当日排程
        ...
        //获取字节流数据
        byte[] data = iTcAssistScheduleService.export(assistSchedule);
        if (data == null) {
            return;
        }
        String fileName = I18nUtil.getMessage("ui.data.column.tc.assistSchedule.modelName");
        ExportLog exportLog = ExportUtil.uploadAndExportExcelByByte(response, data, fileName, assistSchedule.toString(), ApsConstant.PROCEDURE_CODE_TC);
