...
import com.zlt.aps.gsq.api.service.IGsqImportLogManagementService;
...
public class GsqImportLogManagementController extends BaseController {
    private final String prefix = "gsq/importLogManagement";

    @Autowired
    private IGsqImportLogManagementService iGsqImportLogManagementService;

    @Autowired
    private IApsFileService iApsFileService;

    @RequiresPermissions("gsq:importLogManagement:view")
    @GetMapping()
     ...
     * 查询工序导入日志管理列表
     */
    @RequiresPermissions("gsq:importLogManagement:list")
    @ApiOperation("查询工序导入日志管理列表")
    @PostMapping("/list")
    @ResponseBody
    public TableDataInfo list(GsqImportLogManagementDto dto) {
        return iGsqImportLogManagementService.list(dto);
    }
    ...
    public String getImportLogManagement(@PathVariable("id") Long id, ModelMap mmap) {
        mmap.put("Params", iGsqImportLogManagementService.getImportLogManagement(id));
        return prefix + "/edit";
    }
    ...
    @ResponseBody
    public TableDataInfo detailList(GsqImportErrorLogManagementDto dto) {
        return iGsqImportLogManagementService.getImportErrorLogManagement(dto);
    }
