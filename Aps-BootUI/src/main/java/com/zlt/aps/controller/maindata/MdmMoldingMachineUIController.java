...
@Api(tags = "成型机信息管理")
@Controller
@RequestMapping("/maindata/mdmMoldingMachine")
public class MdmMoldingMachineUIController extends BaseUIController<MdmMoldingMachine> {
    ...
    private final String prefix = "aps/maindata/mdmMoldingMachine";

    /**
     * 跳转至主页面
     */
    @RequiresPermissions("maindata:mdmMoldingMachine:view")
    @GetMapping()
    ...
    @RequiresPermissions("maindata:mdmMoldingMachine:list")
    @PostMapping("/list")
    @ResponseBody
     ...
     */
    @ApiOperation("修改或新增")
    @RequiresPermissions("maindata:mdmMoldingMachine:edit")
    @PostMapping("/save")
    @ResponseBody
    public AjaxResult save(MdmMoldingMachine mdmMoldingMachine) {
        if (UserConstants.NOT_UNIQUE.equals(iMdmMoldingMachineService.checkUnique(mdmMoldingMachine))) {
            return AjaxResult.error(I18nUtil.getMessage("ui.data.alert.mdmMoldingMachine.notUnique"));
        }
    ...
    public AjaxResult importData(@RequestPart("file") MultipartFile file, boolean updateSupport) throws Exception {
        ...
        AjaxResult ajaxResult = iMdmMoldingMachineService.importData(context, true);
        return ajaxResult;
    }
}
