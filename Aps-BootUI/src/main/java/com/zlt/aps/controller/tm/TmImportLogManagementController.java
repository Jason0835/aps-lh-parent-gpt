...
import com.zlt.aps.tm.api.service.ITmImportLogManagementService;
...
public class TmImportLogManagementController extends BaseController {
    private final String prefix = "tm/importLogManagement";

    @Autowired
    private ITmImportLogManagementService iTmImportLogManagementService;

    @Autowired
    private IApsFileService iApsFileService;

    @RequiresPermissions("tm:importLogManagement:view")
    @GetMapping()
     ...
     * 查询工序导入日志管理列表
     */
    @RequiresPermissions("tm:importLogManagement:list")
    @ApiOperation("查询工序导入日志管理列表")
    @PostMapping("/list")
    @ResponseBody
    public TableDataInfo list(TmImportLogManagementDto dto) {
        return iTmImportLogManagementService.list(dto);
    }
    ...
    public String getImportLogManagement(@PathVariable("id") Long id, ModelMap mmap) {
        mmap.put("Params", iTmImportLogManagementService.getImportLogManagement(id));
        return prefix + "/edit";
    }
    ...
    @ResponseBody
    public TableDataInfo detailList(TmImportErrorLogManagementDto dto) {
        return iTmImportLogManagementService.getImportErrorLogManagement(dto);
    }
