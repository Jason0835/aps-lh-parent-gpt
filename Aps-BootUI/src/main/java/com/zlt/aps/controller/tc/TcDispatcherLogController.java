...
import com.zlt.aps.tc.api.service.ITcDispatcherLogService;
...
public class TcDispatcherLogController extends BaseController {
    ...
    @Autowired
    private ITcDispatcherLogService dispatcherLogService;
    @Autowired
    ...
    private final String prefix = "tc/dispatcherLog";

    /**
     * 跳转至主页面
     */
    @RequiresPermissions("tc:dispatcherLog:view")
    @GetMapping()
    public String toIndex(ModelMap mmap) {
        mmap.put("initDate", DateUtils.parseDateToStr("yyyy-MM-dd", new Date()));  //当前日期+1天
        return prefix + "/log";
    }
     ...
     * 根据条件查询调度员排程操作日志列表
     */
    @ApiOperation("根据条件查询调度员排程操作日志列表")
    @RequiresPermissions("tc:dispatcherLog:list")
    @PostMapping("/list")
    @ResponseBody
    public TableDataInfo list(TcDispatcherLog entity) {
    ...
    @RequiresPermissions("tc:dispatcherLog:export")
    @GetMapping("/export")
    @ResponseBody
    public void export(HttpServletResponse response, TcDispatcherLog dispatcherLog) throws IOException {
        ...
        ExportLog exportLog = ExportUtil.uploadAndExportExcelByByte(response, data, fileName, dispatcherLog.toString(), ApsConstant.PROCEDURE_CODE_TC);
