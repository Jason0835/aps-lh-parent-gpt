...
import com.zlt.aps.cd15.api.service.ICd15ExportLogManagementService;
...
@Api(tags = {"工序导出日志管理接口"})
public class Cd15EportLogManagementController extends BaseController {
    private final String prefix = "cd15/exportLogManagement";

    @Autowired
    private ICd15ExportLogManagementService iCd15ExportLogManagementService;

    @Autowired
    private IApsFileService iApsFileService;

    @RequiresPermissions("cd15:exportLogManagement:view")
    @GetMapping()
     ...
     * 查询工序导出日志管理列表
     */
    @RequiresPermissions("cd15:exportLogManagement:list")
    @ApiOperation("查询工序导出日志管理列表")
    @PostMapping("/list")
    @ResponseBody
    public TableDataInfo list(Cd15ExportLogManagementDto dto) {
        return iCd15ExportLogManagementService.list(dto);
    }
    ...
    public String getExportLogManagement(@PathVariable("id") Long id, ModelMap mmap) {
        mmap.put("Params", iCd15ExportLogManagementService.getExportLogManagement(id));
        return prefix + "/edit";
    }
