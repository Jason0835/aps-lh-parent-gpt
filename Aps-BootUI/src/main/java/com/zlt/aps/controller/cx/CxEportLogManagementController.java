...
import com.zlt.aps.cx.api.service.ICxExportLogManagementService;
...
@Api(tags = {"工序导出日志管理接口"})
public class CxEportLogManagementController extends BaseController {
    private final String prefix = "cx/exportLogManagement";

    @Autowired
    private ICxExportLogManagementService iCxExportLogManagementService;

    @Autowired
    private IApsFileService iApsFileService;

    @RequiresPermissions("cx:exportLogManagement:view")
    @GetMapping()
     ...
     * 查询工序导出日志管理列表
     */
    @RequiresPermissions("cx:exportLogManagement:list")
    @ApiOperation("查询工序导出日志管理列表")
    @PostMapping("/list")
    @ResponseBody
    public TableDataInfo list(CxExportLogManagementDto dto) {
        return iCxExportLogManagementService.list(dto);
    }
    ...
    public String getExportLogManagement(@PathVariable("id") Long id, ModelMap mmap) {
        mmap.put("Params", iCxExportLogManagementService.getExportLogManagement(id));
        return prefix + "/edit";
    }
