...
@Api(tags = "钢丝圈参数设置")
@Controller
@RequestMapping("/gsq/gsqParams")
public class GsqParamsController extends BaseUIController<GsqParamsDto> {

    private final String prefix = "aps/gsq/gsqParams";

    @Autowired
    private IGsqParamsService iGsqParamsService;

    @RequiresPermissions("gsq:gsqParams:view")
    @GetMapping()
    public String toIndex() {
        return prefix + "/gsqParams";
    }

    @GetMapping("/add")
    public String add(ModelMap mmap) {
        mmap.put("gsqParams", new GsqParamsDto());
        return prefix + "/add";
    }
    ...
    @PostMapping("/list")
    @ResponseBody
    public TableDataInfo list(GsqParamsDto query) {
    ...
    @ApiOperation("保存")
    @PostMapping("/save")
    @RequiresPermissions("gsq:gsqParams:edit")
    @ResponseBody
    public AjaxResult save(GsqParamsDto gsqParams) {
        return iGsqParamsService.edit(gsqParams);
    }

    @ApiOperation("删除")
    @PostMapping("/remove")
    @RequiresPermissions("gsq:gsqParams:remove")
    @ResponseBody
    ...
    @ResponseBody
    public String checkUnique(@RequestBody GsqParamsDto query) {
        return iGsqParamsService.checkUnique(query);
    }

    @ApiOperation("导出数据")
    @GetMapping("/export")
    @RequiresPermissions("gsq:gsqParams:export")
    public void export(HttpServletResponse response, GsqParamsDto entity) throws IOException {
