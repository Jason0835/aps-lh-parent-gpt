...
public class Cd90StorageLaneLimitUIController extends BaseUIController<Cd90StorageLaneLimit> {
    ...
    @ApiOperation("新增") @RequiresPermissions("cd90:storageLaneLimit:add") @PostMapping("/add") @ResponseBody public AjaxResult add(@RequestBody Cd90StorageLaneLimit e) { if (UserConstants.NOT_UNIQUE.equals(remote.checkUnique(e))) return AjaxResult.error(I18nUtil.getMessage("ui.data.column.cd90StorageLaneLimit.checkUnique")); return remote.add(e); }
    @ApiOperation("编辑") @RequiresPermissions("cd90:storageLaneLimit:edit") @PostMapping("/edit") @ResponseBody public AjaxResult edit(@RequestBody Cd90StorageLaneLimit e) { if (UserConstants.NOT_UNIQUE.equals(remote.checkUnique(e))) return AjaxResult.error(I18nUtil.getMessage("ui.data.column.cd90StorageLaneLimit.checkUnique")); return remote.edit(e); }
    ...
    @Override public String getFunctionName() { return I18nUtil.getMessage("ui.data.column.cd90StorageLaneLimit.modelName"); }
    ...
    @ApiOperation("导出") @RequiresPermissions("cd90:storageLaneLimit:export") @GetMapping("/export") @ResponseBody @Override public void export(HttpServletResponse r, Cd90StorageLaneLimit e) throws IOException { byte[] b = remote.exportData(e, getExportTemplateFileName()); ExcelUtil.setResponseHeader(r, getExportTemplateFileName(), ".xlsx"); IOUtils.copy(new ByteArrayInputStream(b), r.getOutputStream()); r.flushBuffer(); }
