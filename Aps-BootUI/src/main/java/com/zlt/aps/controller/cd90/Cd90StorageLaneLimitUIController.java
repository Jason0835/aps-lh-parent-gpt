...
public class Cd90StorageLaneLimitUIController extends BaseUIController<Cd90StorageLaneLimit> {
    @Resource
    private ICd90StorageLaneLimitRemoteService remote;

    @ApiOperation("查询列表")
    @RequiresPermissions("cd90:storageLaneLimit:list")
    @PostMapping("/list")
    @ResponseBody
    public TableDataInfo list(Cd90StorageLaneLimit q) {
    ...
    @ApiOperation("新增")
    @RequiresPermissions("cd90:storageLaneLimit:add")
    @PostMapping("/add")
    @ResponseBody
    public AjaxResult add(@RequestBody Cd90StorageLaneLimit e) {
        if (UserConstants.NOT_UNIQUE.equals(remote.checkUnique(e)))
            return AjaxResult.error(I18nUtil.getMessage("ui.data.column.cd90StorageLaneLimit.checkUnique"));
        return remote.add(e);
    }

    @ApiOperation("编辑")
    @RequiresPermissions("cd90:storageLaneLimit:edit")
    @PostMapping("/edit")
    @ResponseBody
    ...
    @ApiOperation("删除")
    @RequiresPermissions("cd90:storageLaneLimit:remove")
    @PostMapping("/remove")
    @ResponseBody
    ...
    public String getFunctionName() {
        return I18nUtil.getMessage("ui.data.column.cd90StorageLaneLimit.modelName");
    }

    @ApiOperation("下载导入模板")
    @Override
    public AjaxResult importTemplate(HttpServletResponse r) throws IOException {
        ExcelUtil<Cd90StorageLaneLimit> u = new ExcelUtil<>(Cd90StorageLaneLimit.class);
        u.exportExcel(r, null, getExportTemplateFileName(), getExportTemplateFileName());
        return AjaxResult.success();
    }

    @ApiOperation("导出")
    @RequiresPermissions("cd90:storageLaneLimit:export")
    @GetMapping("/export")
    @ResponseBody
    @Override
    public void export(HttpServletResponse r, Cd90StorageLaneLimit e) throws IOException {
    ...
    @ApiOperation("导入")
    @RequiresPermissions("cd90:storageLaneLimit:import")
    @PostMapping("/importData")
    @ResponseBody
    @Override
    public AjaxResult importData(@RequestPart("file") MultipartFile f, boolean u) throws Exception {
