...
import com.zlt.aps.tq.api.service.ITqExportLogManagementService;
...
@Api(tags = {"工序导出日志管理接口"})
public class TqEportLogManagementController extends BaseController {
    private final String prefix = "tq/exportLogManagement";

    @Autowired
    private ITqExportLogManagementService iTqExportLogManagementService;

    @Autowired
    private IApsFileService iApsFileService;

    @RequiresPermissions("tq:exportLogManagement:view")
    @GetMapping()
     ...
     * 查询工序导出日志管理列表
     */
    @RequiresPermissions("tq:exportLogManagement:list")
    @ApiOperation("查询工序导出日志管理列表")
    @PostMapping("/list")
    @ResponseBody
    public TableDataInfo list(TqExportLogManagementDto dto) {
        return iTqExportLogManagementService.list(dto);
    }
    ...
    public String getExportLogManagement(@PathVariable("id") Long id, ModelMap mmap) {
        mmap.put("Params", iTqExportLogManagementService.getExportLogManagement(id));
        return prefix + "/edit";
    }
