...
public class ExportUtil {
    ...
    private static IApsFileService iApsFileService;

    //构造器注入iApsFileService（在配置类ExcelUtilConfig调用此构造方法）
    public ExportUtil(IApsFileService iApsFileService) {
        ExportUtil.iApsFileService = iApsFileService;
    }
    ...
    public static ExportLog uploadAndExportExcel(HttpServletResponse response, Workbook workbook, String fileName, String exportParams, String procedureCode) {
        ...
        try {
            ...
            if (data != null) {
                ...
                String pathFileName = "" ;
                try {
                    pathFileName = iApsFileService.uploadByteFile(data, "export");
                } catch (Exception e) {
