...
<template>
  <basic-container>
    <page-table
      tableRef="sidewallFixedPointMachineMainTable"
      :calcHeight="true"
      v-loading="loading"
      :columns="columns"
    ...
    >
      <template slot="header">
        <el-button
          type="primary"
          plain
          v-hasPermi="['tc:specifyMachine:add']"
          @click="handleAdd"
          >{{ $t("ui.frame.btn.add") }}</el-button
        >
        <!-- <el-button
          type="warning"
          v-hasPermi="['tc:specifyMachine:edit']"
          @click="handleEdit(selection[0])"
          >{{ $t("ui.frame.btn.modify") }}</el-button
        > -->
        <el-button
          v-hasPermi="['tc:specifyMachine:import']"
          @click="$refs.tltUpload.handleImport()"
          >{{ $t("ui.frame.btn.import") }}</el-button
        >
        <el-button
          @click="handleExport"
          v-hasPermi="['tc:specifyMachine:export']"
    ...
    <tlt-upload
      ref="tltUpload"
      downloadUrl="/tc/specifyMachine/importTemplate"
      uploadUrl="/tc/specifyMachine/importData"
...
export default {
  name: "SidewallFixedPointMachine",
  components: {
    tltUpload,
    infoDialog,
  },
    ...
    columns() {
      let columns = [
        { type: "selection", fixed: "left" },
        {
          ...
          align: "center",
          halign: "center",
          label: this.$t("ui.tc.specifyMachine.column.sidewallCode"),
          // sortable: "custom",
        },
        {
          prop: "machineName",
          ...
          render: ({ row }) => {
            return (
              <div>
                ...
                <el-button
                  v-hasPermi={["tc:specifyMachine:remove"]}
                  class="minus"
                  type="danger"
  ...
  methods: {
    ...
    handleExport() {
      downloadLink("/tc/specifyMachine/export", this.formatParams(false));
    },
