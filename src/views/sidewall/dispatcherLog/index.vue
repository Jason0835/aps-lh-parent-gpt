...
<template>
  <basic-container>
    <page-table
      tableRef="sidewallDispatcherLogMainTable"
      :calcHeight="true"
      v-loading="loading"
      :columns="columns"
    ...
    >
      <template slot="header">
        <!-- <el-button
          type="primary"
          v-hasPermi="['tc:dispatcherLog:add']"
          @click="handleAdd"
          >{{ $t("ui.frame.btn.add") }}</el-button
        > -->
        <!-- <el-button
          type="warning"
          v-hasPermi="['tc:dispatcherLog:edit']"
          @click="handleEdit(selection[0])"
          >{{ $t("ui.frame.btn.modify") }}</el-button
        > -->
        <!-- <el-button
          v-hasPermi="['tc:dispatcherLog:import']"
          @click="$refs.tltUpload.handleImport()"
          >{{ $t("ui.frame.btn.import") }}</el-button
        > -->
        <el-button
          @click="handleExport"
          v-hasPermi="['tc:dispatcherLog:export']"
    ...
    <!-- <tlt-upload
      ref="tltUpload"
      downloadUrl="/tc/dispatcherLog/importTemplate"
      uploadUrl="/tc/dispatcherLog/importData"
...
export default {
      ...
      let columns = [
        { type: "selection", fixed: "left" },
        {
          ...
          children: [
            ...
            {
              prop: "materialCode",
              label: this.$t("ui.data.column.tc.dispatcherlog.materialCode"),
              valign: "middle",
              align: "center",
              halign: "center",
             //  sortable: "custom",
  ...
  methods: {
    ...
    handleExport() {
      downloadLink("/tc/dispatcherLog/export", this.formatParams(false));
    },
