...
<template>
  <basic-container>
    <page-table
      tableRef="sidewallScheduleMainTable"
      :calcHeight="true"
      v-loading="loading"
      :columns="columns"
    ...
    >
      <template slot="header">
        <el-button
          type="warning"
          v-hasPermi="['tc:tcScheduleResult:autoPlan']"
          @click="handleAutoPlan"
          >{{ $t("ui.data.column.scheduleResult.autoPlan") }}</el-button
        >
        <el-button
          type="warning"
          v-hasPermi="['tc:tcScheduleResult:add']"
          @click="handleAdd"
        ...
        <el-dropdown>
          ...
          <el-dropdown-menu slot="dropdown">
            ...
            <el-dropdown-item v-hasPermi="['tc:finishQty:import']">
              <el-button
                type="primary"
                class="more-btn"
    ...
    <tlt-upload-form
      ref="tltUploadForm"
      title="导入排程结果数据"
      downloadUrl="/tc/tcScheduleResult/importTemplate"
      uploadUrl="/tc/tcScheduleResult/importScheduleData"
    ...
    <tlt-upload-form
      ref="tltUploadForm2"
      title="导入排程结果数据"
      uploadUrl="/tc/tcScheduleResult/importScheduleData2"
      @uploadSuccess="getList"
      :columns="[
        {
          label: '排程日期',
    ...
    <tlt-upload-form
      ref="tltUploadForm3"
      title="导入排程结果数据"
      uploadUrl="/tc/tcScheduleResult/importFinishQty"
      downloadUrl="/tc/tcScheduleResult/importFinishQtyTemplate"
      @uploadSuccess="getList"
      :columns="[
        {
          label: '排程日期',
...
export default {
  ...
  methods: {
    ...
    handleExportUiExcel() {
      downloadLink("/tc/tcScheduleResult/export", this.formatParams(false));
    },
    ...
    cellStyle({ row, column, rowIndex, columnIndex }) {
      if (column.property === "sidewallCode") {
        if (row.colorCode && row.colorType) {
          if (row.colorType == "0") {
