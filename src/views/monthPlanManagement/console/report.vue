...
export default {
  ...
  computed: {
    columns() {
      let columns = [
        ...
        {
          prop: "specCode",
          label: this.$t("ui.data.column.productionMonthPlanInit.specCode"),
        },
  ...
  methods: {
    ...
    async getList() {
      try {
        this.loading = true;
        const data = await listProduceVersionList(this.formatParams());
        console.log(data);
        this.data = data.rows;
