...
<template>
  <basic-container>
    <page-table
      tableRef="treadDispatcherLogMainTable"
      :calcHeight="true"
      v-loading="loading"
      :columns="columns"
    ...
    >
      <template slot="header">
        <!-- <el-button
          type="primary"
          v-hasPermi="['tm:dispatcherLog:add']"
          @click="handleAdd"
          >{{ $t("ui.frame.btn.add") }}</el-button
        > -->
        <!-- <el-button
          type="warning"
          v-hasPermi="['tm:dispatcherLog:edit']"
          @click="handleEdit(selection[0])"
          >{{ $t("ui.frame.btn.modify") }}</el-button
        > -->
        <!-- <el-button
          v-hasPermi="['tm:dispatcherLog:import']"
          @click="$refs.tltUpload.handleImport()"
          >{{ $t("ui.frame.btn.import") }}</el-button
        > -->
        <el-button
          @click="handleExport"
          v-hasPermi="['tm:dispatcherLog:export']"
    ...
    <!-- <tlt-upload
      ref="tltUpload"
      downloadUrl="/tm/dispatcherLog/importTemplate"
      uploadUrl="/tm/dispatcherLog/importData"
...
export default {
      ...
      let columns = [
        { type: "selection", fixed: "left" },
        {
          ...
          children: [
            ...
            {
              prop: "materialCode",
              label: this.$t("ui.data.column.tm.dispatcherlog.materialCode"),
              valign: "middle",
              align: "center",
              halign: "center",
              width: 160,
  ...
  methods: {
    ...
    handleExport() {
      downloadLink("/tm/dispatcherLog/export", this.formatParams(false));
    },
