    ...
    columns() {
      return [
        ...
        },
        {
          ...
          label: this.$t("ui.data.column.tireCapacity.monthPlanVersion"),
  ...
  methods: {
    ...
    async save(params) {
      try {
        ...
        const res = await saveTireCapacity(params);
    ...
    async handleVersionChange() {
      try {
        if (!this.form.monthPlanVersion || !this.form.proSize || !this.form.tireType) {
          return;
        }
