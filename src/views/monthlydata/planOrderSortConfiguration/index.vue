  ...
  <header-container class="stockHedgingConfiguration" v-loading="loading">
    ...
    <section class="content">
      ...
      <el-card class="container">
        ...
        <draggable v-if="third.length" v-model="third" class="draggable-list">
          <transition-group type="transition" name="flip-list">
            <div
...
import {
  planOrderSortConfigurationList,
  savePlanOrderConfiguration,
} from "@/api/monthplan/businessSortConfiguration";

export default {
  name: "PlanOrderSortConfiguration",
  ...
  methods: {
    async getConfig() {
      try {
        this.loading = true;
        const res = await planOrderSortConfigurationList();
        this.first = res["1"];
        this.second = res["2"];
        this.third = res["3"];
        this.$nextTick(() => {
    ...
    async save(params) {
      try {
        ...
        const res = await savePlanOrderConfiguration(params);
    ...
    handleSave() {
      const params = {
        firstPlanOrderSortConfigurations: this.first.map((row, index) => {
          return {
            ...row,
            priority: index + 1,
          };
        }),
        secondPlanOrderSortConfigurations: this.second.map((row, index) => {
          return {
            ...row,
            priority: index + 1,
          };
        }),
        thirdPlanOrderSortConfigurations: this.third.map((row, index) => {
          return {
            ...row,
            priority: index + 1,
          };
