...
import { editBigRoll, checkBigRollCodeUnique } from "@/api/cd90/bigRoll";
  ...
  computed: {
    title: function () {
      return (
        ...
        this.$t("ui.cd90.bigRoll.column.modalName")
      );
    },
  },
  methods: {
        ...
        checkBigRollCodeUnique({
          ...
          .then((res) => {
            if (res === 0) {
              ...
              reject(
                new Error(this.$t("ui.cd90.bigRoll.alter.isBigRollExist"))
              );
            }
          })
          .catch((error) => {
