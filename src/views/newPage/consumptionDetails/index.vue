    ...
    >
      <!-- <template slot="header">
        <el-button
          type="primary"
          plain
          v-hasPermi="['monthplan:mpMouldShellInfo:edit']"
          @click="handleAdd"
          >{{ $t("ui.frame.btn.add") }}</el-button
        >
        <el-button
          type="danger"
          v-hasPermi="['monthplan:mpMouldShellInfo:remove']"
          :disabled="selection.length == 0"
          @click="handleDeleteAll"
          >{{ $t("ui.frame.btn.delete") }}</el-button
        >
        <el-button
          v-hasPermi="['monthplan:mpMouldShellInfo:import']"
          @click="$refs.tltUpload.handleImport()"
          >{{ $t("ui.frame.btn.import") }}</el-button
        >
        <el-button
          @click="handleExport"
          v-hasPermi="['monthplan:mpMouldShellInfo:export']"
...
import {listConsumptionDetails} from "@/api/newPage/consumptionDetails";
  ...
  methods: {
    ...
    async getList() {
      try {
        ...
        const data = await listConsumptionDetails(this.formatParams());
        this.data = data.rows;
