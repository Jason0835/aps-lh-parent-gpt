...
<template>
  <basic-container>
    <page-table
      tableRef="steelPressFixedPointMachineMainTable"
      :calcHeight="true"
      v-loading="loading"
      :columns="columns"
    ...
    >
      <template slot="header">
        <el-button
          type="primary"
          plain
          v-hasPermi="['gdyy:specifyMachine:add']"
          @click="handleAdd"
          >{{ $t("ui.frame.btn.add") }}</el-button
        >
        <!-- <el-button
          type="warning"
          v-hasPermi="['gdyy:specifyMachine:edit']"
          @click="handleEdit(selection[0])"
          >{{ $t("ui.frame.btn.modify") }}</el-button
        > -->
        <el-button
          v-hasPermi="['gdyy:specifyMachine:import']"
          @click="$refs.tltUpload.handleImport()"
          >{{ $t("ui.frame.btn.import") }}</el-button
        >
        <el-button
          @click="handleExport"
          v-hasPermi="['gdyy:specifyMachine:export']"
    ...
    <tlt-upload
      ref="tltUpload"
      downloadUrl="/gdyy/specifyMachine/importTemplate"
      uploadUrl="/gdyy/specifyMachine/importData"
...
export default {
  name: "SteelPressFixedPointMachine",
  components: {
    tltUpload,
    infoDialog,
  },
    ...
    columns() {
      let columns = [
        { type: "selection", fixed: "left" },
        {
          ...
          align: "center",
          halign: "center",
          label: this.$t("ui.gdyy.specifyMachine.column.bigRollCode"),
          // sortable: "custom",
        },
        {
          prop: "machineName",
          ...
          render: ({ row }) => {
            return (
              <div>
                ...
                <el-button
                  v-hasPermi={["gdyy:specifyMachine:remove"]}
                  class="minus"
                  type="danger"
  ...
  methods: {
    ...
    handleExport() {
      downloadLink("/gdyy/specifyMachine/export", this.formatParams(false));
    },
