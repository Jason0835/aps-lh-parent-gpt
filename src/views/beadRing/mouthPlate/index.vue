...
<template>
  <basic-container>
    <page-table
      tableRef="beadRingMouthPlateMainTable"
      :calcHeight="true"
      v-loading="loading"
      :columns="columns"
    ...
    >
      <template slot="header">
        <el-button
          type="primary"
          plain
          v-hasPermi="['gsq:mouthPlate:add']"
          @click="handleAdd"
          >{{ $t("ui.frame.btn.add") }}</el-button
        >
        <!-- <el-button
          type="warning"
          v-hasPermi="['gsq:mouthPlate:edit']"
          @click="handleEdit(selection[0])"
          >{{ $t("ui.frame.btn.modify") }}</el-button
        > -->
        <el-button
          v-hasPermi="['gsq:mouthPlate:import']"
          @click="$refs.tltUpload.handleImport()"
          >{{ $t("ui.frame.btn.import") }}</el-button
        >
        <el-button
          @click="handleExport"
          v-hasPermi="['gsq:mouthPlate:export']"
    ...
    <tlt-upload
      ref="tltUpload"
      downloadUrl="/gsq/mouthPlate/importTemplate"
      uploadUrl="/gsq/mouthPlate/importData"
...
export default {
    ...
    columns() {
      let columns = [
        ...
        },
        {
          ...
          // sortable: "custom",
          render: ({ row }) => {
            console.log(row.mouthPlateStatus);
            return (
              <el-switch
                value={row.status}
                active-value="0"
          ...
          render: ({ row }) => {
            return (
              <div>
                ...
                <el-button
                  v-hasPermi={["gsq:mouthPlate:remove"]}
                  class="minus"
                  type="danger"
  ...
  methods: {
    ...
    handleExport() {
      downloadLink("/gsq/mouthPlate/export", this.formatParams(false));
    },
