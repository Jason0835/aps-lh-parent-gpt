...
<template>
  <basic-container>
    <page-table
      tableRef="fiberPressFixedPointMachineMainTable"
      :calcHeight="true"
      v-loading="loading"
      :columns="columns"
    ...
    >
      <template slot="header">
        <el-button
          type="primary"
          plain
          v-hasPermi="['xwyy:specifyMachine:add']"
          @click="handleAdd"
          >{{ $t("ui.frame.btn.add") }}</el-button
        >
        <!-- <el-button
          type="warning"
          v-hasPermi="['xwyy:specifyMachine:edit']"
          @click="handleEdit(selection[0])"
          >{{ $t("ui.frame.btn.modify") }}</el-button
        > -->
        <el-button
          v-hasPermi="['xwyy:specifyMachine:import']"
          @click="$refs.tltUpload.handleImport()"
          >{{ $t("ui.frame.btn.import") }}</el-button
        >
        <el-button
          @click="handleExport"
          v-hasPermi="['xwyy:specifyMachine:export']"
    ...
    <tlt-upload
      ref="tltUpload"
      downloadUrl="/xwyy/specifyMachine/importTemplate"
      uploadUrl="/xwyy/specifyMachine/importData"
...
export default {
  name: "FiberPressFixedPointMachine",
  components: {
    tltUpload,
    infoDialog,
  },
          ...
          render: ({ row }) => {
            return (
              <div>
                ...
                <el-button
                  v-hasPermi={["xwyy:specifyMachine:remove"]}
                  class="minus"
                  type="danger"
  ...
  methods: {
    ...
    handleExport() {
      downloadLink("/xwyy/specifyMachine/export", this.formatParams(false));
    },
