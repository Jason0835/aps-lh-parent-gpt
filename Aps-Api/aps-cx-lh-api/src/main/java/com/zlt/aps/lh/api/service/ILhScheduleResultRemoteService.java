 ...
 * @date 2025-02-13
 */
@FeignClient(contextId = "ILhScheduleResultService", value = ServiceNameConstants.GATEWAY_SERVICE, path = "${api.path.cxlh:cxlh}")
public interface ILhScheduleResultRemoteService {
    ...
    /**
     * 根据规格号查询物料号List
     * @param lhSpecCodeParamDTO
     * @return
     */
    @ApiOperation("根据规格号查询物料号List")
    @PostMapping("/lhScheduleResult/selectListMdmProductConstruction")
    List<MdmProductConstruction> selectListMdmProductConstruction(@RequestBody LhSpecCodeParamDTO lhSpecCodeParamDTO);
     ...
     * 查询列表
     */
    @ApiOperation("月硫化计划信息列表")
    @PostMapping("/lhScheduleResult/monthFinishQtyList")
    TableDataInfo monthFinishQtyList(@RequestBody LhMonthPlanSurplusDetail queryVO);

    @ApiOperation("月硫化计划信息列表")
    @PostMapping("/lhScheduleResult/statisticsDay")
    AjaxResult statisticsDay(LhMonthPlanSurplusDetail queryVO);
}
