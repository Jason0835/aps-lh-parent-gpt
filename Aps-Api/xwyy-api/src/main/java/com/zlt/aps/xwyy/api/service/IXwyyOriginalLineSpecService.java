...
public interface IXwyyOriginalLineSpecService {
     ...
     * 根据条件查询原线规格管理列表
     */
    @PostMapping("/xwyy/originalLineSpec/listOriginalLineSpec")
    TableDataInfo listOriginalLineSpec(@RequestBody XwyyOriginalLineSpec dto);

    /**
     * 根据id查询原线规格管理信息
     */
    @GetMapping("/xwyy/originalLineSpec/getOriginalLineSpec/{id}")
    XwyyOriginalLineSpec getOriginalLineSpec(@PathVariable("id") Long id);

    /**
     * 保存原线规格管理信息（id为空则新增，id不为空则修改）
     */
    @PostMapping("/xwyy/originalLineSpec/saveOriginalLineSpec")
    AjaxResult saveOriginalLineSpec(@RequestBody XwyyOriginalLineSpec dto);

    /**
     * 根据code判断代号是否已经存在
     */
    @PostMapping("/xwyy/originalLineSpec/checkOriginalLineSpecCodeUnique")
     ...
     * @param ids 多个id逗号分割
     */
    @PostMapping("/xwyy/originalLineSpec/deleteOriginalLineSpec/{ids}")
     ...
     * @param dto
     */
    @PostMapping("/xwyy/originalLineSpec/exportData")
    List<XwyyOriginalLineSpec> exportData(@RequestBody XwyyOriginalLineSpec dto);

    @PostMapping("/xwyy/originalLineSpec/importData")
