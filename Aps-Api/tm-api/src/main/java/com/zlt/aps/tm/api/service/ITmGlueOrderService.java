...
public interface ITmGlueOrderService {
     ...
     * 根据条件查询胶料顺序列表
     */
    @PostMapping("/tm/glueOrder/listGlueOrder")
    TableDataInfo listGlueOrder(@RequestBody TmGlueOrderDto dto);

    /**
     * 根据id查询胶料顺序信息
     */
    @GetMapping("/tm/glueOrder/getGlueOrder/{id}")
    TmGlueOrderDto getGlueOrder(@PathVariable("id") Long id);

    /**
     * 保存胶料顺序信息（id为空则新增，id不为空则修改）
     */
    @PostMapping("/tm/glueOrder/saveGlueOrder")
    AjaxResult saveGlueOrder(@RequestBody TmGlueOrderDto dto);

    /**
     * 根据code判断胶料顺序是否已经存在
     */
    @PostMapping("/tm/glueOrder/checkGlueCodeUnique")
     ...
     * @param ids 多个id逗号分割
     */
    @PostMapping("/tm/glueOrder/deleteGlueOrder/{ids}")
     ...
     * @param dto
     */
    @PostMapping("/tm/glueOrder/exportData")
    List<TmGlueOrderDto> exportData(@RequestBody TmGlueOrderDto dto);

    @PostMapping("/tm/glueOrder/importData")
