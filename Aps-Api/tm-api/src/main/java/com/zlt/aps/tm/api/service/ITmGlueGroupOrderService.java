...
public interface ITmGlueGroupOrderService {
     ...
     * 根据条件查询胶料组别顺序列表
     */
    @PostMapping("/tm/glueGroupOrder/listGlueGroupOrder")
    TableDataInfo listGlueGroupOrder(@RequestBody TmGlueGroupOrderDto dto);

    /**
     * 根据id查询胶料组别顺序信息
     */
    @GetMapping("/tm/glueGroupOrder/getGlueGroupOrder/{id}")
    TmGlueGroupOrderDto getGlueGroupOrder(@PathVariable("id") Long id);

    /**
     * 保存胶料组别顺序信息（id为空则新增，id不为空则修改）
     */
    @PostMapping("/tm/glueGroupOrder/saveGlueGroupOrder")
    AjaxResult saveGlueGroupOrder(@RequestBody TmGlueGroupOrderDto dto);

    /**
     * 根据code判断胶料组号是否已经存在
     */
    @PostMapping("/tm/glueGroupOrder/checkGlueGroupCodeUnique")
     ...
     * @param ids 多个id逗号分割
     */
    @PostMapping("/tm/glueGroupOrder/deleteGlueGroupOrder/{ids}")
     ...
     * @param dto
     */
    @PostMapping("/tm/glueGroupOrder/exportData")
    List<TmGlueGroupOrderDto> exportData(@RequestBody TmGlueGroupOrderDto dto);

    @PostMapping("/tm/glueGroupOrder/importData")
