package com.zlt.aps.monthplan.api.domain.entity;

import com.baomidou.mybatisplus.annotation.TableField;
import com.baomidou.mybatisplus.annotation.TableName;
import com.ruoyi.common.core.annotation.Excel;
import com.ruoyi.common.core.web.domain.BaseEntity;
import io.swagger.annotations.ApiModel;
import io.swagger.annotations.ApiModelProperty;
import lombok.Data;

import java.math.BigDecimal;

/**
 * Copyright (c) 2022, All rights reserved。
 * 文件名称：ProductionPlanVulcanizing.java
 * 描    述：分厂月生产计划排产过程-硫化-计划硫化排产对象 t_mp_proc_plan_vulcanizing
 *
 * @author ZLT
 * @version 1.0
 * <p>
 * 修改记录：
 * 修改时间：...
 * 修 改 人：ZLT
 * 修改内容：...
 * @date 2025-02-19
 */

@Data
@TableName(value = "T_MP_PROC_PLAN_VULCANIZING")
@ApiModel(value = "分厂月生产计划排产过程-硫化-计划硫化排产对象", description = "分厂月生产计划排产过程-硫化-计划硫化排产对象 ")
public class ProductionPlanVulcanizing extends BaseEntity {

    private static final long serialVersionUID = 1L;

    /**
     * 生产分厂编号
     */
    @Excel(name = "ui.data.column.ProductionPlanVulcanizing.factoryCode")
    @ApiModelProperty(value = "生产分厂编号", name = "factoryCode")
    @TableField(value = "FACTORY_CODE")
    private String factoryCode;

    /**
     * 年份
     */
    @Excel(name = "ui.data.column.ProductionPlanVulcanizing.year")
    @ApiModelProperty(value = "年份", name = "year")
    @TableField(value = "YEAR")
    private Integer year;

    /**
     * 月份
     */
    @Excel(name = "ui.data.column.ProductionPlanVulcanizing.month")
    @ApiModelProperty(value = "月份", name = "month")
    @TableField(value = "MONTH")
    private Integer month;

    /**
     * 序号
     */
    @Excel(name = "ui.data.column.ProductionPlanVulcanizing.planSeq")
    @ApiModelProperty(value = "序号", name = "planSeq")
    @TableField(value = "PLAN_SEQ")
    private Integer planSeq;

    /**
     * 销售生产需求计划版本
     */
    @Excel(name = "ui.data.column.ProductionPlanVulcanizing.monthPlanVersion")
    @ApiModelProperty(value = "销售生产需求计划版本", name = "monthPlanVersion")
    @TableField(value = "MONTH_PLAN_VERSION")
    private String monthPlanVersion;

    /**
     * 分厂版本
     */
    @Excel(name = "ui.data.column.ProductionPlanVulcanizing.productionVersion")
    @ApiModelProperty(value = "分厂版本", name = "productionVersion")
    @TableField(value = "PRODUCTION_VERSION")
    private String productionVersion;

    /**
     * 月计划ID
     */
    @Excel(name = "ui.data.column.ProductionPlanVulcanizing.monthPlanId")
    @ApiModelProperty(value = "月计划ID", name = "monthPlanId")
    @TableField(value = "MONTH_PLAN_ID")
    private Long monthPlanId;

    /**
     * 生产物料编号
     */
    @Excel(name = "ui.data.column.ProductionPlanVulcanizing.productCode")
    @ApiModelProperty(value = "生产物料编号", name = "productCode")
    @TableField(value = "PRODUCT_CODE")
    private String productCode;

    /**
     * 生产规格描述
     */
    @Excel(name = "ui.data.column.ProductionPlanVulcanizing.productDesc")
    @ApiModelProperty(value = "生产规格描述", name = "productDesc")
    @TableField(value = "PRODUCT_DESC")
    private String productDesc;

    /**
     * 规格
     */
    @Excel(name = "ui.data.column.ProductionPlanVulcanizing.specifications")
    @ApiModelProperty(value = "规格", name = "specifications")
    @TableField(value = "SPECIFICATIONS")
    private String specifications;

    /**
     * 花纹
     */
    @Excel(name = "ui.data.column.ProductionPlanVulcanizing.pattern")
    @ApiModelProperty(value = "花纹", name = "pattern")
    @TableField(value = "PATTERN")
    private String pattern;

    /**
     * 层级
     */
    @Excel(name = "ui.data.column.ProductionPlanVulcanizing.hierarchy")
    @ApiModelProperty(value = "层级", name = "hierarchy")
    @TableField(value = "HIERARCHY")
    private String hierarchy;

    /**
     * 等级码
     */
    @Excel(name = "ui.data.column.ProductionPlanVulcanizing.levelCode")
    @ApiModelProperty(value = "等级码", name = "levelCode")
    @TableField(value = "LEVEL_CODE")
    private String levelCode;

    /**
     * 等级名称
     */
    @Excel(name = "ui.data.column.ProductionPlanVulcanizing.levelName")
    @ApiModelProperty(value = "等级名称", name = "levelName")
    @TableField(value = "LEVEL_NAME")
    private String levelName;

    /**
     * 品名编码
     */
    @Excel(name = "ui.data.column.ProductionPlanVulcanizing.productTypeCode")
    @ApiModelProperty(value = "品名编码", name = "productTypeCode")
    @TableField(value = "PRODUCT_TYPE_CODE")
    private String productTypeCode;

    /**
     * 品名
     */
    @Excel(name = "ui.data.column.ProductionPlanVulcanizing.productTypeName")
    @ApiModelProperty(value = "品名", name = "productTypeName")
    @TableField(value = "PRODUCT_TYPE_NAME")
    private String productTypeName;

    /**
     * 施工号
     */
    @Excel(name = "ui.data.column.ProductionPlanVulcanizing.constructionCode")
    @ApiModelProperty(value = "施工号", name = "constructionCode")
    @TableField(value = "CONSTRUCTION_CODE")
    private String constructionCode;

    /**
     * 胎休布层数            1-1            2-2            3-3
     */
    @Excel(name = "ui.data.column.ProductionPlanVulcanizing.carcassClothType")
    @ApiModelProperty(value = "胎休布层数            1-1            2-2            3-3", name = "carcassClothType")
    @TableField(value = "CARCASS_CLOTH_TYPE")
    private Integer carcassClothType;

    /**
     * 是否EXCEL导入（0：默认不是，1：是）
     */
    @Excel(name = "ui.data.column.ProductionPlanVulcanizing.isImport", readConverterExp = "0=：默认不是，1：是")
    @ApiModelProperty(value = "是否EXCEL导入", name = "isImport")
    @TableField(value = "IS_IMPORT")
    private Integer isImport;

    /**
     * 硫化排产顺序
     */
    @Excel(name = "ui.data.column.ProductionPlanVulcanizing.sequence")
    @ApiModelProperty(value = "硫化排产顺序", name = "sequence")
    @TableField(value = "SEQUENCE")
    private Integer sequence;

    /**
     * 成型机ID
     */
    @Excel(name = "ui.data.column.ProductionPlanVulcanizing.moldingMachineId")
    @ApiModelProperty(value = "成型机ID", name = "moldingMachineId")
    @TableField(value = "MOLDING_MACHINE_ID")
    private Long moldingMachineId;

    /**
     * 成型机编号
     */
    @Excel(name = "ui.data.column.ProductionPlanVulcanizing.moldingMachineCode")
    @ApiModelProperty(value = "成型机编号", name = "moldingMachineCode")
    @TableField(value = "MOLDING_MACHINE_CODE")
    private String moldingMachineCode;

    /**
     * 硫化机ID
     */
    @Excel(name = "ui.data.column.ProductionPlanVulcanizing.vulcanizinnMachineId")
    @ApiModelProperty(value = "硫化机ID", name = "vulcanizinnMachineId")
    @TableField(value = "VULCANIZINN_MACHINE_ID")
    private Long vulcanizinnMachineId;

    /**
     * 硫化机编号
     */
    @Excel(name = "ui.data.column.ProductionPlanVulcanizing.vulcanizingMachineCode")
    @ApiModelProperty(value = "硫化机编号", name = "vulcanizingMachineCode")
    @TableField(value = "VULCANIZING_MACHINE_CODE")
    private String vulcanizingMachineCode;

    /**
     * 硫化机模号:L-左边,R-右边
     */
    @Excel(name = "ui.data.column.ProductionPlanVulcanizing.mouldNo")
    @ApiModelProperty(value = "硫化机模号:L-左边,R-右边", name = "mouldNo")
    @TableField(value = "MOULD_NO")
    private String mouldNo;

    /**
     * 模具号
     */
    @Excel(name = "ui.data.column.ProductionPlanVulcanizing.mouldCode")
    @ApiModelProperty(value = "模具号", name = "mouldCode")
    @TableField(value = "MOULD_CODE")
    private String mouldCode;

    /**
     * 源成型机ID
     */
    @Excel(name = "ui.data.column.ProductionPlanVulcanizing.srcMoldingMachineId")
    @ApiModelProperty(value = "源成型机ID", name = "srcMoldingMachineId")
    @TableField(value = "SRC_MOLDING_MACHINE_ID")
    private Long srcMoldingMachineId;

    /**
     * 源成型机编号
     */
    @Excel(name = "ui.data.column.ProductionPlanVulcanizing.srcMoldingMachineCode")
    @ApiModelProperty(value = "源成型机编号", name = "srcMoldingMachineCode")
    @TableField(value = "SRC_MOLDING_MACHINE_CODE")
    private String srcMoldingMachineCode;

    /**
     * 源硫化机ID
     */
    @Excel(name = "ui.data.column.ProductionPlanVulcanizing.srcVulcanizinnMachineId")
    @ApiModelProperty(value = "源硫化机ID", name = "srcVulcanizinnMachineId")
    @TableField(value = "SRC_VULCANIZINN_MACHINE_ID")
    private Long srcVulcanizinnMachineId;

    /**
     * 源硫化机编号
     */
    @Excel(name = "ui.data.column.ProductionPlanVulcanizing.srcVulcanizingMachineCode")
    @ApiModelProperty(value = "源硫化机编号", name = "srcVulcanizingMachineCode")
    @TableField(value = "SRC_VULCANIZING_MACHINE_CODE")
    private String srcVulcanizingMachineCode;

    /**
     * 源硫化机模号:L-左边,R-右边
     */
    @Excel(name = "ui.data.column.ProductionPlanVulcanizing.srcMouldNo")
    @ApiModelProperty(value = "源硫化机模号:L-左边,R-右边", name = "srcMouldNo")
    @TableField(value = "SRC_MOULD_NO")
    private String srcMouldNo;

    /**
     * 数据类型:0-正常，2-未排成型计划排产(成型机合并排产），3-未排完计划排产（查缺补漏）
     */
    @Excel(name = "ui.data.column.ProductionPlanVulcanizing.dataType", readConverterExp = "查=缺补漏")
    @ApiModelProperty(value = "数据类型:0-正常，2-未排成型计划排产(成型机合并排产），3-未排完计划排产", name = "dataType")
    @TableField(value = "DATA_TYPE")
    private Integer dataType;

    /**
     * 开始日期
     */
    @Excel(name = "ui.data.column.ProductionPlanVulcanizing.beginDay")
    @ApiModelProperty(value = "开始日期", name = "beginDay")
    @TableField(value = "BEGIN_DAY")
    private Integer beginDay;

    /**
     * 开始日期
     */
    @Excel(name = "ui.data.column.ProductionPlanVulcanizing.endDay")
    @ApiModelProperty(value = "开始日期", name = "endDay")
    @TableField(value = "END_DAY")
    private Integer endDay;

    /**
     * DAY_1
     */
    @Excel(name = "ui.data.column.ProductionPlanVulcanizing.day1")
    @ApiModelProperty(value = "DAY_1", name = "day1")
    @TableField(value = "DAY_1")
    private Integer day1;

    /**
     * DAY_2
     */
    @Excel(name = "ui.data.column.ProductionPlanVulcanizing.day2")
    @ApiModelProperty(value = "DAY_2", name = "day2")
    @TableField(value = "DAY_2")
    private Integer day2;

    /**
     * DAY_3
     */
    @Excel(name = "ui.data.column.ProductionPlanVulcanizing.day3")
    @ApiModelProperty(value = "DAY_3", name = "day3")
    @TableField(value = "DAY_3")
    private Integer day3;

    /**
     * DAY_4
     */
    @Excel(name = "ui.data.column.ProductionPlanVulcanizing.day4")
    @ApiModelProperty(value = "DAY_4", name = "day4")
    @TableField(value = "DAY_4")
    private Integer day4;

    /**
     * DAY_5
     */
    @Excel(name = "ui.data.column.ProductionPlanVulcanizing.day5")
    @ApiModelProperty(value = "DAY_5", name = "day5")
    @TableField(value = "DAY_5")
    private Integer day5;

    /**
     * DAY_6
     */
    @Excel(name = "ui.data.column.ProductionPlanVulcanizing.day6")
    @ApiModelProperty(value = "DAY_6", name = "day6")
    @TableField(value = "DAY_6")
    private Integer day6;

    /**
     * DAY_7
     */
    @Excel(name = "ui.data.column.ProductionPlanVulcanizing.day7")
    @ApiModelProperty(value = "DAY_7", name = "day7")
    @TableField(value = "DAY_7")
    private Integer day7;

    /**
     * DAY_8
     */
    @Excel(name = "ui.data.column.ProductionPlanVulcanizing.day8")
    @ApiModelProperty(value = "DAY_8", name = "day8")
    @TableField(value = "DAY_8")
    private Integer day8;

    /**
     * DAY_9
     */
    @Excel(name = "ui.data.column.ProductionPlanVulcanizing.day9")
    @ApiModelProperty(value = "DAY_9", name = "day9")
    @TableField(value = "DAY_9")
    private Integer day9;

    /**
     * DAY_10
     */
    @Excel(name = "ui.data.column.ProductionPlanVulcanizing.day10")
    @ApiModelProperty(value = "DAY_10", name = "day10")
    @TableField(value = "DAY_10")
    private Integer day10;

    /**
     * DAY_11
     */
    @Excel(name = "ui.data.column.ProductionPlanVulcanizing.day11")
    @ApiModelProperty(value = "DAY_11", name = "day11")
    @TableField(value = "DAY_11")
    private Integer day11;

    /**
     * DAY_12
     */
    @Excel(name = "ui.data.column.ProductionPlanVulcanizing.day12")
    @ApiModelProperty(value = "DAY_12", name = "day12")
    @TableField(value = "DAY_12")
    private Integer day12;

    /**
     * DAY_13
     */
    @Excel(name = "ui.data.column.ProductionPlanVulcanizing.day13")
    @ApiModelProperty(value = "DAY_13", name = "day13")
    @TableField(value = "DAY_13")
    private Integer day13;

    /**
     * DAY_14
     */
    @Excel(name = "ui.data.column.ProductionPlanVulcanizing.day14")
    @ApiModelProperty(value = "DAY_14", name = "day14")
    @TableField(value = "DAY_14")
    private Integer day14;

    /**
     * DAY_15
     */
    @Excel(name = "ui.data.column.ProductionPlanVulcanizing.day15")
    @ApiModelProperty(value = "DAY_15", name = "day15")
    @TableField(value = "DAY_15")
    private Integer day15;

    /**
     * DAY_16
     */
    @Excel(name = "ui.data.column.ProductionPlanVulcanizing.day16")
    @ApiModelProperty(value = "DAY_16", name = "day16")
    @TableField(value = "DAY_16")
    private Integer day16;

    /**
     * DAY_17
     */
    @Excel(name = "ui.data.column.ProductionPlanVulcanizing.day17")
    @ApiModelProperty(value = "DAY_17", name = "day17")
    @TableField(value = "DAY_17")
    private Integer day17;

    /**
     * DAY_18
     */
    @Excel(name = "ui.data.column.ProductionPlanVulcanizing.day18")
    @ApiModelProperty(value = "DAY_18", name = "day18")
    @TableField(value = "DAY_18")
    private Integer day18;

    /**
     * DAY_19
     */
    @Excel(name = "ui.data.column.ProductionPlanVulcanizing.day19")
    @ApiModelProperty(value = "DAY_19", name = "day19")
    @TableField(value = "DAY_19")
    private Integer day19;

    /**
     * DAY_20
     */
    @Excel(name = "ui.data.column.ProductionPlanVulcanizing.day20")
    @ApiModelProperty(value = "DAY_20", name = "day20")
    @TableField(value = "DAY_20")
    private Integer day20;

    /**
     * DAY_21
     */
    @Excel(name = "ui.data.column.ProductionPlanVulcanizing.day21")
    @ApiModelProperty(value = "DAY_21", name = "day21")
    @TableField(value = "DAY_21")
    private Integer day21;

    /**
     * DAY_22
     */
    @Excel(name = "ui.data.column.ProductionPlanVulcanizing.day22")
    @ApiModelProperty(value = "DAY_22", name = "day22")
    @TableField(value = "DAY_22")
    private Integer day22;

    /**
     * DAY_23
     */
    @Excel(name = "ui.data.column.ProductionPlanVulcanizing.day23")
    @ApiModelProperty(value = "DAY_23", name = "day23")
    @TableField(value = "DAY_23")
    private Integer day23;

    /**
     * DAY_24
     */
    @Excel(name = "ui.data.column.ProductionPlanVulcanizing.day24")
    @ApiModelProperty(value = "DAY_24", name = "day24")
    @TableField(value = "DAY_24")
    private Integer day24;

    /**
     * DAY_25
     */
    @Excel(name = "ui.data.column.ProductionPlanVulcanizing.day25")
    @ApiModelProperty(value = "DAY_25", name = "day25")
    @TableField(value = "DAY_25")
    private Integer day25;

    /**
     * DAY_26
     */
    @Excel(name = "ui.data.column.ProductionPlanVulcanizing.day26")
    @ApiModelProperty(value = "DAY_26", name = "day26")
    @TableField(value = "DAY_26")
    private Integer day26;

    /**
     * DAY_27
     */
    @Excel(name = "ui.data.column.ProductionPlanVulcanizing.day27")
    @ApiModelProperty(value = "DAY_27", name = "day27")
    @TableField(value = "DAY_27")
    private Integer day27;

    /**
     * DAY_28
     */
    @Excel(name = "ui.data.column.ProductionPlanVulcanizing.day28")
    @ApiModelProperty(value = "DAY_28", name = "day28")
    @TableField(value = "DAY_28")
    private Integer day28;

    /**
     * DAY_29
     */
    @Excel(name = "ui.data.column.ProductionPlanVulcanizing.day29")
    @ApiModelProperty(value = "DAY_29", name = "day29")
    @TableField(value = "DAY_29")
    private Integer day29;

    /**
     * DAY_30
     */
    @Excel(name = "ui.data.column.ProductionPlanVulcanizing.day30")
    @ApiModelProperty(value = "DAY_30", name = "day30")
    @TableField(value = "DAY_30")
    private Integer day30;

    /**
     * DAY_31
     */
    @Excel(name = "ui.data.column.ProductionPlanVulcanizing.day31")
    @ApiModelProperty(value = "DAY_31", name = "day31")
    @TableField(value = "DAY_31")
    private Integer day31;

    /**
     * 生产实际排产是
     */
    @Excel(name = "ui.data.column.ProductionPlanVulcanizing.totalQty")
    @ApiModelProperty(value = "生产实际排产是", name = "totalQty")
    @TableField(value = "TOTAL_QTY")
    private Integer totalQty;

    /**
     * 成型总工时（分钟）
     */
    @Excel(name = "ui.data.column.ProductionPlanVulcanizing.totalMoldingMinutes", readConverterExp = "分=钟")
    @ApiModelProperty(value = "成型总工时", name = "totalMoldingMinutes")
    @TableField(value = "TOTAL_MOLDING_MINUTES")
    private Integer totalMoldingMinutes;

    /**
     * 硫化总工时（分钟）
     */
    @Excel(name = "ui.data.column.ProductionPlanVulcanizing.totalVulcanizationMinutes", readConverterExp = "分=钟")
    @ApiModelProperty(value = "硫化总工时", name = "totalVulcanizationMinutes")
    @TableField(value = "TOTAL_VULCANIZATION_MINUTES")
    private Integer totalVulcanizationMinutes;

    /**
     * 库位类别 1 内销 2 外销 3 OE
     */
    @Excel(name = "ui.data.column.ProductionPlanVulcanizing.locationType")
    @ApiModelProperty(value = "库位类别 1 内销 2 外销 3 OE", name = "locationType")
    @TableField(value = "LOCATION_TYPE")
    private Integer locationType;

    /**
     * 渠道
     */
    @Excel(name = "ui.data.column.ProductionPlanVulcanizing.channel")
    @ApiModelProperty(value = "渠道", name = "channel")
    @TableField(value = "CHANNEL")
    private String channel;

    /**
     * 品牌
     */
    @Excel(name = "ui.data.column.ProductionPlanVulcanizing.brand")
    @ApiModelProperty(value = "品牌", name = "brand")
    @TableField(value = "BRAND")
    private String brand;

    /**
     * 寸口
     */
    @Excel(name = "ui.data.column.ProductionPlanVulcanizing.proSize")
    @ApiModelProperty(value = "寸口", name = "proSize")
    @TableField(value = "PRO_SIZE")
    private BigDecimal proSize;


}