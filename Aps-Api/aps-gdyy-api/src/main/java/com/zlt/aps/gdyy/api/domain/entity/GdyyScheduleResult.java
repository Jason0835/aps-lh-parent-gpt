 ...
 */
@Data
@ApiModel(value = "钢带压延排程结果", description = "钢带压延排程结果")
@TableName("T_GDYY_SCHEDULE_RESULT")
public class GdyyScheduleResult extends BaseEntity implements Serializable {
    ...
    @Excel(name = "ui.data.column.gdyyScheduleResult.factoryCode", dictType = "biz_factory_name")
    private String factoryCode;

    /** 排程日期 */
    @ApiModelProperty("排程日期")
    @TableField("SCHEDULE_DATE")
    @Excel(name = "ui.data.column.gdyyScheduleResult.scheduleDate", dateFormat = "yyyy-MM-dd")
    @JsonFormat(pattern = "yyyy-MM-dd")
    private Date scheduleDate;

    /** 对应的15度裁断批次号 */
    @ApiModelProperty("对应的15度裁断批次号")
    @TableField("CD15_BATCH_NO")
    @Excel(name = "ui.data.column.gdyyScheduleResult.cd15BatchNo")
    private String cd15BatchNo;

    /** 批次号 */
    @ApiModelProperty("批次号")
    @TableField("BATCH_NO")
    @Excel(name = "ui.data.column.gdyyScheduleResult.batchNo")
    private String batchNo;

    /** 工单号 */
    @ApiModelProperty("工单号")
    @TableField("ORDER_NO")
    @Excel(name = "ui.data.column.gdyyScheduleResult.orderNo")
    private String orderNo;

    /** 钢带大卷编号 */
    @ApiModelProperty("钢带大卷编号")
    @TableField("BIG_ROLL_CODE")
    @Excel(name = "ui.data.column.gdyyScheduleResult.bigRollCode")
    private String bigRollCode;

    /** 日用参考（个） */
    @ApiModelProperty("日用参考（个）")
    @TableField("DAY_USED")
    @Excel(name = "ui.data.column.gdyyScheduleResult.dayUsed")
    private Double dayUsed;

    /** 库存（个） */
    @ApiModelProperty("库存（个）")
    @TableField("STOCK_QTY")
    @Excel(name = "ui.data.column.gdyyScheduleResult.stockQty")
    private Double stockQty;

    /** 生产线/机台编码 */
    @ApiModelProperty("机台编码")
    @TableField("MACHINE_CODE")
    @Excel(name = "ui.data.column.gdyyScheduleResult.machineCode")
    ...
    @Excel(name = "ui.data.column.gdyyScheduleResult.isRelease", dictType = "IS_RELEASE")
