...
/**
 * 15度裁断调度员排程操作日志对象 t_cd15_dispatcher_log
 * 
 * @author Gim
 * @date 2022-02-25
 */
@ApiModel(value = "15度裁断调度员排程操作日志对象", description = "15度裁断调度员排程操作日志对象 ")
@Data
...
public class Cd15DispatcherLog extends ApsBaseEntity{
    ...
    /** 主键ID，对应自增序列为：SEQ_T_CD15_DISPATCHER_LOG */
    ...
    @Excel(name = "ui.data.column.cd15.dispatcherlog.operType")
    @ApiModelProperty(value = "操作类型：0--转机台、1--调量。对应数据字典：DISPATCHER_OPER_TYPE")
    private String operType;

    /** 排程日期 */
    @JsonFormat(pattern = "yyyy-MM-dd",timezone = "GMT+8")
    @Excel(name = "ui.data.column.cd15.dispatcherlog.scheduleDate", width = 30, dateFormat = "yyyy-MM-dd")
    @ApiModelProperty(value = "排程日期")
    private Date scheduleDate;

    /** 钢带代码 */
    @Excel(name = "ui.data.column.cd15.dispatcherlog.materialCode")
    @ApiModelProperty(value = "钢带代码")
    private String materialCode;

    /** 操作前机台ID,多个逗号分割 */
    @Excel(name = "ui.data.column.cd15.dispatcherlog.beforeMachineId")
    @ApiModelProperty(value = "操作前机台ID,多个逗号分割")
    private String beforeMachineId;

    /** 操作前中班计划量 */
    @Excel(name = "ui.data.column.cd15.dispatcherlog.beforeDayPlan")
    @ApiModelProperty(value = "操作前中班计划量")
    private Double beforeDayPlan;

    /** 操作前夜班计划量 */
    @Excel(name = "ui.data.column.cd15.dispatcherlog.beforeNightPlan")
    @ApiModelProperty(value = "操作前夜班计划量")
    private Double beforeNightPlan;

    /** 操作后机台ID,多个逗号分割 */
    @Excel(name = "ui.data.column.cd15.dispatcherlog.afterMachineId")
    @ApiModelProperty(value = "操作后机台ID,多个逗号分割")
    private String afterMachineId;

    /** 操作后中班计划量 */
    @Excel(name = "ui.data.column.cd15.dispatcherlog.afterDayPlan")
    @ApiModelProperty(value = "操作后中班计划量")
    private Double afterDayPlan;

    /** 操作后夜班计划量 */
    @Excel(name = "ui.data.column.cd15.dispatcherlog.afterNightPlan")
