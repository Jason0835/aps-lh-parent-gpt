...
/**
 * 胎侧排程结果对象 tc_schedule_result
 * 
 * @author zlt
 * @date 2021-06-21
 */
@ApiModel(value = "胎侧排程结果对象", description = "胎侧排程结果对象 ")
public class TcScheduleResult2 extends ApsBaseEntity {

    private static final long serialVersionUID = 1L;

    /** 主键ID，对应自增序列为：SEQ_TC_SCHEDULE */
    ...
    //@Excel(name = "批次号，每重新生成一次排程结果，批次号就递增。规则：工序+年月日+3位定长自增序号")
    ...
    @Excel(name = "ui.construction.sidewall")
    ...
    /** 库存供应成型时长，单位：小时 */
    //@Excel(name = "库存供应成型时长，单位：小时")
    @ApiModelProperty(value = "库存供应成型时长")
    ...
    /** 夜班(0点-12点)计划量 */
    @ImportValidated(number = true,min = 0,max = 9999999,digits=true)
    ...
    public Double getMonthPlanOs() { return monthPlanOs; }

    public void setMonthPlanOs(Double monthPlanOs) { this.monthPlanOs = monthPlanOs; }
