...
public interface ICd15SpecifyMachineService {
     ...
     * 根据条件查询定点机台列表
     */
    @PostMapping("/cd15/specifyMachine/listSpecifyMachine")
    TableDataInfo listSpecifyMachine(@RequestBody Cd15SpecifyMachineDto dto);

    /**
     * 根据id查询定点机台信息
     */
    @GetMapping("/cd15/specifyMachine/getSpecifyMachine/{id}")
    Cd15SpecifyMachineDto getSpecifyMachine(@PathVariable("id") Long id);

    /**
     * 保存定点机台信息（id为空则新增，id不为空则修改）
     */
    @PostMapping("/cd15/specifyMachine/saveSpecifyMachine")
     ...
     * @param ids 多个id逗号分割
     */
    @PostMapping("/cd15/specifyMachine/deleteSpecifyMachine/{ids}")
    AjaxResult deleteSpecifyMachine(@PathVariable("ids") Long[] ids);

    /**
     * 删除全部定点机台信息(逻辑删)
     */
    @PostMapping("/cd15/specifyMachine/deleteAllSpecifyMachine")
     ...
     * @param dto
     */
    @PostMapping("/cd15/specifyMachine/exportData")
    List<Cd15SpecifyMachineDto> exportData(@RequestBody Cd15SpecifyMachineDto dto);

    @PostMapping("/cd15/specifyMachine/importData")
