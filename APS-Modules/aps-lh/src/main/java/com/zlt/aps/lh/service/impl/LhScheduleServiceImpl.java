...
public class LhScheduleServiceImpl extends AbstractDocService<LhScheduleResult> implements ILhScheduleService {
    ...
    private LhScheduleContext buildContext(LhScheduleRequestDTO request) {
        ...
        /*
         * SYS0304028 表示排程窗口之外允许额外向后拉取的自然日数。
         * 截止日期在上下文初始化时只计算一次，后续基础数据、准入和运行账本统一复用，
         * 避免日驱动循环继续形成 currentDay + N 的漂移口径。
         */
        int earlyProductionDays = context.getScheduleConfig().getEarlyProductionDaysThreshold();
        context.setEarlyProductionMaxDate(
                LhScheduleTimeUtil.addDays(context.getWindowEndDate(), earlyProductionDays));
        log.info("排程上下文构建完成, 工厂: {}, 工厂名称: {}, 目标日(业务): {}, T日: {}, "
                        + "窗口结束日: {}, 提前生产额外天数: {}, 提前生产最晚计划日: {}, "
                        + "排程天数: {}, 局部搜索: {}, 定点机台规则: {}",
                ...
                earlyProductionDays,
                LhScheduleTimeUtil.formatDate(context.getEarlyProductionMaxDate()),
                scheduleDays,
    ...
    public byte[] exportData(List<LhScheduleResult> list, LhScheduleResult result) {
        ...
        // 重命名 Sheet 0（硫化日计划）和 Sheet 1（硫化换模计划）为国际化名称，
...
//            scheduleResultMapper.insert(target);
    ...
    private ExportDataBuildResult buildExportDataList(List<LhScheduleResult> list,
        ...
        for (LhScheduleResult result : sortedList) {
            ...
            if (hasBeforeMaterialReference
                    && insertedReferenceMachineCodes.add(result.getLhMachineCode())) {
                Map<String, Object> referenceRow = this.buildBeforeMaterialReferenceRow(beforeMaterialResult,
                        cxMachineCodeMap, recipeTypeMap, endTypeMap);
                dataList.add(referenceRow);
...
//            row.put("scheduleType", buildScheduleTypeName(result.getScheduleType()));
            ...
            row.put("todayNightFinishQty", zeroToEmpty(result.getTotalFinishQty()));
            ...
            String exportRemark = this.mergeExportRemarks(result.getRemark(), exportRemarkMap.get(result));
            row.put("remark", exportRemark);
            row.put("height", this.calculateExportRemarkRowHeight(exportRemark));
    ...
    private Map<LhScheduleResult, LinkedHashSet<String>> buildExportRemarkMap(
        ...
        Set<String> mouldChangeAfterMaterialKeySet = this.buildMouldChangeAfterMaterialKeySet(
                mouldChangePlanList);
    ...
    private void appendNewSpecExportRemarks(List<LhScheduleResult> sortedList,
                                            ...
                                            Set<String> mouldChangeAfterMaterialKeySet,
        ...
        for (LhScheduleResult result : sortedList) {
            ...
            String mouldChangeAfterMaterialKey = this.buildMouldChangeAfterMaterialKey(result.getBatchNo(),
                    result.getScheduleDate(), result.getLhMachineCode(), materialCode);
            if (!recentlyProducedMaterialCodeSet.contains(materialCode)
                    || mouldChangeAfterMaterialKeySet.contains(mouldChangeAfterMaterialKey)) {
    ...
    /**
     * 构建当前批次模具交替计划中的后规格命中键。
     * <p>同一批次、同一排程日、同一机台的交替计划后规格，视为该机台新增上机物料，
     * 即使物料在其他机台近期生产过，也必须标记“新开规格”。</p>
     *
     * @param mouldChangePlanList 当前导出批次的模具交替计划
     * @return 批次+排程日+机台+后物料命中键集合
     */
    private Set<String> buildMouldChangeAfterMaterialKeySet(List<LhMouldChangePlan> mouldChangePlanList) {
        if (CollUtil.isEmpty(mouldChangePlanList)) {
            return Collections.emptySet();
        }
        return mouldChangePlanList.stream()
                .filter(plan -> StringUtils.isNotBlank(plan.getLhResultBatchNo()))
                .filter(plan -> Objects.nonNull(plan.getScheduleDate()))
                .filter(plan -> StringUtils.isNotBlank(plan.getLhMachineCode()))
                .filter(plan -> StringUtils.isNotBlank(plan.getAfterMaterialCode()))
                .filter(plan -> !StringUtils.equals(StringUtils.trimToEmpty(plan.getBeforeMaterialCode()),
                        StringUtils.trimToEmpty(plan.getAfterMaterialCode())))
                .map(plan -> this.buildMouldChangeAfterMaterialKey(plan.getLhResultBatchNo(),
                        plan.getScheduleDate(), plan.getLhMachineCode(), plan.getAfterMaterialCode()))
                .collect(Collectors.toSet());
    }

    /**
     * 构建模具交替计划后规格与排程结果的匹配键。
     */
    private String buildMouldChangeAfterMaterialKey(String batchNo, Date scheduleDate,
                                                     String machineCode, String materialCode) {
        return StringUtils.defaultString(batchNo).trim() + "|"
                + (Objects.nonNull(scheduleDate)
                    ? DateUtil.formatDate(DateUtil.beginOfDay(scheduleDate)) : "") + "|"
    ...
    /**
     * 按备注中的换行数计算导出行高，避免多条备注换行后内容被遮挡。
     *
     * @param remark 导出备注
     * @return Excel 行高，单位为点
     */
    private float calculateExportRemarkRowHeight(String remark) {
        String normalizedRemark = StringUtils.replace(StringUtils.defaultString(remark), "\r\n", "\n");
        int lineCount = normalizedRemark.isEmpty() ? 1 : normalizedRemark.split("\n", -1).length;
        return Math.max(17F, lineCount * 17F);
    }
     ...
     * @param recipeTypeMap        示方类型字典
     * @param endTypeMap           收尾类型字典
     * @return 前规格参考行
     */
    private Map<String, Object> buildBeforeMaterialReferenceRow(
            LhScheduleResult beforeMaterialResult,
            Map<Long, String> cxMachineCodeMap,
        ...
        referenceRow.put("embryoCode", beforeMaterialResult.getEmbryoCode());
        ...
        referenceRow.put("isEnd", this.buildUnifiedShiftType(beforeMaterialResult, endTypeMap));
        referenceRow.put("trialStatus", this.buildUnifiedLhType(beforeMaterialResult, recipeTypeMap));
        referenceRow.put("monthPlanSumTotal", beforeMaterialResult.getMonthPlanSumTotal());

        for (int shift = 1; shift <= LhScheduleConstant.MAX_SHIFT_SLOT_COUNT; shift++) {
            referenceRow.put("class" + shift + "LeftRightMould",
                    this.buildShiftLeftRightMould(beforeMaterialResult, shift));
            Object shiftType = this.buildShiftType(beforeMaterialResult, shift, endTypeMap);
            referenceRow.put("class" + shift + "IsEnd", shiftType);
            referenceRow.put("class" + shift + "Type", shiftType);
            referenceRow.put("class" + shift + "LhType",
                    this.buildShiftProductStatus(beforeMaterialResult, shift, recipeTypeMap));
            referenceRow.put("class" + shift + "MouldMethod",
                    this.buildShiftMouldMethod(beforeMaterialResult, shift, recipeTypeMap));
            referenceRow.put("class" + shift + "Analysis", this.getClassAnalysis(beforeMaterialResult, shift));
            referenceRow.put("class" + shift + "Dot", "");
        }
        return referenceRow;
    }
    ...
    /**
     * 批量查询早班换模或换活字块机台近7天最近一次收尾的前规格物料。
     * <p>先从当前导出明细中收集命中机台，再通过一条SQL按机台分组取最近收尾记录，
     * 避免逐台查询。未命中机台、导出日期为空或工厂为空时不访问数据库。</p>
     *
     * @param list         当前导出排程结果
     * @param scheduleDate 当前导出排程日期
     * @return key=硫化机台编号，value=前规格历史排程记录
     */
    private Map<String, LhScheduleResult> buildBeforeMaterialResultMap(
            List<LhScheduleResult> list, Date scheduleDate) {
        ...
        String factoryCode = triggerResults.stream()
                .map(LhScheduleResult::getFactoryCode)
                .filter(StringUtils::isNotBlank)
                .findFirst()
                .orElse(null);
        ...
        return beforeMaterialList.stream()
     ...
     *
     * @param result 排程结果
     * @return true-早班有计划量且排程类型为换模或换活字块
     */
    private boolean isBeforeMaterialTriggerResult(LhScheduleResult result) {
        if (Objects.isNull(result) || Objects.isNull(result.getClass1PlanQty())
                || result.getClass1PlanQty() <= 0) {
            return false;
        }
     ...
     * <p>规则 3（已移至 exportData 方法）：主字段 isEnd=1 的余量收尾行，
     * dailyPlanQty（合计余量）列涂淡橙；isEmbryoEnding=1 的胎胚收尾行改为胎胚库存列涂淡橙。</p>
     *
     * @param row       导出行数据
