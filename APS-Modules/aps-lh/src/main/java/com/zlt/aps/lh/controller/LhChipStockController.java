...
@Api(tags = "芯片库存")
@RestController
@RequestMapping("/lhChipStock")
public class LhChipStockController extends AbstractDocBizController<LhChipStock> {

    @Autowired
    private ILhChipStockService lhChipStockService;
    ...
    @PostMapping("/list")
    @Override
    public TableDataInfo list(@RequestBody LhChipStock queryVO) {
        TableDataInfo tableDataInfo = super.list(queryVO);
        List<LhChipStock> list = (List<LhChipStock>) tableDataInfo.getRows();
     ...
     * 保存
     */
    @Log(title = "ui.data.column.lhChipStock.modelName", businessType = BusinessType.INSERT_OR_UPDATE)
    @ApiOperation("保存")
    @PostMapping("/save")
    @Override
    public AjaxResult save(@RequestBody LhChipStock billVO){
    ...
    @ResponseBody
    public AjaxResult mergeSave(@RequestBody LhChipStock billVO) {
        if (PubUtil.isEmpty(billVO.getFactoryCode())) {
            billVO.setFactoryCode(FactoryConstant.DEFAULT_FACTORY_CODE);
        }
     ...
     * 删除
     */
    @Log(title = "ui.data.column.lhChipStock.modelName", businessType = BusinessType.DELETE)
    @ApiOperation("删除")
    @PostMapping("/remove")
    @ResponseBody
    @Override
    ...
    @Override
    public LhChipStock getInfo(@PathVariable("billId") Long billId) {
        LhChipStock entity = super.getInfo(billId);
        // 计算剩余可用量
     ...
     * @return 结果
     */
    @Log(title = "ui.data.column.lhChipStock.modelName", businessType = BusinessType.IMPORT)
    @ApiOperation("导入数据")
    ...
    @Override
    public byte[] exportData(@RequestBody LhChipStock queryVO, @PathVariable("fileName") String fileName,
    ...
    protected List<LhChipStock> listExportData(LhChipStock obj) {
        ...
        List<LhChipStock> list = lhChipStockMapper.selectList(wrapper);
        // 计算剩余可用量
        for (LhChipStock item : list) {
    ...
    protected IDocService getDocService(){
        return lhChipStockService;
    }

    @Override
    protected String[] getQueryFormulas() {
        return lhChipStockService.getQueryFormulas();
    }
     ...
     */
    @Override
    protected void builderCondition(QueryWrapper<LhChipStock> queryWrapper, LhChipStock queryVO) {
