...
public class LhMouldCleanPlanServiceImpl extends AbstractDocService<LhMouldCleanPlan> implements ILhMouldCleanPlanService {
     ...
     * 与syncFromMouldCleanWarn的区别：
     * - syncFromMouldCleanWarn只取MAX(DATA_VERSION)的预警数据，用于增量同步
     * - 本方法取全部预警数据（不限制版本号），用于临时任务清空后重新全量生成
     * 逻辑：取全部未删除的预警数据，按机台分组后生成计划，已删除的预警对应生成删除标记的计划
     *
     * @return 同步数量
     */
    @Override
    public int syncAllFromMouldCleanWarn() {
        String lockKey = "sync:mould:clean:plan:all";
        if (redisService.getCacheObject(lockKey) != null) {
            throw new RuntimeException(I18nUtil.getMessage("ui.message.sync.in.progress"));
        }
        ...
        try {
            redisService.setCacheObject(lockKey, "1");
            log.info("开始基于全部预警数据全量生成清洗计划");

            // 取全部预警数据（不限制版本号）
            QueryWrapper<LhMouldCleanWarn> warnWrapper = new QueryWrapper<>();
            List<LhMouldCleanWarn> warnList = lhMouldCleanWarnMapper.selectList(warnWrapper);
            if (warnList == null || warnList.isEmpty()) {
                log.info("模具清洗预警数据为空，无数据可生成计划");
                return 0;
            }
            log.info("获取全部模具清洗预警数据{}条", warnList.size());
            ...
            log.info("基于全部预警数据全量生成清洗计划完成，更新{}条，新增{}条，删除标记{}条", updateCount, insertCount, deleteCount);
            return updateCount + insertCount + deleteCount;
        } finally {
            redisService.deleteObject(lockKey);
        }
     ...
     */
    @Override
    public void cleanAllWarnAndPlan() {
        log.info("开始清空模具清洗预警和清洗计划表全部数据");

        // 物理删除清洗计划表全部数据
        int planCount = lhMouldCleanPlanMapper.delete(new QueryWrapper<>());
        log.info("清空模具清洗计划表，删除{}条", planCount);

        // 物理删除清洗预警表全部数据
        int warnCount = lhMouldCleanWarnMapper.delete(new QueryWrapper<>());
        log.info("清空模具清洗预警表，删除{}条", warnCount);

        log.info("清空模具清洗预警和清洗计划表全部数据完成，计划{}条，预警{}条", planCount, warnCount);
    }
}
