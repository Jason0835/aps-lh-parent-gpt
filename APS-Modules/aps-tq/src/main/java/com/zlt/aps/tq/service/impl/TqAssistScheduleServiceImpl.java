...
import com.zlt.aps.tq.mapper.TqAssistScheduleMapper;
...
/**
 * 胎圈外协排程结果Service业务层处理
 *
 * @author chen
 * @date 2022-02-16
 */
@Service
public class TqAssistScheduleServiceImpl implements TqAssistScheduleService {

    @Value("${excelModelPath}")
    private String excelModelPath;
    @Autowired
    private TqAssistScheduleMapper tqAssistScheduleMapper;
    ...
    public List<TqAssistSchedule> selectTqAssistScheduleList(TqAssistSchedule tqAssistSchedule) {
        return tqAssistScheduleMapper.selectTqAssistScheduleList(tqAssistSchedule);
    }
    ...
    public byte[] export(List<TqAssistSchedule> list) {
        ...
        if (Locale.SIMPLIFIED_CHINESE.equals(lang) || lang == null) {
            // 中文
            inputStream = this.getClass().getClassLoader().getResourceAsStream(excelModelPath + "tqAssistSchedule.xlsx");
        } else if (Locale.US.equals(lang)) {
            // 英文
            inputStream = this.getClass().getClassLoader().getResourceAsStream(excelModelPath + "tqAssistSchedule_en.xlsx");
        }
        ...
        if (CollectionUtils.isNotEmpty(list)) {
            ...
            webBook.setSheetName(0, I18nUtil.getMessage("ui.data.column.tq.assistSchedule.modelName"));
            ...
            BigDecimal nextMidPlan = BigDecimal.ZERO;
            for (int i = 0; i < list.size(); i++) {
                int cellNum = 0;
                TqAssistSchedule scheduleResult = list.get(i);
...
//                row.createCell(cellNum++).setCellValue(DateFormatUtils.format(scheduleResult.getScheduleDate(), "yyyy-MM-dd"));
                ...
                midPlan = midPlan.add(BigDecimal.valueOf(scheduleResult.getMidPlanQty() == null ? 0 : scheduleResult.getMidPlanQty()));
                ...
                nextMidPlan = nextMidPlan.add(BigDecimal.valueOf(scheduleResult.getNextMidPlanQty() == null ? 0 : scheduleResult.getNextMidPlanQty()));
