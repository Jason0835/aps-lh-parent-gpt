 ...
 * 胎圈人工排程操作统一入口。
 *
 * <p>对齐胎面 TmManualOperationFacade，统一处理请求规范化、数据库真实状态读取、
 * 多机台分布式锁、短事务、行锁和调度日志，避免插单、调量、转机台和删除从不同入口
 * 绕过同一组安全约束。</p>
 *
 * <p>与胎面差异：</p>
 * <ul>
 *   <li>胎圈 TqDispatcherLog 表无 affectedBeforeJson/affectedAfterJson/undoStatus 字段，
 *       因此不实现撤销机制（undoLastOperation），仅用现有 beforeClassNPlan/afterClassNPlan
 *       字段记录6班次计划量快照。</li>
 *   <li>胎圈复用 TqManualInsertRollingService 的 insertAndRoll/changeQtyAndRoll/
 *       changeMachineAndRoll/deleteAndRollBatch 等任务链路径方法作为底层执行。</li>
 *   <li>胎圈 RELEASE_STATUS 取值：0-未发布 1-待发布 2-发布中 3-已发布 4-已下推 5-已撤销；
 *       仅 0/1/5 允许人工编辑（对齐 TqRollingUpdateServiceImpl.isEditableReleaseStatus）。</li>
 * </ul>
 *
 * @author APS
 */
...
public class TqManualOperationFacade {

    /** 人工操作分布式锁前缀（对齐胎面 LOCK_PREFIX） */
    private static final String LOCK_PREFIX = "TQ_SCHEDULE:OPER_LOCK:";
    ...
    private final TqDispatcherLogMapper tqDispatcherLogMapper;

    private final TqManualInsertRollingService tqManualInsertRollingService;
     ...
     */
    public int insertTask(TqScheduleResult scheduleResult) {
        ...
        return this.executeWithMachineLocks(scheduleResult.getFactoryCode(), scheduleResult.getScheduleDate(), machineCodes,
                () -> this.executeInTransaction(() -> {
                    List<TqScheduleResult> beforeList = this.lockAndValidateManualOpSnapshot(scheduleResult, machineCodes);
                    int changedCount = tqManualInsertRollingService.insertAndRoll(scheduleResult);
                    List<TqScheduleResult> afterList = this.loadManualOpSnapshot(scheduleResult, machineCodes);
     ...
     */
    public int changeQty(TqScheduleResult scheduleResult) {
        TqScheduleResult persisted = this.loadOperationResult(scheduleResult,
                "ui.tq.schedule.changeQty.idEmpty", "调量排程结果不能为空",
                "ui.tq.schedule.changeQty.resultNotFound", "调量排程结果不存在或已失效");
        ...
        return this.executeWithMachineLocks(persisted.getFactoryCode(), persisted.getScheduleDate(), machineCodes,
                () -> this.executeInTransaction(() -> {
                    TqScheduleResult current = this.reloadAndValidateOperationResult(scheduleResult.getId(),
                            "ui.tq.schedule.changeQty.resultNotFound", "调量排程结果不存在或已失效");
                    this.validateLockedSourceMachine(persisted, current);
                    this.normalizeExistingOperationRequest(scheduleResult, current, false);
                    List<TqScheduleResult> beforeList = this.lockAndValidateManualOpSnapshot(current, machineCodes);
                    int changedCount = tqManualInsertRollingService.changeQtyAndRoll(scheduleResult);
                    List<TqScheduleResult> afterList = this.loadManualOpSnapshot(current, machineCodes);
     ...
     */
    public int changeMachine(TqScheduleResult scheduleResult) {
        TqScheduleResult persisted = this.loadOperationResult(scheduleResult,
                "ui.tq.schedule.changeMachine.idEmpty", "转机台排程结果不能为空",
                "ui.tq.schedule.changeMachine.resultNotFound", "转机台排程结果不存在或已失效");
     ...
     */
    public int batchChangeMachine(String machineCode, List<TqScheduleResult> scheduleResultList) {
        String targetMachineCode = this.normalizeTargetMachineCode(machineCode);
        List<TqScheduleResult> requestList = this.normalizeBatchChangeMachineRequests(scheduleResultList);
        List<TqScheduleResult> initialResultList = requestList.stream()
     ...
     */
    public int batchChangeQty(List<TqScheduleResult> scheduleResultList) {
        List<TqScheduleResult> requestList = this.normalizeBatchRequests(scheduleResultList,
                "ui.tq.schedule.changeQty.batchEmpty", "批量调量请求不能为空");
        List<TqScheduleResult> initialResultList = requestList.stream()
                .map(request -> this.loadOperationResult(request,
                        "ui.tq.schedule.changeQty.idEmpty", "调量排程结果不能为空",
                        "ui.tq.schedule.changeQty.resultNotFound", "调量排程结果不存在或已失效"))
        ...
        return this.executeWithLockKeys(lockKeyList,
                () -> this.executeInTransaction(() -> {
                    List<TqScheduleResult> currentList = new ArrayList<>();
                    for (int index = 0; index < requestList.size(); index++) {
                        TqScheduleResult current = this.reloadAndValidateOperationResult(requestList.get(index).getId(),
                                "ui.tq.schedule.changeQty.resultNotFound", "调量排程结果不存在或已失效");
                    ...
                    List<String> machineCodes = currentList.stream().map(TqScheduleResult::getMachineCode)
                            .distinct().collect(Collectors.toList());
                    List<TqScheduleResult> beforeList = this.lockAndValidateManualOpSnapshot(currentList.get(0), machineCodes);
                    int changedCount = tqManualInsertRollingService.changeQtyAndRollBatch(requestList);
                    List<TqScheduleResult> afterList = this.loadManualOpSnapshot(currentList.get(0), machineCodes);
                    for (TqScheduleResult request : requestList) {
    ...
    public int deleteTasks(List<Long> ids) {
        ...
        List<TqScheduleResult> initialResultList = tqScheduleResultMapper.selectBatchIds(normalizedIds);
    ...
    int executeBatchChangeMachineTransaction(String targetMachineCode,
                                              List<TqScheduleResult> requestList,
                                              List<TqScheduleResult> initialResultList) {
        return this.executeInTransaction(() -> {
            ...
            List<TqScheduleResult> beforeList = this.lockAndValidateManualOpSnapshot(currentList.get(0), machineCodes);
            int changedCount = tqManualInsertRollingService.changeMachineAndRollBatch(requestList);
            List<TqScheduleResult> afterList = this.loadManualOpSnapshot(currentList.get(0), machineCodes);
            for (TqScheduleResult request : requestList) {
    ...
    private int changeMachineInsideTransaction(String targetMachineCode,
                                                TqScheduleResult scheduleResult,
                                                TqScheduleResult persisted) {
        TqScheduleResult current = this.reloadAndValidateOperationResult(scheduleResult.getId(),
                "ui.tq.schedule.changeMachine.resultNotFound", "转机台排程结果不存在或已失效");
        this.validateLockedSourceMachine(persisted, current);
        ...
        List<TqScheduleResult> beforeList = this.lockAndValidateManualOpSnapshot(current, machineCodes);
        int changedCount = tqManualInsertRollingService.changeMachineAndRoll(scheduleResult);
        List<TqScheduleResult> afterList = this.loadManualOpSnapshot(current, machineCodes);
    ...
    private int deleteTasksInsideTransaction(List<Long> ids, List<String> expectedLockKeyList) {
        List<TqScheduleResult> lockedTargetList = tqScheduleResultMapper.selectBatchIdsForUpdate(ids);
        this.validateDeleteResults(ids, lockedTargetList);
        if (!expectedLockKeyList.equals(this.buildDeleteLockKeys(lockedTargetList))) {
            throw new ServiceException(this.resolveTqMessage(
                    "ui.tq.schedule.operation.concurrentChanged", "排程状态已变化，请刷新后重试"));
        }

        List<TqScheduleResult> sortedTargetList = lockedTargetList.stream()
                .sorted(Comparator.comparing(TqScheduleResult::getId)).collect(Collectors.toList());
        List<String> machineCodes = sortedTargetList.stream().map(TqScheduleResult::getMachineCode)
                .distinct().collect(Collectors.toList());
        List<TqScheduleResult> beforeList = this.lockAndValidateManualOpSnapshot(sortedTargetList.get(0), machineCodes);
        // 对齐胎面：用 beforeList 构造 currentTargetList，保证行锁内最新值
        List<TqScheduleResult> currentTargetList = new ArrayList<>();
        for (TqScheduleResult target : sortedTargetList) {
            TqScheduleResult before = beforeList.stream()
        ...
        int deletedCount = tqManualInsertRollingService.deleteAndRollBatch(currentTargetList);
        List<TqScheduleResult> afterList = this.loadManualOpSnapshot(sortedTargetList.get(0), machineCodes);
        for (TqScheduleResult currentTarget : currentTargetList) {
     ...
     */
    private void normalizeInsertRequest(TqScheduleResult scheduleResult) {
        if (scheduleResult == null) {
            throw new ServiceException(this.resolveTqMessage(
                    "ui.tq.schedule.insertTask.empty", "插单排程结果不能为空"));
        }
        ...
        if (scheduleResult.getScheduleDate() == null) {
            throw new ServiceException(this.resolveTqMessage(
                    "ui.tq.schedule.insertTask.scheduleDateEmpty", "插单排程日期不能为空"));
        }
        ...
        scheduleResult.setBeadCode(StrUtil.trim(scheduleResult.getBeadCode()));
        if (StrUtil.isBlank(scheduleResult.getMachineCode())) {
            throw new ServiceException(this.resolveTqMessage(
                    "ui.tq.schedule.machineCode.empty", "排程机台不能为空"));
        }
        if (StrUtil.isBlank(scheduleResult.getBeadCode())) {
            throw new ServiceException(this.resolveTqMessage(
                    "ui.tq.schedule.insertTask.beadCodeEmpty", "插单胎圈不能为空"));
        }
     ...
     */
    private void validateInsertShiftFields(TqScheduleResult scheduleResult) {
        boolean hasPlanQty = false;
        for (int shiftOrder = 1; shiftOrder <= TqScheduleConstants.TQ_MAX_SHIFT_ORDER; shiftOrder++) {
            String planQtyField = String.format(TqScheduleConstants.SHIFT_PLAN_QTY_FIELD_TEMPLATE, shiftOrder);
            String sequenceField = String.format(TqScheduleConstants.SHIFT_SEQUENCE_FIELD_TEMPLATE, shiftOrder);
            String analysisField = String.format(TqScheduleConstants.SHIFT_ANALYSIS_FIELD_TEMPLATE, shiftOrder);
            ...
            if (planQty == null) {
                if (sequence != null || analysis != null) {
                    throw new ServiceException(this.resolveTqMessage(
                            "ui.tq.schedule.insertTask.shiftPairRequired", "插单班次计划量和顺序必须成对填写"));
                }
                continue;
            }
        ...
        if (!hasPlanQty) {
            throw new ServiceException(this.resolveTqMessage(
                    "ui.tq.schedule.insertTask.planQtyRequired", "至少填写一个班次的计划量和顺序"));
        }
    }
     ...
     */
    private TqScheduleResult loadOperationResult(TqScheduleResult scheduleResult, String emptyMessageKey, String emptyMessage,
                                                   String missingKey, String missingMessage) {
        if (scheduleResult == null || scheduleResult.getId() == null) {
            throw new ServiceException(this.resolveTqMessage(emptyMessageKey, emptyMessage));
        }
        TqScheduleResult persisted = tqScheduleResultMapper.selectById(scheduleResult.getId());
        if (persisted == null) {
            throw new ServiceException(this.resolveTqMessage(missingKey, missingMessage));
        }
        return persisted;
    }
     ...
     */
    private TqScheduleResult reloadAndValidateOperationResult(Long resultId, String missingKey, String missingMessage) {
        TqScheduleResult current = tqScheduleResultMapper.selectById(resultId);
        if (current == null) {
            throw new ServiceException(this.resolveTqMessage(missingKey, missingMessage));
        }
        if (tqScheduleResultMapper.isReleasingOrTimeoutByIds(new Long[]{resultId}) > 0) {
            throw new ServiceException(I18nUtil.getMessage(
                    "ui.data.column.scheduleResult.release.isReleasingOrTimeoutById"));
        }
        if (!this.isEditableReleaseStatus(current.getReleaseStatus())) {
            throw new ServiceException(I18nUtil.getMessage("ui.tq.schedule.illegalReleaseTransition"));
        }
        return current;
    }
     ...
     */
    private void normalizeExistingOperationRequest(TqScheduleResult request, TqScheduleResult persisted,
        ...
        request.setBeadCode(persisted.getBeadCode());
        request.setSteelRingCode(persisted.getSteelRingCode());
        request.setTriangleGlueCode(persisted.getTriangleGlueCode());
        ...
        request.setCxClass1Plan(persisted.getCxClass1Plan());
        request.setCxClass2Plan(persisted.getCxClass2Plan());
        request.setCxClass3Plan(persisted.getCxClass3Plan());
        request.setCxClass4Plan(persisted.getCxClass4Plan());
        request.setCxClass5Plan(persisted.getCxClass5Plan());
        request.setCxClass6Plan(persisted.getCxClass6Plan());
        request.setCxClass7Plan(persisted.getCxClass7Plan());
        request.setCxClass8Plan(persisted.getCxClass8Plan());
     ...
     */
    void validateChangeQtyEditableFields(TqScheduleResult request, TqScheduleResult persisted) {
        ...
        this.validateProtectedField("beadCode", request.getBeadCode(), persisted.getBeadCode());
        this.validateProtectedField("steelRingCode", request.getSteelRingCode(), persisted.getSteelRingCode());
        this.validateProtectedField("triangleGlueCode", request.getTriangleGlueCode(), persisted.getTriangleGlueCode());
        ...
        for (int shiftOrder = 1; shiftOrder <= TqScheduleConstants.TQ_MAX_SHIFT_ORDER; shiftOrder++) {
            ...
            this.validateProtectedShiftField(request, persisted,
                    String.format(TqScheduleConstants.SHIFT_START_TIME_FIELD_TEMPLATE, shiftOrder));
            this.validateProtectedShiftField(request, persisted,
                    String.format(TqScheduleConstants.SHIFT_END_TIME_FIELD_TEMPLATE, shiftOrder));
            this.validateProtectedShiftField(request, persisted,
                    String.format(TqScheduleConstants.SHIFT_FINISH_QTY_FIELD_TEMPLATE, shiftOrder));
            this.validateProtectedShiftField(request, persisted,
                    String.format(TqScheduleConstants.SHIFT_TASK_STATUS_FIELD_TEMPLATE, shiftOrder));
        }
    }
     ...
     * @param persisted 数据库当前排程结果
     */
    private void normalizeProtectedShiftFields(TqScheduleResult request, TqScheduleResult persisted) {
        List<String> protectedFieldTemplates = Arrays.asList(
                TqScheduleConstants.SHIFT_SEQUENCE_FIELD_TEMPLATE,
                TqScheduleConstants.SHIFT_START_TIME_FIELD_TEMPLATE,
                TqScheduleConstants.SHIFT_END_TIME_FIELD_TEMPLATE,
                TqScheduleConstants.SHIFT_FINISH_QTY_FIELD_TEMPLATE,
                TqScheduleConstants.SHIFT_TASK_STATUS_FIELD_TEMPLATE);
        for (int shiftOrder = 1; shiftOrder <= TqScheduleConstants.TQ_MAX_SHIFT_ORDER; shiftOrder++) {
     ...
     */
    private void validateProtectedShiftField(TqScheduleResult request, TqScheduleResult persisted, String fieldName) {
    ...
    private void validateProtectedField(String fieldName, Object requestValue, Object persistedValue) {
        ...
        throw new ServiceException(MessageFormat.format(
                I18nUtil.getMessage("ui.tq.schedule.changeQty.fieldNotAllowed"), fieldName));
    }
    ...
    List<String> buildMachineLockKeys(String factoryCode, Date scheduleDate, List<String> machineCodes) {
        ...
        if (StrUtil.isBlank(normalizedFactoryCode) || scheduleDate == null) {
            throw new ServiceException(this.resolveTqMessage("ui.tq.schedule.operation.failed",
                    "人工排程锁范围不完整"));
        }
     ...
     */
    List<TqScheduleResult> normalizeBatchRequests(List<TqScheduleResult> scheduleResultList,
                                                    String emptyKey, String emptyMessage) {
        if (scheduleResultList == null || scheduleResultList.isEmpty()) {
            throw new ServiceException(this.resolveTqMessage(emptyKey, emptyMessage));
        }
        Set<Long> resultIdSet = new LinkedHashSet<>();
        List<TqScheduleResult> requestList = new ArrayList<>();
        for (TqScheduleResult scheduleResult : scheduleResultList) {
            if (scheduleResult == null || scheduleResult.getId() == null) {
                throw new ServiceException(this.resolveTqMessage(
                        "ui.tq.schedule.operation.idEmpty", "排程结果ID不能为空"));
            }
            if (!resultIdSet.add(scheduleResult.getId())) {
                throw new ServiceException(this.resolveTqMessage(
                        "ui.tq.schedule.batch.duplicate", "批量操作包含重复排程结果"));
            }
     ...
     * @throws ServiceException 请求为空或存在重复 ID 时抛出
     */
    List<TqScheduleResult> normalizeBatchChangeMachineRequests(List<TqScheduleResult> scheduleResultList) {
        return this.normalizeBatchRequests(scheduleResultList,
                "ui.tq.schedule.changeMachine.batchEmpty", "批量转机台记录不能为空");
    }
     ...
     */
    List<String> buildBatchLockKeys(List<TqScheduleResult> initialResultList) {
        if (initialResultList == null) {
            return Collections.emptyList();
        }
        return initialResultList.stream()
    ...
    List<Long> normalizeDeleteIds(List<Long> ids) {
        ...
        if (normalizedIds.isEmpty()) {
            throw new ServiceException(this.resolveTqMessage(
                    "ui.tq.schedule.delete.idsEmpty", "删除排程结果不能为空"));
        }
     ...
     */
    void validateDeleteResults(List<Long> ids, List<TqScheduleResult> resultList) {
        if (resultList == null || resultList.size() != ids.size()) {
            throw new ServiceException(this.resolveTqMessage(
                    "ui.tq.schedule.result.notFound", "排程结果不存在或已删除"));
        }
        ...
        if (containsInvalidStatus) {
            throw new ServiceException(this.resolveTqMessage(
                    "ui.tq.schedule.delete.statusInvalid", "只有未发布/待发布/已撤销排程结果允许删除"));
        }
    }
     ...
     */
    List<String> buildDeleteLockKeys(List<TqScheduleResult> resultList) {
        ...
        if (containsInvalidScope) {
            throw new ServiceException(this.resolveTqMessage(
                    "ui.tq.schedule.operation.failed", "人工排程操作失败"));
        }
     ...
     */
    void validateLockedSourceMachine(TqScheduleResult initialResult, TqScheduleResult currentResult) {
    ...
    private <T> T executeWithLockKeys(List<String> lockKeyList, Supplier<T> action) {
        ...
        try {
            ...
            if (!locked) {
                throw new ServiceException(I18nUtil.getMessage("ui.tq.schedule.operating"));
            }
            return action.get();
     ...
     * @return 当前有效排程结果快照
     */
    private List<TqScheduleResult> loadManualOpSnapshot(TqScheduleResult reference, List<String> machineCodes) {
        ...
        wrapper.eq(StrUtil.isNotBlank(reference.getBatchNo()), TqScheduleResult::getBatchNo, reference.getBatchNo());
        wrapper.in(TqScheduleResult::getMachineCode, machineCodes.stream().filter(StrUtil::isNotBlank)
     ...
     */
    private List<TqScheduleResult> lockAndValidateManualOpSnapshot(TqScheduleResult reference,
                                                                     List<String> machineCodes) {
        List<TqScheduleResult> candidateList = this.loadManualOpSnapshot(reference, machineCodes);
        List<Long> candidateIds = candidateList.stream().map(TqScheduleResult::getId).filter(Objects::nonNull)
                .distinct().sorted().collect(Collectors.toList());
        if (!candidateIds.isEmpty()) {
            List<TqScheduleResult> lockedList = tqScheduleResultMapper.selectBatchIdsForUpdate(candidateIds);
            if (lockedList == null || lockedList.size() != candidateIds.size()) {
                throw new ServiceException(this.resolveTqMessage(
                        "ui.tq.schedule.operation.concurrentChanged", "排程状态已变化，请刷新后重试"));
            }
        }
        List<TqScheduleResult> lockedSnapshot = this.loadManualOpSnapshot(reference, machineCodes);
        Set<Long> lockedIds = lockedSnapshot.stream().map(TqScheduleResult::getId).filter(Objects::nonNull)
     ...
     * 记录人工操作及其前后6班次计划量快照。
     *
     * <p>胎圈 TqDispatcherLog 表无 affectedBeforeJson/affectedAfterJson 字段，
     ...
     */
    private void recordDispatcherLog(String operType, TqScheduleResult scheduleResult,
        ...
        dispatcherLog.setBeadCode(scheduleResult.getBeadCode());
        TqScheduleResult beforePrimary = beforeList.stream()
     ...
     */
    private void copyBeforePlanQty(TqDispatcherLog dispatcherLog, TqScheduleResult scheduleResult) {
     ...
     * @param scheduleResult 排程结果
     */
    private void copyAfterPlanQty(TqDispatcherLog dispatcherLog, TqScheduleResult scheduleResult) {
     ...
     * 判断释放状态是否可编辑。
     *
     * <p>对齐 TqRollingUpdateServiceImpl.isEditableReleaseStatus，
     * 仅未发布(0)、待发布(1)、已撤销(5)允许人工操作。</p>
     *
    ...
    /**
     * 读取胎圈排程国际化提示，未命中时回退默认文案。
     *
     * @param messageKey     国际化 key
     * @param defaultMessage 默认提示
     * @return 当前语言环境下的提示文案
     */
    private String resolveTqMessage(String messageKey, String defaultMessage) {
