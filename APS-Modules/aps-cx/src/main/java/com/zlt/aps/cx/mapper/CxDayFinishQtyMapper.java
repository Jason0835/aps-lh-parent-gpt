...
public interface CxDayFinishQtyMapper extends CommBaseMapper<CxDayFinishQty> {
     ...
     * @return 更新的记录数
     */
    @Update("UPDATE T_CX_DAY_FINISH_QTY SET IS_DELETE = 1, UPDATE_BY = #{updateBy}, UPDATE_TIME = #{updateTime} WHERE FACTORY_CODE = #{factoryCode} AND IS_DELETE = 0")
     ...
     * @return 更新的记录数
     */
    @Update("UPDATE T_CX_DAY_FINISH_QTY SET IS_DELETE = 1, UPDATE_BY = #{updateBy}, UPDATE_TIME = #{updateTime} WHERE FACTORY_CODE = #{factoryCode} AND DATE(FINISH_DATE) = #{finishDate} AND IS_DELETE = 0")
     ...
     * @return 更新的记录数
     */
    @Update("UPDATE T_CX_DAY_FINISH_QTY SET IS_DELETE = 1, UPDATE_BY = 'CLEAN_TASK', UPDATE_TIME = NOW() WHERE DATE(FINISH_DATE) < CURDATE() AND IS_DELETE = 0")
