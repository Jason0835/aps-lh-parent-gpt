...
public class ScheduleDataValidationResult implements Serializable {
    ...
    public String generateSummary() {
        ...
        if (errorCount > 0) {
            sb.append(String.format(I18nUtil.getMessage("ui.data.column.cxScheduleResult.validation.summary.errorCount"), errorCount));
        }
        if (warnCount > 0) {
            sb.append(String.format(I18nUtil.getMessage("ui.data.column.cxScheduleResult.validation.summary.warnCount"), warnCount));
        }
        if (infoCount > 0) {
            sb.append(String.format(I18nUtil.getMessage("ui.data.column.cxScheduleResult.validation.summary.infoCount"), infoCount));
        }
