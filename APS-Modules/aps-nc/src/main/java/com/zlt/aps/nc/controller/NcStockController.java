...
public class NcStockController extends BaseController {
    ...
    public TableDataInfo list(@RequestBody NcStock stock) {
        startPage();
        stock.setOrderStr(orderStr());
        List<NcStock> list = stockService.selectStockList(stock);
    ...
    @ApiOperation("新增内衬库存信息（id不为空）")
    @PostMapping
    public AjaxResult add(@RequestBody NcStock stock) {
        //唯一性校验（使用库存日期+物料编号为逻辑主键）
        List<NcStock> list = stockService.checkStockListUnic(stock);
    ...
    @ApiOperation("修改内衬库存信息（id不为空）")
    @PutMapping
    public AjaxResult edit(@RequestBody NcStock stock) {
        //唯一性校验（使用库存日期+物料编号为逻辑主键）
        List<NcStock> list = stockService.checkStockListUnic(stock);
        if (CollectionUtils.isNotEmpty(list)) {
            return AjaxResult.error(I18nUtil.getMessage("ui.stock.message.unique"));
    ...
    public List<NcStock> exportList(@RequestBody NcStock stock) {
        startPage();
        stock.setOrderStr(orderStr());
        List<NcStock> list = stockService.selectStockList(stock);
        return list;
