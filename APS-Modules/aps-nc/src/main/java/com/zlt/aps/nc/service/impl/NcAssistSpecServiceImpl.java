...
import com.zlt.aps.nc.mapper.NcAssistSpecMapper;
 ...
 */
@Service
public class NcAssistSpecServiceImpl extends ServiceImpl<NcAssistSpecMapper, NcAssistSpec> implements NcAssistSpecService {

    @Resource
    private NcAssistSpecMapper ncAssistSpecMapper;
     ...
     * @param entity
     */
    public void saveAssistSpec(NcAssistSpec entity) {
    ...
    public void deleteAssistSpec(Long[] ids) {
        LambdaUpdateWrapper<NcAssistSpec> wrapper = new LambdaUpdateWrapper<>();
     ...
     */
    @Override
    public AjaxResult importData(List<NcAssistSpec> list, boolean updateSupport, Long importLogId) {
        int successNum = 0;
        int failureNum = 0;
        // 校验
        List<ImportErrorLog> importErrorLogs = new ArrayList<>();
        List<NcAssistSpec> importList = new ArrayList<>();

        //按业务主键分组
        Map<String, Long> groupMap = list.stream().collect(Collectors.groupingBy(NcAssistSpec::getMaterialCode, Collectors.counting()));

        for (int i = 0; i < list.size(); i++) {
            NcAssistSpec bigRoll = list.get(i);
            ...
            if (hasValue > 1) {
                ...
                String columnName = I18nUtil.getMessage("ui.common.column.assist.nc.materialCode");
        ...
        if (CollectionUtils.isNotEmpty(list)) {
            try {
                ...
                if (updateSupport && CollectionUtils.isNotEmpty(importList)) {
                    ...
                    //查询数据库已存在对象
                    for (int i = 0; i < list.size(); i++) {
                        NcAssistSpec excelItem = list.get(i);
                        ...
                        if (UserConstants.UNIQUE.equals(unique)) {
                            //不存在插入
                            successNum++;
                            NcAssistSpec ncAssistSpec = new NcAssistSpec();
                            BeanUtils.copyProperties(excelItem, ncAssistSpec);
                            ncAssistSpecMapper.insert(ncAssistSpec);
                        } else {
                            // 存在，插入错误详细日志
                            failureNum++;
                            addImportErrorLog(importLogId, i + 2,
                                    I18nUtil.getMessage("ui.nc.assistSpec.alter.isAssistSpecExist"), importErrorLogs);
                        }
                    }
                }
            } catch (Exception e) {
