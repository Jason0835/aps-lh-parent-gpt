...
public class ScheduleDateGroup {
    ...
    /** 各原始班次位置对应的排产日期 */
    private Map<Integer, Date> positionDates = new HashMap<>();

    /** 输入位置→输出NcScheduleResult class位置的映射，key=输入position(1~lastClass), value=输出class位置(1~6) */
    private Map<Integer, Integer> positionClassMap = new HashMap<>();
