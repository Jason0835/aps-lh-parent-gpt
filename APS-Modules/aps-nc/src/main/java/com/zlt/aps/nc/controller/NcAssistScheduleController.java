...
public class NcAssistScheduleController extends BaseController {
    ...
    @Autowired
    private NcAssistScheduleService ncAssistScheduleService;
    ...
    @PostMapping("/list")
    public TableDataInfo list(@RequestBody NcAssistSchedule ncAssistSchedule) {
        ncAssistSchedule.setOrderStr(orderStr());
        List<NcAssistSchedule> list = ncAssistScheduleService.selectNcAssistScheduleList(ncAssistSchedule);
    ...
    @PostMapping("/export")
    public byte[] export(@RequestBody NcAssistSchedule assistSchedule) throws Exception {

        //查询数据
//        startPage("a.GLUE_SEQ,a.GLUE_CODE asc");
        assistSchedule.setOrderStr(orderStr());  //拿到前端传的排序字段+排序方式
        List<NcAssistSchedule> list = ncAssistScheduleService.selectNcAssistScheduleList(assistSchedule);
        ...
        if (CollectionUtils.isNotEmpty(list)) {
            ...
            for (int i = 0; i < list.size(); i++) {
                int n = 0;
                NcAssistSchedule scheduleResult = list.get(i);
