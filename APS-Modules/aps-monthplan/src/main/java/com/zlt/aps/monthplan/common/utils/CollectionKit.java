...
public class CollectionKit {
	...
	public static <T> List<T> sub(int pageNum, int pagesize, List<T> list){
		if(isEmpty(list)) {
			return  newArrayList();
		}
        // 总记录条数
	 ...
	 * @return treeSet
	 */
	public static <T> SortedSet<T> toTreeSet(Collection<T> collection, Comparator<T> comparator){
		final  SortedSet<T> treeSet = new TreeSet<>(comparator);
		for (T t : collection) {
			treeSet.add(t);
		}
		return treeSet;
