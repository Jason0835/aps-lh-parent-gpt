...
public class Cd90StorageLaneLimitServiceImpl extends AbstractDocService<Cd90StorageLaneLimit> implements ICd90StorageLaneLimitService {
    ...
    public AjaxResult importData(List<Cd90StorageLaneLimit> list, boolean updateSupport, Long importLogId) {
        ...
        for (int i = 0; i < list.size(); i++) { int en = i + 2; Cd90StorageLaneLimit de = list.get(i); List<ImportErrorLog> v = ImportExcelValidatedUtils.validated(importLogId, en, de); ImportExcelValidatedUtils.validatedRepeat(list, de, i, 2, importLogId, v); if (CollectionUtils.isNotEmpty(v)) { fn++; de.setId(-999L); el.addAll(v); } }
