...
public class Cd90MachineRollMappingController extends AbstractDocBizController<Cd90MachineRollMapping> {
    ...
    public TableDataInfo list(@RequestBody Cd90MachineRollMapping queryVO) { return super.list(queryVO); }

    @Log(title = "ui.data.column.machineRollMapping.modelName", businessType = BusinessType.INSERT)
    @ApiOperation("新增") @PostMapping("/add")
    public AjaxResult add(@RequestBody Cd90MachineRollMapping entity) { return super.save(entity); }

    @Log(title = "ui.data.column.machineRollMapping.modelName", businessType = BusinessType.UPDATE)
    @ApiOperation("编辑") @PostMapping("/edit")
    public AjaxResult edit(@RequestBody Cd90MachineRollMapping entity) { return super.save(entity); }
    ...
    public Cd90MachineRollMapping getInfo(@PathVariable("id") Long id) { return super.getInfo(id); }

    @ApiOperation("校验唯一性") @PostMapping("/checkUnique")
    public String checkUnique(@RequestBody Cd90MachineRollMapping entity) { return cd90MachineRollMappingService.checkUnique(entity); }
    ...
    @Override protected List<Cd90MachineRollMapping> listExportData(Cd90MachineRollMapping obj) {
        QueryWrapper<Cd90MachineRollMapping> wrapper = new QueryWrapper<>(); this.builderCondition(wrapper, obj);
        List<Cd90MachineRollMapping> list = cd90MachineRollMappingMapper.selectList(wrapper); AppUtils.formatData(list, getQueryFormulas()); return list;
    }
    @Override protected IDocService getDocService() { return cd90MachineRollMappingService; }
    @Override protected void builderCondition(QueryWrapper<Cd90MachineRollMapping> qw, Cd90MachineRollMapping vo) {
