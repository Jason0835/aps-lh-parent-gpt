...
import com.zlt.aps.mp.factory.service.*;
...
public abstract class AbstractBaseWeekAdjustService implements IMpWeekAdjustService {
    ...
    @Autowired
    private MonthPlanValidateService monthPlanValidateService;
...
//        checkFieldMap.put("dayVulcanizationQty", "日硫化量单模");
    ...
    public void confirmAdjust(MpRollAdjustContextDTO contextDTO) throws BusinessException {
        ...
        List<MpMonthPlanStatistics> monthPlanStatisticsList = this.handleMonthPlanStatistics(contextDTO, null);
        // 9.1、保存月计划统计结果
        if (!CollectionUtils.isEmpty(monthPlanStatisticsList)) {
        ...
        // 13、校验胎胚分配
        checkEmbryoAllocation(contextDTO);
            ...
            for (Map.Entry<String, List<FactoryMonthPlanFinalAdjustVo>> entry1 : mpCombineProdFinalMap.entrySet()) {
                for (int iDay = FactoryConstant.MONTH_START_DAY; iDay <= maxDays; iDay++) {
                    ...
                    for (String shell : mouldShellArr) {
                        shellRemainQty = mouldShellCombineRemainQtyMap.getOrDefault(shell + iDay, 0);
     ...
     * @param contextDTO
     */
    private void checkEmbryoAllocation(MpRollAdjustContextDTO contextDTO) {
        List<FactoryMonthPlanFinalAdjustVo> allMonthPlanList = contextDTO.getFactoryMonthPlanProdFinalList();
        List<FactoryMonthPlanFinalAdjustVo> checkMonthPlanList;
        String structureName = contextDTO.getStructureName();
        if (!StringUtil.isEmptyWithTrim(structureName)) {
            checkMonthPlanList = allMonthPlanList.stream().filter(x -> x.getStructureName().equals(structureName)).collect(Collectors.toList());
        } else {
    ...
    public void recalculate(MpRollAdjustContextDTO contextDTO, Boolean isHandleMonthPlanStatistics) throws BusinessException {
        ...
        if (isHandleMonthPlanStatistics) {
            // 设置周程滚动参数
            String tempFlag = ProductTypeEnum.WHOLE_STEEL.getValue();
            contextDTO.setParamMap(mpAdjustStructureInService.getMpWeekAdjustParam(contextDTO.getFactoryCode(), tempFlag));
            List<MpMonthPlanStatistics> monthPlanStatisticsList = this.handleMonthPlanStatistics(contextDTO, YesOrNoEnum.YES.getCode());
            // 保存月计划统计结果
            if (!CollectionUtils.isEmpty(monthPlanStatisticsList)) {
                contextDTO.setMonthPlanStatisticsList(monthPlanStatisticsList);
     ...
     */
    @Override
    public List<MpMonthPlanStatistics> handleMonthPlanStatistics(MpRollAdjustContextDTO contextDTO, String tempFlag) {
        // 获取月度生产计划
        ...
        return monthPlanStatisticsList;
    }
