...
/**
 * 在机结构，续作Sku续作模具数处理控制
 * 如果在机结构成型机台数 > 估算需要使用的成型机台数，则优先释放配比高的机台，其次是成型机编号大的
 * 进行月初释放机台：优先释放配比大的机台
 ...
 * 按 胎胚计划量/模具数 比值大的优先释放，比值一样则模具数少的优先，逐个胎胚释放，直到达到胎胚个数限制标准
 ...
 * 4.2.2、如果模具总数也超，则按 胎胚计划量/模具数 比值最大的胎胚(比值一样则模具数多的优先)，移除一个胎胚。
...
public class CxContinueSkuAllocationMouldHandler {
    ...
    private static void releaseContinueSkuMouldNumber(CxContinueInfoHelper groupContinueInfo, Integer releaseCount) {
        ...
        //胎胚没有超，模具数(即硫化机台)多-需要续作Sku减模具数即可
        if (continueEmbryoCount <= maxEmbryoCodeCount && continueMouldNumber > maxMouldNumber) {
        ...
        reduceEmbryoCountAndMouldNumber(groupContinueInfo, hasPlanList, leftOverMap, needReduceEmbryoCount, maxMouldNumber);
...
//        }
    ...
    private static void reduceEmbryoCount(CxContinueInfoHelper groupContinueInfo, List<CxContinueEmbryoInfoHelper> continueEmbryoInfoList, Map<String, CxContinueSkuInfoHelper> continueSkuMouldNumberMap, Integer reduceEmbryoCount) {
        //计划量/模具数 比值大的优先，其次是模具数少的优先
        continueEmbryoInfoList.sort(Comparator.comparing(CxContinueEmbryoInfoHelper::getDemandMouldRatioInfo).thenComparing(CxContinueEmbryoInfoHelper::getMouldNumber, Comparator.reverseOrder()));
        //提取需要移除的续作胎胚信息
    ...
    private static void reduceEmbryoCountAndMouldNumber(CxContinueInfoHelper groupContinueInfo, List<CxContinueEmbryoInfoHelper> continueEmbryoInfoList, Map<String, CxContinueSkuInfoHelper> leftOverMap, Integer needReduceEmbryoCount, Integer maxMouldNumber) {
        //胎胚种类数超，模具数也超，则优先 计划量/模具数 比值大的优先，其次是模具数多的优先
        continueEmbryoInfoList.sort(Comparator.comparing(CxContinueEmbryoInfoHelper::getDemandMouldRatioInfo, Comparator.reverseOrder()).thenComparing(CxContinueEmbryoInfoHelper::getMouldNumber, Comparator.reverseOrder()));
        //需要从续作胎胚中移除的
