...
public class CalculateStructureCxMachineNumber {
    ...
    private static Map<String, ProductionPlanGroupInfo> buildGroupPlanInfoMap(TbrProductionContext productionContext, List<MonthPlanProductionRequirePlanVo> requirePlanList) {
        ...
        mapGroupByStructureName.forEach((structureName, groupDatas) -> {
              ProductionPlanGroupInfo groupInfo = ProductionPlanGroupInfo.createInitByGroupList(structureName, productionContext.getProductType(), groupDatas);
              //是否零度结构
              ...
              groupInfo.setCxMachineLhRationMap(getCxMachineLhRationMap(structureName,productionContext));
              // 计算结构净需求量
