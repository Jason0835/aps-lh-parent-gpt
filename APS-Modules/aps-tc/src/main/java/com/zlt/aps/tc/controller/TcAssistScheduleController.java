...
public class TcAssistScheduleController extends BaseController {
    ...
    @Autowired
    private TcAssistScheduleService tcAssistScheduleService;
    ...
    @PostMapping("/list")
    public TableDataInfo list(@RequestBody TcAssistSchedule tcAssistSchedule) {
        tcAssistSchedule.setOrderStr(orderStr());
        List<TcAssistSchedule> list = tcAssistScheduleService.selectTcAssistScheduleList(tcAssistSchedule);
     ...
     * 导出列表
     */
    @Log(title = "ui.data.column.tc.assistSchedule.modelName", businessType = BusinessType.EXPORT)
    @PostMapping("/export")
    public byte[] export(@RequestBody TcAssistSchedule tcScheduleResult) throws Exception {

        //查询数据
        tcScheduleResult.setOrderStr(orderStr());  //拿到前端传的排序字段+排序方式
        List<TcAssistSchedule> list = tcAssistScheduleService.selectTcAssistScheduleList(tcScheduleResult);
        ...
        if (Locale.SIMPLIFIED_CHINESE.equals(lang) || lang == null) {
            // 中文
            in = this.getClass().getClassLoader().getResourceAsStream(excelModelPath + "tcAssistSchedule.xlsx");
        } else if (Locale.US.equals(lang)) {
            // 英文
            in = this.getClass().getClassLoader().getResourceAsStream(excelModelPath + "tcAssistSchedule_en.xlsx");
        }
        Workbook webBook = ExcelUtils.readExcel(in);
        webBook.setSheetName(0, I18nUtil.getMessage("ui.data.column.tc.assistSchedule.modelName"));
        //填充数据
        if (CollectionUtils.isNotEmpty(list)) {
            ...
            for (int i = 0; i < list.size(); i++) {
                int n = 0;
                TcAssistSchedule scheduleResult = list.get(i);
