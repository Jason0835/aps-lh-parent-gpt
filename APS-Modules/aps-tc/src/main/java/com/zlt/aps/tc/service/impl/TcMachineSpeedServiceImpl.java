...
public class TcMachineSpeedServiceImpl extends AbstractDocService<TcMachineSpeed> implements ITcMachineSpeedService {
    ...
    protected SysDocType getSysDocType() {
        ...
        sysDocType.setDocTypeCode("TC0905");
        return sysDocType;
    }

    @Override
    public String checkUnique(TcMachineSpeed query) {
        String unique = super.checkUnique(query);
        if (UserConstants.NOT_UNIQUE.equals(unique)) {
            throw new ServiceException(I18nUtil.getMessage("ui.data.alert.tc.machineSpeed.notUnique"));
        }
        return unique;
    }
    ...
    protected List<String> getCheckUniqueFields() {
        return new ArrayList<>(Arrays.asList("factoryCode", "machineCode", "sidewallCode"));
    }

    @Override
    protected Map<Object, Object> getServiceCheckParams(List<TcMachineSpeed> list, List<TcMachineSpeed> importList) {
        Map<Object, Object> serviceCheckParams = super.getServiceCheckParams(list, importList);
        // 提取所有非空、去重的机台编码
        List<String> machineCodeList = list.stream()
                .map(TcMachineSpeed::getMachineCode)
    ...
    @Override
    protected Boolean serviceCheckAndDataHandle(TcMachineSpeed importDocEntity, List<ImportErrorLog> importErrorLogs,
