...
public class TcDjSharedMachineServiceImpl extends AbstractDocService<TcDjSharedMachine> implements ITcDjSharedMachineService {
    ...
    protected SysDocType getSysDocType() {
        ...
        sysDocType.setDocTypeCode("TC0915");
        return sysDocType;
    }

    @Override
    public String checkUnique(TcDjSharedMachine query) {
        String unique = super.checkUnique(query);
        if (UserConstants.NOT_UNIQUE.equals(unique)) {
            throw new ServiceException(I18nUtil.getMessage("ui.data.alert.tc.djSharedMachine.notUnique"));
        }
        return unique;
    }
    ...
    @Override
    protected Map<Object, Object> getServiceCheckParams(List<TcDjSharedMachine> list, List<TcDjSharedMachine> importList) {
        Map<Object, Object> serviceCheckParams = super.getServiceCheckParams(list, importList);
        // 提取所有非空、去重的机台编码
        List<String> machineCodeList = list.stream()
                .map(TcDjSharedMachine::getMachineCode)
    ...
    @Override
    protected Boolean serviceCheckAndDataHandle(TcDjSharedMachine importDocEntity, List<ImportErrorLog> importErrorLogs,
