...
public class TcStockServiceImpl extends AbstractDocService<TcStock> implements ITcStockService {
     ...
     */
    @Override
    public void logicDeleteAndSaveBatch(Date stockDate, String updateBy, List<TcStock> list) {
        if (stockDate == null) {
            throw new IllegalArgumentException("库存日期不能为空");
        }
        // 步骤1：逻辑删除当天库存日期的旧数据
        Date updateTime = new Date();
        int deleteCount = tcStockMapper.logicDeleteByStockDate(stockDate, updateBy, updateTime);
        log.info("胎侧库存同步：逻辑删除库存日期={}的旧数据，删除数量={}", stockDate, deleteCount);
        ...
        if (CollectionUtils.isNotEmpty(list)) {
            ...
            for (int i = 0; i < list.size(); i += batchSize) {
                ...
                List<TcStock> batch = list.subList(i, end);
                for (TcStock stock : batch) {
                    baseDao.save(stock);
                }
            }
