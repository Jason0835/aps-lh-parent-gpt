...
/**
 * 胎侧自动排程 Redis 基础资料缓存服务。
 *
 * <p>该服务只缓存跨请求复用的自动排程基础资料，运行态排程上下文仍由单次请求内存对象承载。</p>
 */
@Slf4j
@Service
public class TcAutoScheduleRedisCacheService {
     ...
     * 创建缓存服务。
     */
    public TcAutoScheduleRedisCacheService() {
    }
     ...
     */
    public TcAutoScheduleRedisCacheService(RedisService redisService) {
     ...
     *
     * @param factoryCode  工厂编码，为空时清理全部胎侧自动排程基础资料缓存
    ...
    private Collection<String> findKeys(String pattern) {
        try {
            ...
            log.warn("[TC_AUTO_SCHEDULE_CACHE] 查询 Redis 缓存 key 失败，pattern={}，原因={}", pattern, ex.getMessage());
