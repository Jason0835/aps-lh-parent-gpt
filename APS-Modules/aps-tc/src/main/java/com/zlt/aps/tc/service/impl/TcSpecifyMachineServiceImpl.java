...
public class TcSpecifyMachineServiceImpl extends AbstractDocService<TcSpecifyMachine> implements ITcSpecifyMachineService {
    ...
    protected SysDocType getSysDocType() {
        ...
        sysDocType.setDocTypeCode("TC0907");
        return sysDocType;
    }

    @Override
    public String checkUnique(TcSpecifyMachine query) {
        String unique = super.checkUnique(query);
        if (UserConstants.NOT_UNIQUE.equals(unique)) {
            throw new ServiceException(I18nUtil.getMessage("ui.data.alert.tc.specifyMachine.notUnique"));
        }
        return unique;
    }
    ...
    @Override
    protected Map<Object, Object> getServiceCheckParams(List<TcSpecifyMachine> list, List<TcSpecifyMachine> importList) {
        Map<Object, Object> serviceCheckParams = super.getServiceCheckParams(list, importList);
        // 提取所有非空、去重的机台编码
        List<String> machineCodeList = list.stream()
                .map(TcSpecifyMachine::getMachineCode)
    ...
    @Override
    protected Boolean serviceCheckAndDataHandle(TcSpecifyMachine importDocEntity, List<ImportErrorLog> importErrorLogs,
