...
@Api(tags = "胎侧库存")
@RestController
@RequestMapping("/tcStock")
public class TcStockController extends AbstractDocBizController<TcStock> {
    ...
    @PostMapping("/list")
    @Override
    public TableDataInfo list(@RequestBody TcStock queryVO) {
        return super.list(queryVO);
    }

    @Log(title = "ui.data.column.tc.Stock.modelName", businessType = BusinessType.INSERT_OR_UPDATE)
    @ApiOperation("保存")
    @PostMapping("/save")
    @Override
    public AjaxResult save(@RequestBody TcStock billVO) {
    ...
    @Log(title = "ui.data.column.tc.Stock.modelName", businessType = BusinessType.DELETE)
    @ApiOperation("删除")
    ...
    @GetMapping(value = "/{id}")
    @Override
    public TcStock getInfo(@PathVariable("id") Long id) {
    ...
    public String checkUnique(@RequestBody TcStock query) {
        return tcStockService.checkUnique(query);
    }

    @Log(title = "ui.data.column.tc.Stock.modelName", businessType = BusinessType.IMPORT)
    @ApiOperation("导入数据")
    ...
    @Log(title = "ui.data.column.tc.Stock.modelName", businessType = BusinessType.EXPORT)
    @ApiOperation("导出数据")
    @PostMapping("/exportData/{fileName}")
    @Override
    public byte[] exportData(@RequestBody TcStock queryVO, @PathVariable("fileName") String fileName,
                             HttpServletResponse response) throws IOException {
        return super.exportData(queryVO, fileName, response);
    }

    @Override
    protected List<TcStock> listExportData(TcStock obj) {
        QueryWrapper<TcStock> wrapper = new QueryWrapper<>();
        this.builderCondition(wrapper, obj);
        return tcStockMapper.selectList(wrapper);
    }

    @Override
    protected IDocService getDocService() {
        return tcStockService;
    }

    @Override
    protected void builderCondition(QueryWrapper<TcStock> queryWrapper, TcStock queryVO) {
