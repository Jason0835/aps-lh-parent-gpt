...
import com.tlt.aps.enums.ReasonAnalysisEnums;
...
import com.zlt.aps.monthplan.api.remoteService.IProductStockMonthRemoteApiService;
...
public class LhScheduleResultServiceImpl extends AbstractDocService<LhScheduleResult> implements LhScheduleResultService {
    ...
    public void insertOrder(LhOrderInsertDTO dto) {
        ...
        boolean bSummerSeason = CxLhCommonUtils.isSummerSeason(paramsMap.get(LhParamCodeEnums.START_CURING_SUMMER_DAY.getCode()), paramsMap.get(LhParamCodeEnums.START_CURING_WINTER_DAY.getCode()));
        ...
        CxStock cxStock = cxStockService.queryStockByEmbryo(dto.getScheduleDate(), lhScheduleResult.getEmbryoCode());
        if (cxStock != null) {
            lhScheduleResult.setEmbryoStock(cxStock.getStockNum());
        }
        //设置数据来源 插单
    ...
    private void calcShiftCapacity(Map<String, String> paramsMap, LhScheduleResult lhScheduleResult) {
        ...
        int shiftCapacity = CxLhCommonUtils.calcPeriodCapacity(durationSec, lhTime, moldNum);
    ...
    public void changeMachine(LhTransferDeskDTO dto) {
        ...
        //插入转机台日志
        autoScheduleLogService.insertLhScheduleLog(ApsConstant.APS_STRING_11, lhScheduleResult.getBatchNo(), lhScheduleResult.getOrderNo(), "转机台日志",
                logSplit("原始机台：" + sourceMachineCode, ",转入机台：" + dto.getLhMachineCode(), "操作人员：" + SecurityUtils.getUsername(), "操作时间：" + DateUtils.getTime())); //添加日志
        //插入排程操作日志
        LhDispatcherLog lhDispatcherLog = new LhDispatcherLog();
        lhDispatcherLog.setFactoryCode(dto.getFactoryCode());
    ...
    private List<LhScheduleResult> filterValidMonthPlans(List<FactoryMonthPlanProdFinalVo> plans, Date currentDay) {
        ...
        List<LhScheduleResult> validPlans = new ArrayList<>();
        long id = 1;
        for (FactoryMonthPlanProdFinalVo plan : plans) {
            long t1dayPlan = plan.getFieldValueByFieldName("day" + (dayOfMonth - 1)) == null ? 0 : (long) plan.getFieldValueByFieldName("day" + (dayOfMonth - 1));
            long t2dayPlan = plan.getFieldValueByFieldName("day" + dayOfMonth) == null ? 0 : (long) plan.getFieldValueByFieldName("day" + dayOfMonth);
            if (t1dayPlan == 0 && t2dayPlan > 0) {
                // t+2 日有计划，t+1日没计划，补充
                validPlans.add(initializeT1scheduledTask(plan, id));
                id++;
            }
        }
        log.info("[硫化排程] 发现有效月度计划数量：{}", validPlans.size());

        log.debug("[硫化排程] 补充物料施工信息...");
        Map<String, List<MdmProductConstructionVO>> constructionMap = commonQueryCacheService
                .queryProductConstructionInfo(validPlans)
                .stream()
                .collect(Collectors.groupingBy(item -> item.getProductCode() + item.getSpecCode()));

        StringBuilder errorMsg = new StringBuilder();
        for (LhScheduleResult task : validPlans) {
            List<MdmProductConstructionVO> constructions = constructionMap.get(task.getProductCode() + task.getSpecCode());
            if (constructions != null && !constructions.isEmpty()) {

                task.setEmbryoCode(constructions.get(0).getEmbryoCode());
                task.setBomVersion(constructions.get(0).getBomVersion());
                task.setMouldMethod(constructions.get(0).getMouldMethod());
            } else {
                errorMsg.append("规格代码：").append(task.getSpecCode()).append("，物料代码").append(task.getProductCode()).append("，对应的施工信息不存在！").append("\n");
            }
