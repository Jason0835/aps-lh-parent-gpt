...
import com.zlt.aps.cx.mapper.entity.ConstructionParseMapper;
...
import com.zlt.aps.cxlh.cx.api.domain.vo.TMesConstructionParam;
import com.zlt.aps.maindata.enums.InsideRubberSortEnum;
import com.zlt.aps.maindata.enums.SideRubberSortEnum;
import com.zlt.aps.maindata.enums.TreadRubberSortEnum;
 ...
 * @Description
 */
@Service("constructionParseService")
public class ConstructionParseServiceImpl implements ConstructionParseService {
	/**
	 * 1000，主要用于米与毫米的换算
	 */
	 ...
	 * 施工表字段，用于反射赋值
	 */
	private static Map<String, Field> CONSTRUCTION_FIELDS_MAP = null;
	 ...
	 */
	@Override
    public List<CxProductConstructionInfo> getPartsConstruction(CxProductConstructionInfo params,
                                                                boolean isShowDisabled) {
		// 取出施工表字段映射配置
		List<TMesConstructionMapping> mappingList = constructionParseMapper.selectTPlmBomConstructionMapping();
		...
		// 查询本次需要更新的bom信息
		// 查询中间库与配置数据，构建成胎号与各待更新字段组合的数据结构
		String embryoCode = params.getEmbryoCode();
		String embryoVersion = params.getEmbryoVersion();
		String queryType = params.getQueryType();
		List<TMesConstructionInfo> constructionList = constructionParseMapper.selectTMesConstructionInfo(embryoCode,
				embryoVersion, queryType, !isShowDisabled);
		// 将组合好的施工表映射关系构建成施工信息列表
		List<TMesConstructionParam> parseConstructionList = this.parseConstruction(mappingList, columnNameList,
				constructionList);
		List<CxProductConstructionInfo> productConstructionList = parseConstructionList.stream()
				// 按序列号（版本号）排序
				.sorted(Comparator.comparing(TMesConstructionParam::getOrderNo))
				// 对象转换（转成CxProductConstructionInfo类型）
				.map(c -> transferColumn2Product(c, queryType)).collect(Collectors.toList());
		return productConstructionList;
	}
	 ...
	 * @return
	 */
	private CxProductConstructionInfo transferColumn2Product(TMesConstructionParam constructionInfo, String queryType) {
		CxProductConstructionInfo info = new CxProductConstructionInfo();
		info.setEmbryoCode(constructionInfo.getEmbryoCode());
		info.setEmbryoVersion(constructionInfo.getEmbryoVersion());
		constructionInfo.getColumnList().stream().filter(c -> queryType.equals(c.getMaterialType())).forEach(c -> {
			String columnName = c.getColumnName();
			String columnValue = c.getColumnValue();
			if (StringUtils.isEmpty(columnValue)) {
				return;
			}
			Field field = getConstructionFieldsMap().get(columnName);
			if (field != null) {
				Class<?> fieldType = field.getType();
				Object val = null;
				if (Integer.TYPE == fieldType || Integer.class == fieldType) {
					val = Integer.parseInt(columnValue);
				} else if (Double.TYPE == fieldType || Double.class == fieldType) {
					val = Double.parseDouble(columnValue);
				} else if (Long.TYPE == fieldType || Long.class == fieldType) {
					val = Long.parseLong(columnValue);
				} else if (String.class == fieldType) {
					val = columnValue;
				}
				ReflectUtils.invokeSetter(info, field.getName(), val);
			}
		});
		return info;
	}
	 ...
	 * @return
	 */
	private static synchronized Map<String, Field> getConstructionFieldsMap()  {
		if (CONSTRUCTION_FIELDS_MAP == null) {
			CONSTRUCTION_FIELDS_MAP = new ConcurrentHashMap<>();
			Field[] allFields = CxProductConstructionInfo.class.getDeclaredFields();
			for (int col = 0; col < allFields.length; col++) {
				Field field = allFields[col];
				TableField attr = field.getAnnotation(TableField.class);
				if (attr != null) {
					field.setAccessible(true);
					String columnName = attr.value();
					CONSTRUCTION_FIELDS_MAP.put(columnName, field);
				}
			}
		}
		return CONSTRUCTION_FIELDS_MAP;
	}
	...
	private int compareId(String id1, String id2) {
		...
		// 拆分ID
		String[] idArray1 = id1.split("/");
		String[] idArray2 = id2.split("/");
		// 判断两个ID的长度是否相等
	 ...
	 * @return
	 */
	private List<TMesConstructionParam> parseConstruction(List<TMesConstructionMapping> mappingList,
		...
		// 构建施工信息对象，用于数据库更新
		// 相同的版本施工信息对象中，并存放在map<版本号，施工信息>中
		Map<String, TMesConstructionParam> constructionMap = new HashMap<>();
		for (TMesConstructionInfo columnInfo : constructionList) {
			// 取出正常的版本号
			if (columnInfo.getColumnName().contains(VERSION) && StringUtils.isNotEmpty(columnInfo.getColumnValue())) {
				...
				TMesConstructionParam construction = new TMesConstructionParam();
				...
				// 版本号作为序号，用于排序
				construction.setOrderNo(columnInfo.getColumnValue());
				...
				constructionMap.put(columnInfo.getId(), construction);
			}
		}
		...
		for (TMesConstructionInfo columnInfo : constructionList) {
			...
			this.checkDataFormatError(columnInfo, org.springframework.util.CollectionUtils.firstElement(columnMappingList));
			TMesConstructionParam construction = null;
			// 通过胎胚号 + 胎胚版本号 + sap版本号 匹配施工信息
			for (Entry<String, TMesConstructionParam> entry: constructionMap.entrySet()) {
		...
		return new ArrayList<>(constructionMap.values());
	}
			...
			for (Entry<String, List<TMesConstructionInfo>> entry : columnMap.entrySet()) {
				...
				construction.getColumnList().stream().filter(column -> column.getColumnName().equals(columnName))
						.findAny().ifPresent(column -> {
							...
							column.setModify(finalColumnInfo.isModify());
							column.setMaterialType(finalColumnInfo.getMaterialType());
							// 取sapcode的更新日期
	...
	private void handleOriginalLineCode(TMesConstructionParam construction) {
		...
		originalLineCodeList.stream()
				...
				.ifPresent(c -> {
					...
					oriColumn.setModify(c.isModify());
					oriColumn.setMaterialType(c.getMaterialType());
				});
	}
	 ...
	 * @return
	 */
	private String handleColumnValueAfterSave(String columnName, String columnValue, String embryoCode) {
		if (StringUtils.isEmpty(columnValue)) {
			return columnValue;
		}
		...
		if (DIMENSION.equals(columnName)) {
			// 字符R的位置
			int flagIndex = columnValue.lastIndexOf("R");
			newColumnValue = this.substring(columnValue, flagIndex, 2, true);
		} else
		// 断面宽，取第一个“/”的左3位数字
		if (SECTION_WIDTH.equals(columnName)) {
			int index = columnValue.lastIndexOf("/");
			if (index < -0) {
				// 没有"/"则取"R"左边的3位数字
				index = columnValue.lastIndexOf("R");
			}
			// 截取长度
		...
		// 胎侧长度，bom表里取到的单位是米，需要换算成毫米
		if (SIDEWALL_LENGTH.equals(columnName)) {
			newColumnValue = new BigDecimal(columnValue).multiply(THOUSAND).toString();
		} else
		// 规格描述，需要截取胎胚之后的文字
		if (SPEC_DESC.equals(columnName)) {
			newColumnValue = columnValue.replaceFirst(embryoCode, "").trim();
		} else
		// 胎面长度，bom表里取到的单位是米，需要换算成毫米
		if (TREAD_SHOULDER_LENGTH.equals(columnName)) {
			newColumnValue = new BigDecimal(columnValue).multiply(THOUSAND).toString();
		}
	...
	private TMesConstructionInfo getDistinctValueColumn(String columnName, List<TMesConstructionInfo> columnMapList) {
		TMesConstructionInfo newColumnInfo = null;
		// 胎面胶料，如果有多个胶，需要斜杠合并起来，合并顺序是HT开头的胶放第一个，HS开头的胶放第二个，最后是HB开头的胶。
		if (TREAD_RUBBER_CATEGORY.equals(columnName)) {
			newColumnInfo = this.joinColumnValue(columnMapList, "/", new Comparator<TMesConstructionInfo>() {
				...
				public int compare(TMesConstructionInfo o1, TMesConstructionInfo o2) {
					Integer sort1 = TreadRubberSortEnum.getSortNo(o1.getColumnValue());
					Integer sort2 = TreadRubberSortEnum.getSortNo(o2.getColumnValue());
		...
		if (SIDEWALL_RUBBER.equals(columnName)) {
			// 过滤空值
			List<TMesConstructionInfo> tempColumnList = columnMapList.stream()
					.filter(c -> StringUtils.isNotEmpty(c.getColumnValue())).collect(Collectors.toList());
			newColumnInfo = this.joinColumnValue(tempColumnList, "/", new Comparator<TMesConstructionInfo>() {
				@Override
				public int compare(TMesConstructionInfo o1, TMesConstructionInfo o2) {
		...
		if (COLUMN_NAME_TIRE_FABRIC.contains(columnName)) {
			...
			// 子物料名称编码
			String childMaterialNameCode = latestColumnInfo.getChildMaterialCode();
			if (TIRE_FABRIC.equals(childMaterialNameCode)) {
				// 半钢胎胚，不带序号的形式
				// 字段名最后一位为本字段序号
				Integer seq = Integer.parseInt(columnName.substring(columnName.length() - 1));
				// 获取数值列表
				List<TMesConstructionInfo> columnList = columnMapList.stream()
		...
		// 如果都不符合条件，则直接取第一笔即可
		return newColumnInfo != null ? newColumnInfo : org.springframework.util.CollectionUtils.firstElement(columnMapList);
	}
	...
	private TMesConstructionInfo joinColumnValue(List<TMesConstructionInfo> columnMapList, String separator,
		...
		// 取出最新操作的栏位信息
		TMesConstructionInfo latestColumnInfo = columnMapList.stream().sorted(this.buildLastupdateSortor()).findFirst()
				.get();
		// 如果最新更新字段为空，则直接清空该字段
		if (StringUtils.isEmpty(latestColumnInfo.getColumnValue())) {
			return latestColumnInfo;
		...
		// 先对字段信息去重以及排序
		List<TMesConstructionInfo> newColumnMapList = columnMapList.stream()
				// 过滤掉数值为空的
		...
		List<String> valueList = newColumnMapList.stream().map(TMesConstructionInfo::getColumnValue)
				.filter(c -> StringUtils.isNotEmpty(c)).collect(Collectors.toList());
		// 按排好序的字段列表用指定分隔符拼接起来
		String columnValue = StringUtils.join(valueList, separator);
		// 构建一个新对象存值返回
		if (CollectionUtils.isNotEmpty(valueList)) {
			TMesConstructionInfo oldColumnInfo = org.springframework.util.CollectionUtils.firstElement(newColumnMapList);
