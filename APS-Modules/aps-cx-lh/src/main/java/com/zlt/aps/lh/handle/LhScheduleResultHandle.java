...
public class LhScheduleResultHandle {
    ...
    @Resource
    private AutoScheduleLogEntityMapper autoScheduleLogMapper;
    /**
     * 月度计划相关service
     */
    @Autowired
    ...
    public AutoLhScheduleResultContextDTO buildLhScheduleResult(AutoLhScheduleResultDTO autoLhScheduleResultDTO) {
        ...
        // 判断是否夏季
        contextDTO.setBSummerSeason(CxLhCommonUtils.isSummerSeason(paramsMap.get(LhParamCodeEnums.START_CURING_SUMMER_DAY.getCode()),paramsMap.get(LhParamCodeEnums.START_CURING_WINTER_DAY.getCode())));
        //初始化动态班次
        ...
        //初始化库存
        contextDTO.setCxStockMap(getEmbryoStockMap(contextDTO.getScheduleTime()));
...
//                .thenComparing(LhScheduleResultVo::getSpecCode);
    ...
    private void createAutoScheduleLog(AutoLhScheduleResultContextDTO contextDTO,String taskName) {
        AutoScheduleLog scheduleLog = new AutoScheduleLog();
        scheduleLog.setProcedureCode(ApsConstant.PROCEDURE_CODE_LH);
        scheduleLog.setBatchNo(contextDTO.getBatchNo());
        String title = "硫化自动排程("+DateUtils.parseDateToStr(DateUtils.YYYY_MM_DD,contextDTO.getScheduleTime())+")-"+taskName;
        scheduleLog.setTitle(title);
        scheduleLog.setLogDetail(contextDTO.getLogDetail().toString());
        autoScheduleLogMapper.insert(scheduleLog);
    ...
    private void calcOneMachineRemainCapacity(AutoLhScheduleResultContextDTO contextDTO, LhScheduleResultVo vo, List<LhScheduleResultVo> lhScheduledResultFinalVoList, LhMachineInfoVo machineVo, boolean isReplenishment) {
        ...
        // 计算理论单日产能
        int theoreticalCapacity = CxLhCommonUtils.calcPeriodCapacity(BigDecimal.valueOf(machineVo.getRemainTime()), BigDecimal.valueOf(curingTime), moldNum);
     ...
     * @return Map<胎胚号, 库存对象>胎胚库存Map
     */
    private Map<String, CxStock> getEmbryoStockMap(Date scheduleTime){
        //获取排程日期前1天的库存
        Date newScheduleTime = DateUtils.addDays(scheduleTime,-1);
        List<CxStock> cxStockList = cxStockService.queryStockByDate(newScheduleTime);
        // 构造“胎胚号 -> 库存信息”的映射（假设同一胎胚号只有一条库存记录）
        Map<String, CxStock> embryoStockMap = new HashMap<>();
        if (cxStockList != null && !cxStockList.isEmpty()) {
            for (CxStock stock : cxStockList) {
                embryoStockMap.put(stock.getEmbryoCode(), stock);
            }
        }
        return embryoStockMap;
    }
    ...
    private void buildEmbryoCode(AutoLhScheduleResultContextDTO contextDTO, String factoryCode, Date scheduleTime) throws BusinessException {
        // 1. 查询当天所有胎胚库存情况
        Map<String,CxStock> embryoStockMap = contextDTO.getCxStockMap();
        // 2. 从 T 日和 T+1 日硫化计划中，获取所有的规格代号
        Set<String> specCodes = new HashSet<>();
        if (contextDTO.getTDayScheduleList() != null) {
            for (LhScheduleResultVo vo : contextDTO.getTDayScheduleList()) {
                // 假设 LhScheduleResultVo 中包含 specCode 字段
                specCodes.add(vo.getProductCode()+"_"+vo.getSpecCode());
            }
        }
        if (contextDTO.getT1DayScheduleList() != null) {
            for (LhScheduleResultVo vo : contextDTO.getT1DayScheduleList()) {
        ...
        // 3. 根据规格代号和分厂编号查询 SAP 与施工关系表（施工对象），获取胎胚号
        List<MdmProductConstructionVO> constructionList = mdmProductConstructionService.queryByFactoryCodeAndSpecCodes(factoryCode, specCodes);
        if (constructionList == null || constructionList.isEmpty()) {
            throw new BusinessException(I18nUtil.getMessage("ui.data.column.lhUnScheduleResult.constructionRelationshipNotFound"));
        }

        // 构造“规格代号 -> 施工对象”的映射（假设每个规格对应一条记录）
        Map<String, MdmProductConstructionVO> specToConstructionMap = new HashMap<>();
        for (MdmProductConstructionVO cons : constructionList) {
            specToConstructionMap.put(cons.getProductCode()+"_"+cons.getSpecCode(), cons);
        }
        String trialPrefix = contextDTO.getLhParamsMap().get(LhParamCodeEnums.TRIAL_PRODUCTION_PRE_FIX.getCode());
        // 4. 遍历 T 日硫化计划，为每个 VO 设置胎胚号和库存情况
        if (contextDTO.getTDayScheduleList() != null) {
           for (LhScheduleResultVo vo : contextDTO.getTDayScheduleList()) {
                MdmProductConstructionVO cons = specToConstructionMap.get(vo.getProductCode() +"_"+vo.getSpecCode());
                if (cons != null) {
                    vo.setEmbryoCode(cons.getEmbryoCode());
                    // 根据胎胚号获取库存情况
                    CxStock stock = embryoStockMap.get(cons.getEmbryoCode());
                    vo.setEmbryoStock(stock != null && stock.getStockNum() != null ? stock.getStockNum():0);
                    vo.setBomVersion(cons.getBomVersion());
                    vo.setMouldMethod(cons.getMouldMethod());
                    vo.setMoldCavity(cons.getMoldCavity());
                    vo.setMouldClampingPressure(cons.getMouldClampingPressure());
                    if (StringUtils.isNotEmpty(trialPrefix) && StringUtils.isNotEmpty(cons.getConstructionCode())){
                        vo.setIsTrial(trialPrefix.indexOf(cons.getConstructionCode().substring(0,1)) >= 0 ? ApsConstant.TRUE:ApsConstant.FALSE);
                    }
                    if (contextDTO.getBSummerSeason()){
                        //夏季
                        vo.setMachineryCuringTime(cons.getCuringTime()+contextDTO.getMechanicalMachineOperTime());
                        vo.setHydraulicPressureCuringTime(cons.getHydraulicPressureCuringTime()+contextDTO.getHydraulicMachineOperTime());
                    }else{
                        //冬季
                        vo.setMachineryCuringTime(cons.getCuringTime2()+contextDTO.getMechanicalMachineOperTime());
                        vo.setHydraulicPressureCuringTime(cons.getHydraulicPressureCuringTime2()+contextDTO.getHydraulicMachineOperTime());
                    }
                }else{
                    throw new BusinessException(StringUtils.format(I18nUtil.getMessage("ui.data.column.lhUnScheduleResult.constructionNotMatch"),vo.getSpecCode()));
                }
            }
        }

        // 5. 遍历 T + 1 日硫化计划，为每个 VO 设置胎胚号和库存情况
        if (contextDTO.getT1DayScheduleList() != null) {
            for (LhScheduleResultVo vo : contextDTO.getT1DayScheduleList()) {
                MdmProductConstructionVO cons = specToConstructionMap.get(vo.getProductCode() +"_"+vo.getSpecCode());
                if (cons != null) {
    ...
    private void addTSpec(AutoLhScheduleResultContextDTO contextDTO,List<LhScheduleResultVo> currentDayList,List<FactoryMonthPlanProdFinalVo> monthPlanList,int currentDay){
        ...
        for (LhScheduleResultVo lastDayScheduleVo:lastDayScheduleList){
            ...
            if (diffQty >0){
                ...
                //单班硫化量
                int shiftCapacity = CxLhCommonUtils.calcPeriodCapacity(durationSec, lastDayScheduleVo.getLhTime(),lastDayScheduleVo.getMoldQty());
                //日计划量 = 单班硫化量 * 班制
    ...
    private List<FactoryMonthPlanProdFinalVo> mergeFactoryMonthPlanProdFinal(List<FactoryMonthPlanProdFinalVo> voList) {
        ...
        for (FactoryMonthPlanProdFinalVo vo : voList) {
            ...
            if (resultMap.containsKey(key)) {
                ...
                merged.setDay24( safeAdd(merged.getDay24(), vo.getDay24()) );
    ...
    private void setSingleMoldQty(LhScheduleResultVo lhScheduleResultVo, AutoLhScheduleResultContextDTO contextDTO) {
        ...
        int singleMoldShiftLhQty = CxLhCommonUtils.calcPeriodCapacity(BigDecimal.valueOf(diffMillTimes),lhScheduleResultVo.getLhTime(),lhScheduleResultVo.getMoldQty());
    ...
    private void machineOneSideReplenishment(AutoLhScheduleResultContextDTO contextDTO, List<LhScheduleResultVo> lhScheduleResultFinalVoList, String machineCode, LhScheduleResultVo scheduleResultVo) {
      ...
      /*  if (!checkNeedReplenishment(contextDTO,scheduleResultVo)){
        ...
        int replenishmentLhQty = CxLhCommonUtils.calcPeriodCapacity(BigDecimal.valueOf(diffMillTimes),BigDecimal.valueOf(realLhTime), scheduleResultVo.getMoldQty());
        //3、检查规格月度剩余,获取实际的补量
    ...
    private Integer getRemainClassQty(LhScheduleResultVo lhScheduleResultVo,
       ...
       /* List<LhScheduleResultVo> machineScheduledResutlList = lhScheduleResultFinalVoList.stream().filter(x -> lhScheduleResultVo.getLhMachineCode().equals(x.getLhMachineCode())).collect(Collectors.toList());
        ...
        //Double realLhTime = lhScheduleResultVo.getLhTime().doubleValue() + contextDTO.getBrushBagTime();
        Double realLhTime = lhScheduleResultVo.getLhTime().doubleValue();
        return CxLhCommonUtils.calcPeriodCapacity(BigDecimal.valueOf(diffMillTimes),BigDecimal.valueOf(realLhTime),lhScheduleResultVo.getMoldQty());
    }
