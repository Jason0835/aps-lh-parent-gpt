 ...
 */
@Service
public class DjLossSettingServiceImpl extends ServiceImpl<DjLossSettingMapper, DjLossSetting> implements DjLossSettingService {
    @Autowired
    private DjLossSettingMapper ncLossSettingMapper;
    @Autowired
     ...
     */
    private void checkParamAndUnique(DjLossSetting ncLossSetting) {
     ...
     */
    @Override
    public AjaxResult importData(List<DjLossSettingDto> list, boolean updateSupport, Long importLogId) {
        int successNum = 0;
        int failureNum = 0;
        // 校验
        List<ImportErrorLog> importErrorLogs = new ArrayList<>();
        List<DjLossSettingDto> importList = new ArrayList<>();
        //将机台名称转换为机台id，并做校验
...
//        Map<String, Long> machineCodeMap = machineInfoList.stream().collect(Collectors.toMap(NcMachineInfo::getMachineCode, NcMachineInfo::getId));
        ...
        for (int i = 0; i < list.size(); i++) {
            DjLossSettingDto lossSetting = list.get(i);
        ...
        try {
            ...
            if (updateSupport && CollectionUtils.isNotEmpty(importList)) {
                ...
                //查询数据库已存在对象
                for (int i = 0; i < list.size(); i++) {
                    DjLossSettingDto excelItem = list.get(i);
                    // 错误记录跳过
                    if (excelItem.getId() != null && excelItem.getId().equals(-999L)) {
                        continue;
                    }
                    // 唯一性校验
                    DjLossSetting tmLossSetting = new DjLossSetting();
