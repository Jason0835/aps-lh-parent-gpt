...
public class DjAssistSpecServiceImpl extends ServiceImpl<DjAssistSpecMapper, DjAssistSpec> implements DjAssistSpecService {
     ...
     * @return
     */
    public List<DjAssistSpec> listAssistSpec(DjAssistSpec dto) {
     ...
     * @param entity
     */
    public void saveAssistSpec(DjAssistSpec entity) {
    ...
    public void deleteAssistSpec(Long[] ids) {
        LambdaUpdateWrapper<DjAssistSpec> wrapper = new LambdaUpdateWrapper<>();
     ...
     * 根据code判断是否已经存在
     */
    public String checkAssistSpecCodeUnique(DjAssistSpec dto) {
        if (dto == null || StringUtils.isBlank(dto.getMaterialCode())) {
            return UserConstants.NOT_UNIQUE;
        }
        QueryWrapper<DjAssistSpec> queryWrapper = new QueryWrapper<>();
        ...
        List<DjAssistSpec> list = ncAssistSpecMapper.selectList(queryWrapper);
        if (list.size() > 0) {
            return UserConstants.NOT_UNIQUE;
        }
    ...
    public AjaxResult importData(List<DjAssistSpec> list, boolean updateSupport, Long importLogId) {
        ...
        List<DjAssistSpec> importList = new ArrayList<>();

        //按业务主键分组
        Map<String, Long> groupMap = list.stream().collect(Collectors.groupingBy(DjAssistSpec::getMaterialCode, Collectors.counting()));

        for (int i = 0; i < list.size(); i++) {
            DjAssistSpec bigRoll = list.get(i);
        ...
        if (CollectionUtils.isNotEmpty(list)) {
            try {
                ...
                if (updateSupport && CollectionUtils.isNotEmpty(importList)) {
                    ...
                    //查询数据库已存在对象
                    for (int i = 0; i < list.size(); i++) {
                        DjAssistSpec excelItem = list.get(i);
                        ...
                        if (UserConstants.UNIQUE.equals(unique)) {
                            //不存在插入
                            successNum++;
                            DjAssistSpec ncAssistSpec = new DjAssistSpec();
