...
import com.zlt.aps.dj.mapper.DjMouthPlateMapper;
 ...
 */
@Service
public class DjMouthPlateServiceImpl extends ServiceImpl<DjMouthPlateMapper, DjMouthPlate> implements DjMouthPlateService {

    @Autowired
    private DjMouthPlateMapper ncMouthPlateMapper;
    ...
    public DjMouthPlate selectMouthPlateById(Long id) {
        LambdaQueryWrapper<DjMouthPlate> wrapper = new LambdaQueryWrapper<>();
        wrapper.eq(DjMouthPlate::getDelFlag, ApsConstant.DEL_FLAG_NORMAL);
        wrapper.eq(DjMouthPlate::getId, id);
     ...
     */
    @Override
    public void saveMouthPlate(DjMouthPlate mouthPlate) {
    ...
    public void deleteMouthPlateByIds(Long[] ids) {
        ...
        LambdaUpdateWrapper<DjMouthPlate> wrapper = new LambdaUpdateWrapper<>();
     ...
     */
    @Override
    public AjaxResult importData(List<DjMouthPlateDto> list, boolean updateSupport, Long importLogId) {
        int successNum = 0;
        int failureNum = 0;

        List<DjMouthPlateDto> importList = new ArrayList<>();
...
//        Map<String, Long> machineCodeMap = machineInfoList.stream().collect(Collectors.toMap(NcMachineInfo::getMachineCode, NcMachineInfo::getId));
        ...
        for (int i = 0; i < list.size(); i++) {
            DjMouthPlateDto mouthPlate = list.get(i);
        ...
        try {
            ...
            if (updateSupport && CollectionUtils.isNotEmpty(importList)) {
                ...
                //查询数据库已存在对象
                for (int i = 0; i < list.size(); i++) {
                    DjMouthPlateDto excelItem = list.get(i);
                    // 错误记录跳过
                    if (excelItem.getId() != null && excelItem.getId().equals(-999L)) {
                        continue;
                    }
                    // 唯一性校验
                    DjMouthPlate ncMouthPlate = new DjMouthPlate();
