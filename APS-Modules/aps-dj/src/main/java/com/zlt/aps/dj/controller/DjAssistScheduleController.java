 ...
 */
@RestController
@RequestMapping("/dj/assistSchedule")
public class DjAssistScheduleController extends BaseController {

    @Value("${excelModelPath}")
    public String excelModelPath;
    @Autowired
    private DjAssistScheduleService ncAssistScheduleService;
    ...
    @PostMapping("/list")
    public TableDataInfo list(@RequestBody DjAssistSchedule ncAssistSchedule) {
        ncAssistSchedule.setOrderStr(orderStr());
        List<DjAssistSchedule> list = ncAssistScheduleService.selectNcAssistScheduleList(ncAssistSchedule);
     ...
     * 导出列表
     */
    @Log(title = "ui.data.column.nc.assistSchedule.modelName", businessType = BusinessType.EXPORT)
    @PostMapping("/export")
    public byte[] export(@RequestBody DjAssistSchedule assistSchedule) throws Exception {

        //查询数据
//        startPage("a.GLUE_SEQ,a.GLUE_CODE asc");
        assistSchedule.setOrderStr(orderStr());  //拿到前端传的排序字段+排序方式
        List<DjAssistSchedule> list = ncAssistScheduleService.selectNcAssistScheduleList(assistSchedule);
        ...
        if (Locale.SIMPLIFIED_CHINESE.equals(lang) || lang == null) {
            // 中文
            in = this.getClass().getClassLoader().getResourceAsStream(excelModelPath + "ncAssistSchedule.xlsx");
        } else if (Locale.US.equals(lang)) {
            // 英文
            in = this.getClass().getClassLoader().getResourceAsStream(excelModelPath + "ncAssistSchedule_en.xlsx");
        }
        Workbook webBook = ExcelUtils.readExcel(in);
        webBook.setSheetName(0, I18nUtil.getMessage("ui.data.column.nc.assistSchedule.modelName"));
        //填充数据
        if (CollectionUtils.isNotEmpty(list)) {
            ...
            for (int i = 0; i < list.size(); i++) {
                int n = 0;
                DjAssistSchedule scheduleResult = list.get(i);
...
//                row.createCell(n++).setCellValue(DateUtils.parseDateToStr("yyyy-MM-dd",scheduleResult.getScheduleDate()));
            ...
            if("zh_CN".equals(lang.toString())){
                dateStr= DateUtils.parseDateToStr("MM月dd日",assistSchedule.getScheduleDate());
            }else{
