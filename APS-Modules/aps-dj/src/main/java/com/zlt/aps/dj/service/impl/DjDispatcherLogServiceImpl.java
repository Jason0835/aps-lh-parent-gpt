...
import com.zlt.aps.dj.mapper.DjDispatcherLogMapper;
 ...
 */
@Service
public class DjDispatcherLogServiceImpl implements DjDispatcherLogService
{
    @Autowired
    private DjDispatcherLogMapper ncDispatcherLogMapper;
     ...
     */
    @Override
    public List<DjDispatcherLog> selectNcDispatcherLogList(DjDispatcherLog dispatcherLog)
    {
    ...
    public byte[] export(DjDispatcherLog dispatcherLog) {
        List<DjDispatcherLog> list = this.selectNcDispatcherLogList(dispatcherLog);
        //按用户语言读取模板
        ...
        if (CollectionUtils.isNotEmpty(machineInfoList)) {
            machineMap = machineInfoList.stream().collect(Collectors.toMap(item -> item.getId().toString(), DjMachineInfo::getMachineName));
        }
        ...
        if (CollectionUtils.isNotEmpty(list)) {
            ...
            for (int i = 0; i < list.size(); i++) {
                DjDispatcherLog log = list.get(i);
