...
import com.zlt.aps.itf.mes.mapper.DjMesSourceMapper;
import com.zlt.aps.itf.mes.mapper.MesDjScheduleResultMapper;
...
public class MesItfDjServiceImpl implements IMesItfDjService {
    ...
    @Autowired
    private DjMesSourceMapper djMesSourceMapper;
    @Autowired
    private MesDjScheduleResultMapper mesDjScheduleResultMapper;
    @Autowired
    ...
    public AjaxResult syncStock(AuxReqSyncDataLogs request) {
        ...
        List<DjStock> stockList = djMesSourceMapper.selectStockList(request);
    ...
    public AjaxResult issueDjScheduleResult(Long[] ids, String factoryCode,
        ...
        // 转换为MES实体
        List<MesDjScheduleResult> mesList = this.convertToMesList(todayList, dataVersion, companyCode, factoryCode);
        ...
        try {
            ...
            for (List<MesDjScheduleResult> batch : this.partitionList(mesList)) {
                mesDjScheduleResultMapper.batchInsert(batch);
            }
        } finally {
            DynamicDataSourceContextHolder.clear();
            /** 切换APS数据源 end **/
     ...
     * 转换为MES实体列表
     */
    private List<MesDjScheduleResult> convertToMesList(List<DjScheduleResult> list, String dataVersion,
     ...
     * 转换为MES中间表实体
     */
    private MesDjScheduleResult convertToMesEntity(DjScheduleResult item, String dataVersion, String companyCode,
            String factoryCode) {
        MesDjScheduleResult mesItem = new MesDjScheduleResult();
        mesItem.setScheduleDate(item.getScheduleDate());
        mesItem.setBatchNo(item.getBatchNo());
        mesItem.setOrderNo(item.getOrderNo());
        mesItem.setLiningCode(item.getPaddingCode());
