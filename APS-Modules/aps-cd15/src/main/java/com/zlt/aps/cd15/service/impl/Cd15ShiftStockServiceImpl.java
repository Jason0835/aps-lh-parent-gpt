...
import com.zlt.aps.cd15.mapper.Cd15ShiftStockMapper;
...
public class Cd15ShiftStockServiceImpl implements ICd15ShiftStockService {

    private final BaseDao baseDao;
    private final Cd15ShiftStockMapper shiftStockMapper;

    @Override
    public void replaceShiftStock(String factoryCode, Date stockDate, String shiftCode,
                                  Date shiftStartTime, String updateBy,
                                  List<Cd15ShiftStock> stockList) {
        if (StringUtils.isBlank(factoryCode) || stockDate == null || StringUtils.isBlank(shiftCode)
                || shiftStartTime == null) {
            throw new ServiceException(I18nUtil.getMessage("ui.cd15.shiftStock.syncArgumentsInvalid"));
        }
        ...
        List<Cd15ShiftStock> normalizedList = stockList == null
                ? new ArrayList<>() : new ArrayList<>(stockList);
        this.validateSource(normalizedList);
        normalizedList.sort(Comparator.comparing(Cd15ShiftStock::getMaterialCode));

        List<Cd15ShiftStock> existingList = this.shiftStockMapper.selectList(
                new LambdaQueryWrapper<Cd15ShiftStock>()
                        .eq(Cd15ShiftStock::getFactoryCode, normalizedFactoryCode)
                        .eq(Cd15ShiftStock::getShiftCode, normalizedShiftCode)
        ...
        if (this.isSameSnapshot(existingList, normalizedList)) {
            log.info("斜裁班次库存快照未变化，跳过替换：factoryCode={}，shiftCode={}，shiftStartTime={}，数量={}",
        ...
        Date now = new Date();
        this.shiftStockMapper.deleteInvalidByScope(
                normalizedFactoryCode, shiftStartTime, normalizedShiftCode);
        this.shiftStockMapper.logicDeleteByScope(
                normalizedFactoryCode, shiftStartTime, normalizedShiftCode, operator, now);
    ...
    private void validateSource(List<Cd15ShiftStock> stockList) {
        Set<String> materialCodes = new HashSet<>();
        for (Cd15ShiftStock stock : stockList) {
            if (stock == null || StringUtils.isBlank(stock.getMaterialCode())
                ...
                throw new ServiceException(I18nUtil.getMessage("ui.cd15.shiftStock.syncSourceInvalid"));
            }
            stock.setMaterialCode(stock.getMaterialCode().trim());
        }
    }

    private boolean isSameSnapshot(List<Cd15ShiftStock> existingList,
                                   List<Cd15ShiftStock> incomingList) {
        if (existingList == null || existingList.size() != incomingList.size()) {
            return false;
        }
        for (int index = 0; index < existingList.size(); index++) {
            Cd15ShiftStock existing = existingList.get(index);
            Cd15ShiftStock incoming = incomingList.get(index);
