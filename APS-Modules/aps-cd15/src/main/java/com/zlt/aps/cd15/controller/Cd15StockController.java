...
public class Cd15StockController extends BaseController {
    @Autowired
    private Cd15StockService stockService;
    ...
    public TableDataInfo list(@RequestBody Cd15Stock stock) {
        startPage();
        stock.setOrderStr(orderStr());
        List<Cd15Stock> list = stockService.selectStockList(stock);
    ...
    @ApiOperation("新增15°裁断库存信息（id不为空）")
    @PostMapping
    public AjaxResult add(@RequestBody Cd15Stock stock) {
        //唯一性校验（使用库存日期+物料编号为逻辑主键）
        List<Cd15Stock> list = stockService.checkStockListUnic(stock);
    ...
    @PutMapping
    public AjaxResult edit(@RequestBody Cd15Stock stock) {
        //唯一性校验（使用库存日期+物料编号为逻辑主键）
        List<Cd15Stock> list = stockService.checkStockListUnic(stock);
        if (CollectionUtils.isNotEmpty(list)) {
            return AjaxResult.error(I18nUtil.getMessage("ui.stock.message.unique"));
    ...
    public List<Cd15Stock> exportList(@RequestBody Cd15Stock stock) {
        stock.setOrderStr(orderStr());
        List<Cd15Stock> list = stockService.selectStockList(stock);
        return list;
    }
