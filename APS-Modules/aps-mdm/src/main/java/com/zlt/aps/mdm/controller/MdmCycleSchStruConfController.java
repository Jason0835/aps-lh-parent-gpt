...
import com.zlt.aps.mdm.mapper.MdmCycleSchStruConfEntityMapper;
import com.zlt.aps.mdm.service.IMdmCycleSchStruConfService;
...
public class MdmCycleSchStruConfController extends AbstractDocBizController<MdmCycleSchStruConf> {
    ...
    public AjaxResult importData(@RequestBody ImportContext importContext, @RequestParam("updateSupport") boolean updateSupport) throws Exception {
        ...
        ExcelUtil<MdmCycleSchStruConf> util = new ExcelUtil<>(this.getTClass());
        InputStream is = new ByteArrayInputStream(importContext.getFileBytes());
        List<MdmCycleSchStruConf> list = util.importExcel(is);
        return this.doImportDataAsync(list, updateSupport, importLog.getId(), importLog, beginTime);
    }

    public AjaxResult doImportDataAsync(List<MdmCycleSchStruConf> list, boolean updateSupport, long importLogId, ImportLog importLog, Date beginTime) {
        ...
        if (list.size() > 500) {
            ...
            mdmCycleSchStruConfService.importDataAsync(list, updateSupport, importLogId, importLog, beginTime, virtualAttr);
