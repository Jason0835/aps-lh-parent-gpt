...
public class TmDepthConfigServiceImpl extends AbstractDocService<TmDepthConfig> implements ITmDepthConfigService {
    ...
    public String checkRangeCross(TmDepthConfig entity) {
        ...
        List<TmDepthConfig> existingConfigList = tmDepthConfigMapper.selectList(queryWrapper);
        List<TmDepthConfig> configList = existingConfigList == null
     ...
     */
    @Override
    public AjaxResult importData(List<TmDepthConfig> configList, boolean updateSupport, Long menuId) {
     ...
     * @return 校验结果
     */
    private String validateImportConfig(List<TmDepthConfig> importConfigList) {
        ...
        Map<String, List<TmDepthConfig>> importConfigMap = importConfigList.stream()
                .collect(Collectors.groupingBy(TmDepthConfig::getFactoryCode));
        for (Map.Entry<String, List<TmDepthConfig>> entry : importConfigMap.entrySet()) {
            LambdaQueryWrapper<TmDepthConfig> queryWrapper = new LambdaQueryWrapper<>();
            queryWrapper.eq(TmDepthConfig::getFactoryCode, entry.getKey());
            List<TmDepthConfig> existingConfigList = tmDepthConfigMapper.selectList(queryWrapper);
            Set<Long> importIdSet = entry.getValue().stream()
                    .map(TmDepthConfig::getId)
                    .filter(Objects::nonNull)
                    .collect(Collectors.toCollection(HashSet::new));
            List<TmDepthConfig> combinedConfigList = existingConfigList == null
     ...
     *
     * @param entity 胎面备库班数配置
     * @return 合法返回 true，否则返回 false
     */
    private boolean isValidConfig(TmDepthConfig entity) {
     ...
     * @return 校验结果
     */
    private String validateContinuity(List<TmDepthConfig> configList) {
        if (configList == null || configList.isEmpty()) {
            return UserConstants.UNIQUE;
        }
        configList.sort(Comparator.comparing(TmDepthConfig::getMinMachineQty));
        if (!this.isValidConfig(configList.get(0)) || configList.get(0).getMinMachineQty() != 1) {
            return UserConstants.NOT_UNIQUE;
        }
        for (int index = 0; index < configList.size(); index++) {
            TmDepthConfig currentConfig = configList.get(index);
