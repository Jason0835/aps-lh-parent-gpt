...
import com.zlt.aps.xwyy.mapper.XwyyDispatcherLogMapper;
...
/**
 * 纤维压延调度员排程操作日志Service业务层处理
 * 
 * @author Gim
 * @date 2022-02-25
 */
@Service
public class XwyyDispatcherLogServiceImpl implements XwyyDispatcherLogService
{
    @Autowired
    private XwyyDispatcherLogMapper xwyyDispatcherLogMapper;
    ...
    {
        return xwyyDispatcherLogMapper.selectXwyyDispatcherLogById(id);
    }
     ...
     */
    @Override
    public List<XwyyDispatcherLog> selectXwyyDispatcherLogList(XwyyDispatcherLog dispatcherLog)
    {
        if (StringUtils.isNotEmpty(dispatcherLog.getEndTime())) {
            dispatcherLog.setEndTime(dispatcherLog.getEndTime() + " 23:59:59");
        }
        return xwyyDispatcherLogMapper.selectXwyyDispatcherLogList(dispatcherLog);
    }
    ...
    {
        xwyyDispatcherLog.setBaseVale(null);
        return xwyyDispatcherLogMapper.insertXwyyDispatcherLog(xwyyDispatcherLog);
    }
    ...
    public byte[] export(XwyyDispatcherLog dispatcherLog) {
        List<XwyyDispatcherLog> list = this.selectXwyyDispatcherLogList(dispatcherLog);
        //按用户语言读取模板
        InputStream in = this.getClass().getClassLoader().getResourceAsStream(excelModelPath +
                I18nUtil.getMessage("ui.data.column.xwyy.dispatcherlog.modelName") + ".xlsx");
        List<XwyyMachineInfo> machineInfoList = machineInfoService.selectMachineInfoList(new XwyyMachineInfo());
        Map<String, String> machineMap = new HashMap<>();
        if (CollectionUtils.isNotEmpty(machineInfoList)) {
            machineMap = machineInfoList.stream().collect(Collectors.toMap(item -> item.getId().toString(), XwyyMachineInfo::getMachineName));
        }
        ...
        if (CollectionUtils.isNotEmpty(list)) {
            ...
            for (int i = 0; i < list.size(); i++) {
                XwyyDispatcherLog log = list.get(i);
