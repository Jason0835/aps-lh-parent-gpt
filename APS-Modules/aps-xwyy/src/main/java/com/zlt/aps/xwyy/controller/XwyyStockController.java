...
public class XwyyStockController extends BaseController {
    @Autowired
    private XwyyStockService stockService;
    ...
    public TableDataInfo list(@RequestBody XwyyStock stock) {
        startPage();
        stock.setOrderStr(orderStr());
        List<XwyyStock> list = stockService.selectStockList(stock);
     ...
     * 新增纤维压延库存信息
     */
    @Log(title = "ui.frame.page.xwyy.stock.title", businessType = BusinessType.INSERT)
    @ApiOperation("新增纤维压延库存信息（id不为空）")
    @PostMapping
    public AjaxResult add(@RequestBody XwyyStock stock) {
        //唯一性校验（使用库存日期+物料编号为逻辑主键）
        List<XwyyStock> list = stockService.checkStockListUnic(stock);
     ...
     * 修改纤维压延库存信息
     */
    @Log(title = "ui.frame.page.xwyy.stock.title", businessType = BusinessType.UPDATE)
    @ApiOperation("修改纤维压延库存信息（id不为空）")
    @PutMapping
    public AjaxResult edit(@RequestBody XwyyStock stock) {
        //唯一性校验（使用库存日期+物料编号为逻辑主键）
        List<XwyyStock> list = stockService.checkStockListUnic(stock);
        if (CollectionUtils.isNotEmpty(list)) {
            return AjaxResult.error(I18nUtil.getMessage("ui.stock.message.unique"));
     ...
     * 删除纤维压延库存信息
     */
    @Log(title = "ui.frame.page.xwyy.stock.title", businessType = BusinessType.DELETE)
    @ApiOperation("根据id批量删除纤维压延库存信息")
     ...
     * 查询列表
     */
    @Log(title = "ui.frame.page.xwyy.stock.title", businessType = BusinessType.EXPORT)
    @PostMapping("/exportList")
    public List<XwyyStock> exportList(@RequestBody XwyyStock stock) {
        stock.setOrderStr(orderStr());
        List<XwyyStock> list = stockService.selectStockList(stock);
        return list;
    }

    @Log(title = "ui.frame.page.xwyy.stock.title", businessType = BusinessType.IMPORT)
    @ApiOperation("导入数据")
