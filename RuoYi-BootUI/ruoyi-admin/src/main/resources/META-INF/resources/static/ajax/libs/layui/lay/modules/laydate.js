    ...
    */
    ...
    Class.prototype.checkDate = function (fn) {
        ...
        if (typeof value === 'string' && value) {
            if (that.EXP_IF.test(value)) { //校验日期格式
                ...
                that.hint(that.lang().hintDateFormate + (
                    options.range ? (options.format + ' ' + options.range + ' ' + options.format) : options.format
                ) + that.lang().hintHasReset);
                error = true;
            }
    ...
    Class.prototype.tool = function (btn, type) {
        var that = this
            ...
            , confirm: function () {
                if (options.range) {
                    if (!that.endDate) return that.hint(that.lang().hintChooseDateRange);
                    if (lay(btn).hasClass(DISABLED)) return that.hint(
                        options.type === 'time' ? TIPS_OUT.replace(that.lang().type[0], that.lang().type[1]) : TIPS_OUT
                    );
                } else {
                    if (lay(btn).hasClass(DISABLED)) return that.hint(that.lang().hintNotInRange);
                }
                that.done();
                that.setValue(that.parse()).remove()
            }
