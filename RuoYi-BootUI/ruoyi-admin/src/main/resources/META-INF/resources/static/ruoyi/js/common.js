    ...
    if ($(".time-input").length > 0) {
        layui.use('laydate', function () {
            ...
            $(".time-input").each(function (index, item) {
                ...
                // 控制回显格式
                var format = time.attr("format") || 'yyyy-MM-dd';
                // 工具按钮,右下角显示的按钮，会按照数组顺序排列，内置可识别的值有：clear、now、confirm
                var buttons = time.attr("btns") || 'clear|now|confirm', newBtnArr = [];
                // 日期控件选择完成后回调处理
                var callback = time.attr("data-callback") || {};
                //有效最小日期，年月日必须用“-”分割，时分秒必须用“:”分割。注意：它并不是遵循 format 设定的格式。
                var min = time.attr("min") || '1900-1-1';
                ////有效最大日期，同上
                var max = time.attr("max") || '2099-12-31';
                //主题,内置了多种主题，theme的可选值有：default（默认简约）、molv（墨绿背景）、#颜色值（自定义颜色背景）、grid（格子主题）
                var theme = time.attr("theme") || 'default';
                //语言，只支持cn/en，即中文和英文
                var lang = time.attr("theme") || 'cn';
                //控件定位方式定位, 默认absolute，支持：fixed/absolute/static
                var position = time.attr("position") || null;
                //是否开启公历重要节日，仅支持中文版
                var calendar = eval(time.attr("calendar")) || false;
                //开启左右面板范围选择;如果设置 true，将默认采用 “ - ” 分割。 你也可以直接设置 分割字符 range: '~'
                var range = eval(time.attr("range")) || false;
                //层叠顺序,一般用于解决与其它元素的互相被遮掩的问题。如果 position 参数设为 static 时，该参数无效
                var zIndex = eval(time.attr("zIndex")) || null;
                //支持传入符合format参数设定的日期格式字符
                var value = time.val() || null;
...
function addColumnsClick(){
    //获取当前iframe页地址
    var localHrefKey = window.location.pathname;
    var reg=/\d/;
    var hrefList = localHrefKey.split("/");
    var clearSelf = function(){
        if(hrefList[hrefList.length-1].indexOf("?")>-1||reg.test(hrefList[hrefList.length-1])){
            hrefList.pop();
            clearSelf();
        }
    }
    clearSelf();
    localHrefKey = hrefList.join("/")
    //获取当前用户名
    var userName = parent.$(".user-menu").find('.hidden-xs').text();
    //获取当前iframe页列设置
    var localList = storage.get(localHrefKey+userName);
    //获取当前iframe页所有列按钮
    var allColumns = $(".dropdown-item-marker").find("input");
    //初始加载列配置
    if(localList!=null){
        localList = localList.split(",");
        for(var i = 0;i<localList.length;i++){
            if(localList[i]=="false"&&allColumns[i].checked){
                allColumns[i].click();
            }else if(localList[i]=="true"&&!allColumns[i].checked){
                allColumns[i].click();
            }
        }
    }
    $(".dropdown-item-marker").find("input").on('click',function(){
        //列表是否显示列表
        var columnsListStatus = [];
        for(var i = 0;i<allColumns.length;i++){
            columnsListStatus.push(allColumns[i].checked)
        }
        storage.set(localHrefKey+userName,columnsListStatus.join(","))
    })
}
