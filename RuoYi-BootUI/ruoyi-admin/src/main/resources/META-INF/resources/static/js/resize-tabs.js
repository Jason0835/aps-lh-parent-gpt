(function (c, a, e) {
    ...
    b = function (j, i) {
        var h = this.$tabs = e(j), k = this.options = e.extend(true, {}, f, i), g = this.$nav = h.find(k.navSelector),
            l = this.$dropdown = g.find(k.dropdownSelector); Def = this.$rightView = h.next(),tabLeft = this.$leftView = h.prev();
    ...
    b.prototype = {
        init: function () {
            ...
            var l = this.$tabs.width() - this.$rightView.width()-this.$leftView.width()-10, j = 0, h = this, g = this.options.activeClassName,
                m = -1,k = function (p) {
                    var o = p;
                    if (p === h.itemsLenth) {
                        o = p - 1
            ...
            for(var m_index = 0;m_index<this.$tabPanel.length;m_index++){
                if(this.$tabPanel.filter("." + g)[0]==this.$tabPanel[m_index]){
                    m=m_index;
                }
            }
