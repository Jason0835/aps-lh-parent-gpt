...
import com.ruoyi.common4ui.utils.AddressUtils;
...
//                operLog.setOperLocation(AddressUtils.getRealAddressByIP(operLog.getOperIp()));
                SpringUtils4BootUI.getBean(ISysOperLogService.class).add(operLog);
            }
        };
    }
    ...
    public static TimerTask recordLogininfor(final String username, final String status, final String message, final Object... args) {
        ...
        final String ip = AuthorizationUtils.getIp();
        return new TimerTask() {
            @Override
            public void run() {
...
//                    logininfor.setStatus(Constants.SUCCESS.toString());
//                } else if (Constants.LOGIN_FAIL.equals(status)) {
//                    logininfor.setStatus(Constants.FAIL.toString());
//                }
                // 插入数据
                SpringUtils.getBean(ISysLogininforService.class).add(username, status, message);
            }
        };
    }
