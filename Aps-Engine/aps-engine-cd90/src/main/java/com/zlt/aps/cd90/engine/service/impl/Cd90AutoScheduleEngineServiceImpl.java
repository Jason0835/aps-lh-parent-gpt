...
public class Cd90AutoScheduleEngineServiceImpl implements Cd90AutoScheduleEngineService {
    ...
    public Cd90AutoScheduleContext prepare(String factoryCode, Date scheduleDate) {
        ...
        int enabledShiftCount = countEnabledShifts(factoryCode);
        Cd90AutoScheduleParameters parameters = parameterService.load(factoryCode, enabledShiftCount);
        ...
        log.info("[直裁自动排程] Engine计算上下文准备完成, factoryCode={}, scheduleDate={}, "
                ...
                factoryCode, localScheduleDate, enabledShiftCount,
                parameters.getScheduleWindow(), parameters.getFingerprint());
        return context;
    }

    private int countEnabledShifts(String factoryCode) {
        ...
        return Math.toIntExact(shiftMapper.selectCount(wrapper));
    }
