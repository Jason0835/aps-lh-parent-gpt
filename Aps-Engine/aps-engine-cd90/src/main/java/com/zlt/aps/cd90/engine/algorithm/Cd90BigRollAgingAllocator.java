 ...
 * 大卷静置成熟流水分配器。
 *
 * <p>大卷帘布入库后需经过一定时间的自然静置（成熟）方可上机使用。
...
public class Cd90BigRollAgingAllocator {
    ...
    public Cd90BigRollAgingAllocation preview(List<Cd90BigRollAgingStock> stocks, String bigRollCode,
        ...
        List<Cd90BigRollAgingAllocationItem> items = new ArrayList<>();
        BigDecimal remaining = requestQty;
        LocalDateTime latestReleaseTime = originalStartTime;
        // 按释放时间升序遍历可用大卷，逐卷分配直至满足需求
        for (Cd90BigRollAgingStock stock : sortedAvailableStocks(stocks, bigRollCode)) {
            ...
            items.add(Cd90BigRollAgingAllocationItem.builder().stock(stock).quantity(used).build());
     ...
     */
    public Cd90BigRollAgingAllocation allocate(List<Cd90BigRollAgingStock> stocks, String bigRollCode,
            BigDecimal requestedQuantity, LocalDateTime originalStartTime) {
        // 先试算确定分配方案，再提交扣减
     ...
     */
    public void commit(Cd90BigRollAgingAllocation allocation) {
    ...
    /**
     * 筛选指定帘布规格下所有可用的库存流水，按释放时间升序排列。
     * 释放时间相同时按入库时间排序，仍相同按来源ID排序保证确定性。
     */
    private List<Cd90BigRollAgingStock> sortedAvailableStocks(List<Cd90BigRollAgingStock> stocks, String bigRollCode) {
    ...
    /** 判断库存流水是否属于指定帘布规格 */
    private boolean sameBigRoll(Cd90BigRollAgingStock stock, String bigRollCode) {
        return stock != null && StringUtils.hasText(stock.getBigRollCode())
                && stock.getBigRollCode().equals(bigRollCode);
    }

    /** 构建分配失败结果：已分配量为零，延迟秒数为零 */
    private Cd90BigRollAgingAllocation failure(BigDecimal requestedQuantity, LocalDateTime originalStartTime) {
        return Cd90BigRollAgingAllocation.builder()
                .success(false)
