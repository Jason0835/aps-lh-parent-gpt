...
public class DeductionDayProductionInfoLogRecorder {
    ...
    /**
     * 增加 日产能释放(日产能排产扣减) 日志信息记录
     * =====工厂%s, 计划年月：%d-%d, 需求计划版本：%s, 排产版本：%s，结构：%s [%s]模具日产能开始释放====
     *
     * @param context           排程上下文
     * @param groupName         分组
     * @param deductionType     类型
     ...
     * @return
     */
    public static String addStartGroupMouldDeductionDayCapacityLog(Context context, String groupName, DeductionDayProductionTypeEnum deductionType, String cxMachineCode, String deductionDaysInfo) {
        String logContent = String.format("=====工厂%s, 计划年月：%d-%d, 需求计划版本：%s, 排产版本：%s，结构：%s [%s]模具日产能开始释放====",
                context.getFactoryCode(), context.getYear(), context.getMonth(), context.getMonthPlanVersion(), context.getProductionVersion(),
                groupName, cxMachineCode, deductionType.getDesc(), deductionDaysInfo);
        ProductionPlanLogDto productionPlanInfo = ProductionPlanLogDto.getEmpty();
        TbrProductionLogUtils.addProductionLog(context, productionPlanInfo, TbrMouldProductionLogType.GROUP_DEDUCTION_DAY, logContent);
