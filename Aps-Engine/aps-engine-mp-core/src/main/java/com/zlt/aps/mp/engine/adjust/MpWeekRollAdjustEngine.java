...
public class MpWeekRollAdjustEngine {
     ...
     */
    public void doProductAlignForOne(MpRollAdjustContextDTO contextDTO, List<FactoryMonthPlanFinalAdjustVo> mpProdFinalList) throws BusinessException {
        //1.解析出超产减量、在产SKU增量
        List<FactoryMonthPlanFinalAdjustVo> deductAdjustList = new ArrayList<>();
        List<FactoryMonthPlanFinalAdjustVo> onIncrementAdjustList = new ArrayList<>();
        ...
        for (FactoryMonthPlanFinalAdjustVo finalAdjustVo:mpProdFinalList){
            ...
            if (finalAdjustVo.getPendingQty() < 0){
                //1.1 减量
                deductAdjustList.add(finalAdjustVo);
            }
            if (finalAdjustVo.getPendingQty() > 0){
                //1.2 增量
                onIncrementAdjustList.add(finalAdjustVo);
            }
        }
        ...
        doProductAlignWithDeduct(contextDTO,deductAdjustList);
        endTime = new Date();
        ...
        mpProdFinalList.stream().forEach(x->{
            ...
            if (isConvertionProd(x) && x.getConventionProductionQty() !=null && x.getConventionProductionQty() >0) {
                splitMatchQtyByDay(contextDTO,x.getConventionProductionQty(), lockNextDay,x);
            }else if (x.getCycleProductionQty() !=null && x.getCycleProductionQty() >0){
                splitMatchQtyByDay(contextDTO,x.getCycleProductionQty(), lockNextDay,x);
            }
        });
        ...
        doProductAlignWithOnlineInc(contextDTO,onIncrementAdjustList,mpProdFinalList);
        endTime = new Date();
        ...
        //8.优化：其他SKU往后移动
        startTime = new Date();
        contextDTO.getLogDetail().append(String.format("结构:%s,【其他SKU向后移动】,开始时间:%s",contextDTO.getStructureName(), DateUtils.parseDateToStr(DateUtils.YYYY_MM_DD_HH_MM_SS,startTime))).append(ApsConstant.DIVISION);
        moveBackwardWithOtherSku(contextDTO,lockNextDay,mpProdFinalList);
        endTime = new Date();
        contextDTO.getLogDetail().append(String.format("结构:%s,【其他SKU向后移动】,结束时间:%s,总耗时:%s毫秒",contextDTO.getStructureName(), DateUtils.parseDateToStr(DateUtils.YYYY_MM_DD_HH_MM_SS,endTime),DateUtils.getDiffMillTime(startTime,endTime))).append(ApsConstant.DIVISION);
    }
    ...
    private void doTransferConvertionQty(List<FactoryMonthPlanFinalAdjustVo> mpProdFinalList) {
        ...
        for (FactoryMonthPlanFinalAdjustVo mpFinalVo : mpProdFinalList){
            ...
            if ((mpFinalVo.getCycleProductionQty() !=null && mpFinalVo.getCycleProductionQty() > 0) ||
                    (mpFinalVo.getConventionProductionQty() != null && mpFinalVo.getConventionProductionQty() >0)){
     ...
     * @return
     */
    private boolean isConvertionProd(FactoryMonthPlanFinalAdjustVo mpFinalVo){
        return ProductionGroupTypeEnum.CONVENTION.getGroupType().equals(mpFinalVo.getStructureType());
    }
    /**
     * 初始化日产信息（包括日最大排产量、开停产标识、日产比例、最大硫化机台数、OEM排产量）
     * @param contextDTO
    ...
    public void initDayProductionInfo(MpRollAdjustContextDTO contextDTO,Map<Integer, MpDailyCapacityLimitVo> dailyCapacityLimitVoMap){
        ...
        Integer dayMaxChangeMould = (Integer) contextDTO.getParamMap().get(MonthPlanEnums.CHANGE_MOULD_LH_MACHINE_NUMBER.getCode());
    ...
    private int[] calcOtherStructureCapacity(MpRollAdjustContextDTO contextDTO, Map<String, MpMonthPlanStatistics> structureStatisticMap, int iDay) {
        ...
        for (Map.Entry<String, MpMonthPlanStatistics> entry1 : structureStatisticMap.entrySet()){
            ...
            if (StringUtils.isNotEmpty(dayStatisticsStr) && JSONValidator.from(dayStatisticsStr).validate()) {
                ...
                //按日初始统计硫化机台
                initStatisticsMachinesByDay(contextDTO, iDay, dayStatistics,statistics);
            }
        }
        return resultArr;
    }
     ...
     * @param dayStatistics
     */
    private void initStatisticsMachinesByDay(MpRollAdjustContextDTO contextDTO, int iDay, MpDayProductionStatisticsDetailVo dayStatistics,MpMonthPlanStatistics statistics) {
        if (PubUtil.isNotEmpty(dayStatistics.getMouldShellList()) && contextDTO.getMouldShellBlockMachinesMap() != null){
            //按模壳标准初始换活块机台数
            for (MpDayProductionStatisticsShellVo shellVo : dayStatistics.getMouldShellList()){
                String key = shellVo.getMouldShell() + BusiConstant.WeekRollAdjust.SPLIT_GROUP_KEY+ iDay;
                Integer blockMachines = Convert.toInt(contextDTO.getMouldShellBlockMachinesMap().get(key),0);
                blockMachines += Convert.toInt(shellVo.getBlockMachines());
                contextDTO.getMouldShellBlockMachinesMap().put(key,blockMachines);
            }
        }
        if (contextDTO.getInchMachinesMap() != null){
            //英寸日，硫化机台数
            String proSizeKey = statistics.getProSize() + BusiConstant.WeekRollAdjust.SPLIT_GROUP_KEY+ iDay;
            Integer lhMachines = Convert.toInt(contextDTO.getInchMachinesMap().get(proSizeKey),0);
            lhMachines += Convert.toInt(dayStatistics.getLhMachines(),0);
            contextDTO.getInchMachinesMap().put(proSizeKey, lhMachines);
        }

        if (contextDTO.getStructureMachinesMap() != null){
            //结构日，硫化机台数
            String structureKey = statistics.getStructureName() + BusiConstant.WeekRollAdjust.SPLIT_GROUP_KEY+ iDay;
     ...
     */
    private void moveBackwardWithOtherSku(MpRollAdjustContextDTO contextDTO, int lockNextDay,
                                         List<FactoryMonthPlanFinalAdjustVo> mpProdFinalList){
        //1、从锁定次日向后遍历排产计划，检测每日硫化机台数超限制数或每日胎胚种类数超限制数的日期，记为有超限的日期；
        //2、在有超限的日期向前依次找SKU，越靠近的SKU优先移动；
        //3、将SKU整体模拟往后移动，并向后再次检测每日硫化机台数、胎胚种类数的符合性，若符合，则可以移动，否则不能移动，继续找下一个SKU；
        int secStartDay;
        ...
        for (int i = lockNextDay; i<= contextDTO.getStructureDeadLine(); i++){
            ...
            //1、从第2天开始查找SKU
            secStartDay = i+1;
            canMoveFinalList = findCanMoveSkuListByBackward(mpProdFinalList,secStartDay);
            ...
            if (PubUtil.isEmpty(canMoveFinalList)){
                //若没有可以移动的列表，则退出
                contextDTO.getLogDetail().append(String.format("结构:%s,【其他SKU向后移动】,排产日:%s,排产次日:%s查找可移动的SKU,未找到退出！",contextDTO.getStructureName(),i,secStartDay)).append(ApsConstant.DIVISION);
                break;
            }
            //2、移动SKU列表，直到第I天没有剩余空间
            int cavityQty;
            for (FactoryMonthPlanFinalAdjustVo finalVo:canMoveFinalList){
                adjustDailyCapacityLimitObj.calcLhMachinesWithEmbryoTypes(mpProdFinalList,i, dailyCapacityLimitVoMap.get(i), contextDTO.getParamMap(),finalVo.getMainPattern(),finalVo.getEmbryoCode());
                contextDTO.getLogDetail().append(String.format("结构:%s,【其他SKU向后移动】,排产日:%s,其产能限制信息:%s！",contextDTO.getStructureName(),i,contextDTO.getDailyCapacityLimitVoMap().get(i) == null ? "" : contextDTO.getDailyCapacityLimitVoMap().get(i).toString())).append(ApsConstant.DIVISION);
                //2.1、检查: 当前每日硫化机台数\当前每日胎胚种类数 符合性
                //2.2、检查：主花纹向下模具数量(/2转成机台数) 符合性
                ...
                if (adjustDailyCapacityLimitObj.checkCapacitySatisfy(contextDTO.getDailyCapacityLimitVoMap().get(i)) &&
                        checkMouldSatisfy(dailyCapacityLimitVoMap.get(i),cavityQty)){
                    continue;
                }
                ...
                //2.4、增模排产
                contextDTO.getLogDetail().append(String.format("结构:%s,【其他SKU向后移动】--增模排产,排产日:%s,物料编码:%s,开始！",contextDTO.getStructureName(), i,finalVo.getMaterialCode())).append(ApsConstant.DIVISION);
                incMouldProduction(mpProdFinalList, contextDTO, i, finalVo.getTotalQty(), finalVo,null);
                contextDTO.getLogDetail().append(String.format("结构:%s,【其他SKU向后移动】--增模排产,排产日:%s,物料编码:%s,结束！",contextDTO.getStructureName(), i,finalVo.getMaterialCode())).append(ApsConstant.DIVISION);
                //重置开始日\结束日\汇总值
     ...
     * @return 可以移动的SKU列表
     */
    private List<FactoryMonthPlanFinalAdjustVo> findCanMoveSkuListByBackward(List<FactoryMonthPlanFinalAdjustVo> mpProdFinalList, int secStartDay) {
        List<FactoryMonthPlanFinalAdjustVo> finalVoList = mpProdFinalList.stream().filter(x-> (!x.isMoveFlag()) && x.getBeginDay()!=null && x.getBeginDay()>=secStartDay)
                .sorted(Comparator.comparing(FactoryMonthPlanFinalAdjustVo::getBeginDay).reversed()
                        .thenComparing(new AdjustAlignMoveComparator())).collect(Collectors.toList());
     ...
     * @throws BusinessException
     */
    private void doProductAlignWithDeduct(MpRollAdjustContextDTO contextDTO, List<FactoryMonthPlanFinalAdjustVo> mpProdFinalList) throws BusinessException {
        if(PubUtil.isEmpty(mpProdFinalList)){
            return;
        }
        //注：实单减量，先扣月计划的已排实单
        ...
        for (FactoryMonthPlanFinalAdjustVo mpFinalVo:mpProdFinalList){
            ...
            //剩余调整量绝对值
            reAdjustQty =  Math.abs(mpFinalVo.getPendingQty());
            ...
            contextDTO.getLogDetail().append(String.format("结构:%s,【减量调整】,物料编码:%s,待调整量:%s,实单量:%s,锁定量:%s,允许扣减量:%s",contextDTO.getStructureName(),mpFinalVo.getMaterialCode(),reAdjustQty,realBillQty,mpFinalVo.getLockQty(),allowDeductQty)).append(ApsConstant.DIVISION);
            if (needDeductQty == 0){
                //提示：【减量】物料编码:%s，确认调整量:%s，实单量:%s，锁定量:%s，允许扣减量:%s！
                addAdjustProcLog(contextDTO,mpFinalVo,String.format(I18nUtil.getMessage("alg.data.mp.weekRollAdjust.log.zeroAdjustDeduct"),mpFinalVo.getMaterialCode(),reAdjustQty,realBillQty,mpFinalVo.getLockQty(),allowDeductQty));
                continue;
            }
     ...
     * @throws BusinessException
     */
    private void doProductAlignWithOnlineInc(MpRollAdjustContextDTO contextDTO,
                                            List<FactoryMonthPlanFinalAdjustVo> onIncrementAdjustList,
                                            List<FactoryMonthPlanFinalAdjustVo> mpProdFinalList) throws BusinessException {
        ...
        Map<String, FactoryMonthPlanFinalAdjustVo> mpAdjustStructInMap = onIncrementAdjustList.stream().collect(Collectors.groupingBy(item->item.getMaterialCode(),
                Collectors.collectingAndThen(Collectors.toList(),m-> {
                    return m.get(0);
                })));
        ...
        FactoryMonthPlanFinalAdjustVo adjustStructInVo;
        ...
        //2、续作优先排产；
        doProductAlignWithContinueProduction(contextDTO,reLocateProdFinalList,mpAdjustStructInMap);
        //3、非续作排产
        for (int i=lockNextDay; i<contextDTO.getStructureDeadLine();i++) {
            //3.1、敲定SKU新的上机日期
            ...
            while (reLocateProdFinalIter.hasNext()){
                ...
                //3.3、先排实单->自带的搭配
                doProductAlignWithOnlineOneSku(contextDTO, mpProdFinalList, lockNextDay,mpFinalVo,newOnLineDay);
                reLocateProdFinalIter.remove();
            }
        }
    ...
    private void doStructureInWithOnlineInc(MpRollAdjustContextDTO contextDTO,
        ...
        List<FactoryMonthPlanFinalAdjustVo> reLocateProdFinalList = mpProdFinalList.stream().sorted(Comparator.comparing(FactoryMonthPlanFinalAdjustVo::getBeginDay,Comparator.nullsLast(Comparator.naturalOrder()))).collect(Collectors.toList());
        //2、续作优先排产；
        doStructureInWithContinueProduction(contextDTO,reLocateProdFinalList,mpAdjustStructInMap);
        //3、非续作排产
        for (int i=lockNextDay; i<contextDTO.getStructureDeadLine();i++) {
            //3.1、敲定SKU新的上机日期
            newOnLineDay = getNewOnLineDay(contextDTO, i, null);
     ...
     * @param mpProdFinalList 定额记录列表
     * @param mpAdjustStructInMap 结构内调整Map
     */
    private void doProductAlignWithContinueProduction(MpRollAdjustContextDTO contextDTO,List<FactoryMonthPlanFinalAdjustVo> mpProdFinalList,Map<String, FactoryMonthPlanFinalAdjustVo> mpAdjustStructInMap){
        if (PubUtil.isEmpty(mpProdFinalList)){
            return;
        }
        FactoryMonthPlanFinalAdjustVo adjustStructInVo;
        ...
        for (FactoryMonthPlanFinalAdjustVo mpFinalVo:mpProdFinalList){
            ...
            //单个续作SKU，使用调整量
            remainAdjustQty = incContinueProductionOneSku(contextDTO,mpProdFinalList,mpFinalVo,adjustStructInVo.getPendingQty());
            adjustStructInVo.setPendingQty(remainAdjustQty);
        }
    }
     ...
     * @param mpAdjustStructInMap 结构内调整Map
     */
    private void doStructureInWithContinueProduction(MpRollAdjustContextDTO contextDTO,List<FactoryMonthPlanFinalAdjustVo> mpProdFinalList,Map<String, MpAdjustStructureIn> mpAdjustStructInMap){
        if (PubUtil.isEmpty(mpProdFinalList)){
            return;
        }
        MpAdjustStructureIn adjustStructInVo;
     ...
     * @param contextDTO 上下文
     * @param mpProdFinalList 定额记录列表
     * @param mpAdjustStructOutMap 结构间调整Map
     */
    private void doStructureOutWithContinueProduction(MpRollAdjustContextDTO contextDTO,List<FactoryMonthPlanFinalAdjustVo> mpProdFinalList,Map<String, MpAdjustStructureOut> mpAdjustStructOutMap){
        if (PubUtil.isEmpty(mpProdFinalList)){
            return;
        }
     ...
     * @param newPlanQty 排产计划量
     */
    private synchronized Integer incContinueProductionOneSku(MpRollAdjustContextDTO contextDTO,List<FactoryMonthPlanFinalAdjustVo> mpProdFinalList,FactoryMonthPlanFinalAdjustVo mpFinalVo,int newPlanQty){

        Map<Integer, MpDailyCapacityLimitVo> dailyCapacityLimitVoMap = contextDTO.getDailyCapacityLimitVoMap();
        MpAdjustDailyCapacityLimit adjustDailyCapacityLimitObj = new MpAdjustDailyCapacityLimit();
        int dailyQty,intPart,remainPart,dayVulcanizationQty,diffQty,cavityQty,initMaxMachines,startMachines,curMachines,dayValue;
        String dayField;
        int lockNextDay = contextDTO.getLockEndDay() + 1;
        int otherTotalQty = getOtherTotalQtyForSpecMaterial(mpProdFinalList,mpFinalVo);
        int oriNewPlanQty = newPlanQty;
        //1.获取日硫化量
        dailyQty = getDayVulcanizationQty(mpFinalVo);
        for (int i=contextDTO.getStructureDeadLine();i>= contextDTO.getStructureStartDay(); i--){
            ...
            //2.初始最大机台数
            initMaxMachines = intPart + (remainPart > 0 ? 1:0);
            //3.续作-循环补量
            while (newPlanQty > 0){
                startMachines = 1;
                while (startMachines <= initMaxMachines){
                    //3.1 逐天补量，2模2模上
                    for (int m = i;m <= contextDTO.getStructureDeadLine();m++){
                        dayField = FactoryConstant.DAY_FIELD + m;
                        intPart = 0;
                        remainPart = 0;
                        if (hasPlanByDay(mpFinalVo,m)){
                            intPart = (Integer)mpFinalVo.getFieldValueByFieldName(dayField) / dailyQty;
                            remainPart = (Integer)mpFinalVo.getFieldValueByFieldName(dayField) % dailyQty;
                        }
                        ...
                        //3.1 计算产能限制
                        adjustDailyCapacityLimitObj.calcLhMachinesWithEmbryoTypes(mpProdFinalList,m, dailyCapacityLimitVoMap.get(m), contextDTO.getParamMap(), mpFinalVo.getMainPattern(),mpFinalVo.getEmbryoCode());
                        contextDTO.getLogDetail().append(String.format("结构:%s,【续作排产】,物料编码:%s,排产日:%s,其产能限制信息:%s！",contextDTO.getStructureName(),mpFinalVo.getMaterialCode(),m,dailyCapacityLimitVoMap.get(m).toString())).append(ApsConstant.DIVISION);
                        //3.2 检查：主花纹向下模具数量(/2转成机台数) 符合性
                        cavityQty = getNewCavityQty(contextDTO,mpFinalVo,m);
                        contextDTO.getLogDetail().append(String.format("结构:%s,【续作排产】,物料编码:%s,排产日:%s,获取到新的型腔数:%s！",contextDTO.getStructureName(),mpFinalVo.getMaterialCode(),m,cavityQty)).append(ApsConstant.DIVISION);
                        //3.2.1 预检查: 当前每日硫化机台数\当前每日胎胚种类数 符合性
                        if (!adjustDailyCapacityLimitObj.preCheckCapacitySatisfy(dailyCapacityLimitVoMap.get(m))){
                            contextDTO.getLogDetail().append(String.format("结构:%s,【续作排产】,物料编码:%s,排产日:%s,每日硫化机台数或每日胎胚种类数不符合产能限制,退出！",contextDTO.getStructureName(),mpFinalVo.getMaterialCode(),m)).append(ApsConstant.DIVISION);
                            break;
                        }
                        //3.2.1 预检查：主花纹向下模具数量(/2转成机台数) 符合性
                        if (!preCheckMouldSatisfy(dailyCapacityLimitVoMap.get(m),cavityQty)){
                            contextDTO.getLogDetail().append(String.format("结构:%s,【续作排产】,物料编码:%s,排产日:%s,主花纹:%s,其主花纹模具数不符合产能限制,退出！",contextDTO.getStructureName(),mpFinalVo.getMaterialCode(),m,mpFinalVo.getMainPattern())).append(ApsConstant.DIVISION);
                            break;
                        }
                        //3.3 检查总产能限制(允许上下波动)
                        if (!checkTotalCapacityLimit(contextDTO,m,mpFinalVo,dailyCapacityLimitVoMap.get(m),mpProdFinalList)){
                            break;
                        }

                        //3.4 计算计划量
                        dayVulcanizationQty = dailyQty - remainPart;

                        //检查是否自动补量
                        boolean isAutoReplenishment = checkAutoReplenishment(contextDTO.getParamMap(),m,mpFinalVo);
                        contextDTO.getLogDetail().append(String.format("结构:%s,【续作排产】,物料编码:%s,排产日:%s,自动补量标识:%s！",contextDTO.getStructureName(),mpFinalVo.getMaterialCode(),m,isAutoReplenishment ? "是":"否")).append(ApsConstant.DIVISION);
                        if (!isAutoReplenishment){
                            //3.4.1 若剩余计划量 < 日硫化量，则按剩余计划量累加
                            dayVulcanizationQty = newPlanQty < dayVulcanizationQty ? newPlanQty : dayVulcanizationQty;
                        }
                        //3.4.2 若是开产日，计划量全部等比例减少
                        dayVulcanizationQty = getPlanQtyForOpenProductionFirstDay(contextDTO,adjustDailyCapacityLimitObj,dailyCapacityLimitVoMap.get(m),dayVulcanizationQty);
                        //3.4.3 若是特殊结构，其调整计划量不能超过原月计划总量
                        ...
                        dayValue = dayValue + dayVulcanizationQty;
                ...
                if (startMachines > initMaxMachines){
                    //将剩余的计划量置回确认调整量
                    contextDTO.getLogDetail().append(String.format("结构:%s,【续作排产】,物料编码:%s,原调整量:%s,续作实际排产:%s,剩余调整量:%s！",contextDTO.getStructureName(),mpFinalVo.getMaterialCode(),oriNewPlanQty,oriNewPlanQty - newPlanQty,newPlanQty)).append(ApsConstant.DIVISION);
                    //退出while (newPlanQty > 0)
                    break;
                }
            }

            //4、重置一下搭配排产量标识
            if (isConvertionProd(mpFinalVo) && mpFinalVo.getConventionProductionQty() >0) {
                splitMatchQtyByDay(contextDTO,mpFinalVo.getConventionProductionQty(), lockNextDay,mpFinalVo);
            }else if (mpFinalVo.getCycleProductionQty() >0){
                splitMatchQtyByDay(contextDTO,mpFinalVo.getCycleProductionQty(), lockNextDay,mpFinalVo);
            }
            //5.重置开始日\结束日\汇总值
            resetBegin2EndDay2TotalQty(contextDTO.getStructureStartDay(),contextDTO.getStructureDeadLine(),mpFinalVo);
            //退出 for (int i=contextDTO.getStructureDeadLine();i>= contextDTO.getStructureStartDay(); i--)
            break;
        }
        return newPlanQty;
    }
     ...
     * @return
     */
    private void doProductAlignWithOnlineOneSku(MpRollAdjustContextDTO contextDTO, List<FactoryMonthPlanFinalAdjustVo> mpProdFinalList, int lockNextDay,
                                                FactoryMonthPlanFinalAdjustVo mpFinalVo, int newOnLineDay) {
        if (mpFinalVo == null){
            return;
        }
        ...
        Integer newPlanQty = getNewPlanQty(contextDTO,mpFinalVo, lockNextDay);
        ...
        Integer curServeProductionQty = isConvertionProd(mpFinalVo) ? mpFinalVo.getConventionProductionQty() : mpFinalVo.getCycleProductionQty();
        int remainPlanQtyByMatchOtherSku = 0;
        if (remainPlanQty > curServeProductionQty){
            // 若剩余量 > 搭配量，说明实单还有剩余
            // 去掉搭配量，将实单量排产
            int newRemainPlanQty = newPlanQty - curServeProductionQty;
            // 本身搭配被挤掉，置0
            if (isConvertionProd(mpFinalVo)){
            ...
            remainPlanQtyByMatchOtherSku = deductMatchOtherSku(contextDTO, lockNextDay,newEndDay,newRemainPlanQty,mpFinalVo, mpProdFinalList);
            ...
            addAdjustProcLog(contextDTO,mpFinalVo,String.format(I18nUtil.getMessage("alg.data.mp.weekRollAdjust.log.online.realProduction"),mpFinalVo.getMaterialCode(),newOnLineDay,remainPlanQty,curServeProductionQty,newRemainPlanQty,oriTotalQty - mpFinalVo.getTotalQty()));
            //若实单还有剩余，进行延后排产 sandy+ 2026.08.19
            if (remainPlanQtyByMatchOtherSku > 0){
                //重置开始日和结束日
                resetBegin2EndDay2TotalQty(contextDTO.getStructureStartDay(),contextDTO.getStructureDeadLine(),mpFinalVo);
                int remainPlanQtyDelayIncMould;
                for (int iDay = mpFinalVo.getEndDay(); iDay <=contextDTO.getStructureDeadLine(); iDay++){
                    FactoryMonthPlanFinalAdjustVo bakMpFinalVo2 = new FactoryMonthPlanFinalAdjustVo();
                    BeanUtils.copyProperties(mpFinalVo,bakMpFinalVo2);
                    //清空定稿表日计划量
                    clearMpFinalDayValue(contextDTO, mpFinalVo.getBeginDay(), mpFinalVo);
                    //延后增模排产
                    remainPlanQtyDelayIncMould = delayIncMouldProductionByAlign(mpProdFinalList,contextDTO,mpFinalVo.getBeginDay(),iDay,mpFinalVo.getEndDay(),newRemainPlanQty,mpFinalVo,bakMpFinalVo2);
                    if (remainPlanQtyDelayIncMould <=0){
                        break;
                    }
                }
            }
            ...
            addAdjustProcLog(contextDTO,mpFinalVo,String.format(I18nUtil.getMessage("alg.data.mp.weekRollAdjust.log.online.realProduction2"),mpFinalVo.getMaterialCode(),newOnLineDay,newPlanQty,newPlanQty - remainPlanQty,remainPlanQty,curServeProductionQty,remainPlanQty));
            if (curServeProductionQty >= remainPlanQty){
                if (isConvertionProd(mpFinalVo)){
                    mpFinalVo.setConventionProductionQty(remainPlanQty);
                }else{
                    mpFinalVo.setCycleProductionQty(remainPlanQty);
                }
            }
        }
     ...
     * @return
     */
    private void doProductAlignWithOnlineOneSkuByPostpone(MpRollAdjustContextDTO contextDTO, List<FactoryMonthPlanFinalAdjustVo> mpProdFinalList, int lockNextDay,
                                                FactoryMonthPlanFinalAdjustVo mpFinalVo, int newOnLineDay) {
...
*/
    ...
    public boolean checkTotalCapacityLimit(MpRollAdjustContextDTO contextDTO,Integer checkDay,FactoryMonthPlanFinalAdjustVo mpFinalVo,MpDailyCapacityLimitVo limitVo,List<FactoryMonthPlanFinalAdjustVo> mpProdFinalList){
        ...
        contextDTO.getLogDetail().append(String.format("结构:%s,【续作/增模排产】,物料编码:%s,排产日:%s,日最大排产量:%s,日剩余排产量:%s,已排产总计划量:%s,比例:%s,%s！",contextDTO.getStructureName(),mpFinalVo.getMaterialCode(),checkDay,limitVo.getMaxDayProductionQty(),limitVo.getRemainMaxDayProductionQty(),dayTotalCapacityChecker.getTotalPlanQty(),limitVo.getDayProductionRate(),hint)).append(ApsConstant.DIVISION);
        if (bCheck && YesOrNoEnum.YES.getCode().equals(mpFinalVo.getOemFlag())){
            ...
            hint = bCheck ? "满足":"不满足,退出！";
            contextDTO.getLogDetail().append(String.format("结构:%s,【续作/增模排产】,物料编码:%s,排产日:%s,剩余OEM排产量:%s,已排产OEM计划量:%s,%s！",contextDTO.getStructureName(),mpFinalVo.getMaterialCode(),checkDay,limitVo.getRemainOemQty(),oemTotalCapacityChecker.getTotalPlanQty(),hint)).append(ApsConstant.DIVISION);
        }
        return bCheck;
    }
    ...
    private void doStructureOutWithOnlineInc(MpRollAdjustContextDTO contextDTO,
        ...
        List<FactoryMonthPlanFinalAdjustVo> reLocateProdFinalList = mpProdFinalList.stream().sorted(Comparator.comparing(FactoryMonthPlanFinalAdjustVo::getBeginDay,Comparator.nullsLast(Comparator.naturalOrder()))).collect(Collectors.toList());
        //2、续作优先排产；
        doStructureOutWithContinueProduction(contextDTO,reLocateProdFinalList,mpAdjustStructOutMap);
        //3、非续作排产
        for (int i=lockNextDay; i<contextDTO.getStructureDeadLine();i++) {
            //2.1、敲定SKU新的上机日期
     ...
     * @param mpProdFinalList 定稿列表
     */
    private Integer deductMatchOtherSku(MpRollAdjustContextDTO contextDTO,int lockNextDay,int endDay,int planQty,FactoryMonthPlanFinalAdjustVo curFinalVo,
                                List<FactoryMonthPlanFinalAdjustVo> mpProdFinalList){
        if (PubUtil.isEmpty(mpProdFinalList)){
            return 0;
        }
        ...
        adjustDailyCapacityLimitObj.calcLhMachinesWithEmbryoTypes(mpProdFinalList,endDay, dailyCapacityLimitVo, contextDTO.getParamMap(), curFinalVo.getMainPattern(),curFinalVo.getEmbryoCode());
        ...
        FactoryMonthPlanFinalAdjustVo optimalFinalVo,bakOptimalFinalVo,bakMpFinalVo;
        int remainPlanQty,deductMatchQty;
        Integer curServeProductionQty;
        while (newOtherFinalList.size() >0 ){
            // 2.从多个SKU中，匹配其他最优的定稿SKU记录
            optimalFinalVo = getOptimalOtherSku(curFinalVo, newOtherFinalList);
            curServeProductionQty = isConvertionProd(optimalFinalVo) ? optimalFinalVo.getConventionProductionQty() : optimalFinalVo.getCycleProductionQty();
            // 3.清空搭配日计划 及扣减搭配总量
            bakMpFinalVo = new FactoryMonthPlanFinalAdjustVo();
            BeanUtils.copyProperties(curFinalVo,bakMpFinalVo);
            bakOptimalFinalVo = new FactoryMonthPlanFinalAdjustVo();
            BeanUtils.copyProperties(optimalFinalVo,bakOptimalFinalVo);

            clearMpFinalDayValue(contextDTO,endDay,curFinalVo);
            int clearDayValue = clearMpFinalDayValue(contextDTO,endDay,optimalFinalVo);
            if (curServeProductionQty >= clearDayValue){
                ...
                if (isConvertionProd(optimalFinalVo)){
                    ...
                    optimalFinalVo.setCycleProductionQty(optimalFinalVo.getCycleProductionQty() - clearDayValue);
                }
            }else {
                //若搭配量比清空的值小，即不能覆盖，那减少的搭配量 = 搭配量
                deductMatchQty = curServeProductionQty;
                //恢复多清的实单量(从备份中获取原始)
                ...
                if (isConvertionProd(optimalFinalVo)){
                    ...
                    optimalFinalVo.setCycleProductionQty(0);
                }
            }
            //重置开始日\结束日\汇总值
            resetBegin2EndDay2TotalQty(contextDTO.getStructureStartDay(),contextDTO.getStructureDeadLine(),optimalFinalVo);
            contextDTO.getLogDetail().append(String.format("结构:%s,物料编码:%s,【扣减其他SKU的搭配】,排产%s日,已匹配到最优的有搭配量的物料编码:%s,总搭配量:%s,减少搭配量:%s！",contextDTO.getStructureName(),curFinalVo.getMaterialCode(),endDay,optimalFinalVo.getMaterialCode(),curServeProductionQty,deductMatchQty)).append(ApsConstant.DIVISION);
            // 4.增模模拟排产
        ...
        remainPlanQty = deductMatchOtherSku(contextDTO,lockNextDay,endDay-1,planQty,curFinalVo,mpProdFinalList);
    ...
    private List<FactoryMonthPlanFinalAdjustVo> getMatchFinalListByDay(int endDay, FactoryMonthPlanFinalAdjustVo curFinalVo,
        ...
        for (FactoryMonthPlanFinalAdjustVo mpFinalVo: mpProdFinalList){
            if (isConvertionProd(mpFinalVo)){
                ...
                if (mpFinalVo.getCycleProductionQty() == null ||
                        mpFinalVo.getCycleProductionQty() <= 0){
                    continue;
                }
            }
    ...
    private FactoryMonthPlanFinalAdjustVo getOptimalOtherSku(FactoryMonthPlanFinalAdjustVo curFinalVo, List<FactoryMonthPlanFinalAdjustVo> newOtherFinalList) {
        ...
        FactoryMonthPlanFinalAdjustVo firstMpFinalVo = newOtherFinalList.get(0);
        int minMatchQty = isConvertionProd(firstMpFinalVo) ? firstMpFinalVo.getConventionProductionQty():firstMpFinalVo.getCycleProductionQty();
        for (FactoryMonthPlanFinalAdjustVo tFinalVo: newOtherFinalList){
            ...
            Integer curServeProductionQty = isConvertionProd(tFinalVo) ? tFinalVo.getConventionProductionQty():tFinalVo.getCycleProductionQty();
            if (minMatchQty >= curServeProductionQty ){
                minMatchQtyVo = tFinalVo;
                minMatchQty = curServeProductionQty;
            }
        }
    ...
    private synchronized int incMouldProduction(List<FactoryMonthPlanFinalAdjustVo> mpProdFinalList,
        ...
        Integer shortestProductionDays = (Integer)contextDTO.getParamMap().get(MonthPlanEnums.SKU_SHORTEST_PRODUCTION_DAYS.getCode());
        int maxDays = YearMonth.of(contextDTO.getMpYear(), contextDTO.getMpMonth()).lengthOfMonth();
        Map<Integer, MpDailyCapacityLimitVo> dailyCapacityLimitVoMap = contextDTO.getDailyCapacityLimitVoMap();
        int structureDeadLine = contextDTO.getStructureDeadLine();
        MpAdjustDailyCapacityLimit adjustDailyCapacityLimitObj = new MpAdjustDailyCapacityLimit();
        int otherTotalQty = getOtherTotalQtyForSpecMaterial(mpProdFinalList,mpFinalVo);
        ...
        //非首次上模，最短排产量
        //Integer shortestProductionQty = getShortestProductionQty(shortestProductionDays,dailyQty);
        IncMouldContext incMouldContext = new IncMouldContext();
        incMouldContext.setBFirstAddMould(true);
        incMouldContext.setHasProductionQty(getProductionQty(newOnLineDay,structureDeadLine,bakMpFinalVo));
        //量试标识
        ...
        while (newPlanQty > 0){
            ...
            //按硫化机维度，记录已排产天数及排产量
            incMouldContext.setUsedProductionMap(new HashMap<>());
            ...
            for (int i = newOnLineDay; i<= structureDeadLine; i++){
                ...
                //检查总产能限制(允许上下波动)
                if (!checkTotalCapacityLimit(contextDTO,i,mpFinalVo,dailyCapacityLimitVoMap.get(i),mpProdFinalList)){
                    if (getBeforeProductionQty(incMouldContext) > 0 && YesOrNoEnum.YES.getCode().equals(dailyCapacityLimitVoMap.get(i).getDayOpenCloseFlag())){
                        newPlanQty += revertPreDayQty(contextDTO,mpFinalVo,i,incMouldContext);
                        break;
                    }
                    newPlanQty += revertPreDayQty(contextDTO,mpFinalVo,i,incMouldContext);
                ...
                if (i == newOnLineDay){
                    ...
                    if (isIncMouldFirstDay(mpFinalVo,newOnLineDay-1,dayValue,dailyQty)){
                        ...
                        dayVulcanizationQty = adjustDailyCapacityLimitObj.getFirstDayQty(mpProdFinalList,newOnLineDay, dailyCapacityLimitVoMap.get(newOnLineDay), contextDTO.getParamMap(), mpFinalVo.getMainPattern(),mpFinalVo.getEmbryoCode());

                    }
                }else{
                    if (incMouldContext.isBFirstAddMould()){
                        if (i == structureDeadLine){
                            //若是结构收尾日且是增模，因只会增1天，退出
                            continue;
                        }
                        dayVulcanizationQty = adjustDailyCapacityLimitObj.getFirstDayQty(mpProdFinalList,i, dailyCapacityLimitVoMap.get(newOnLineDay), contextDTO.getParamMap(), mpFinalVo.getMainPattern(),mpFinalVo.getEmbryoCode());
                    }
                }
                ...
                //检查: 当前每日硫化机台数\当前每日胎胚种类数\每日换模次数 符合性
                ...
                if (mpFinalVo.getFieldValueByFieldName(dayField) !=null && (Integer) mpFinalVo.getFieldValueByFieldName(dayField) !=0){
                    ...
                    incMouldContext.getUsedProductionMap().put(i,dayVulcanizationQty);
                }
            }

            if (incMouldContext.getUsedProductionMap().size() == 0){
            ...
            //不是临近月低 incMouldContext.getUsedProductionMap().get(maxDays) == null
            if (startMould/2 >= 2 && incMouldContext.getUsedProductionMap().size() < shortestProductionDays &&
                    incMouldContext.getUsedProductionMap().get(maxDays) == null){
                contextDTO.getLogDetail().append(String.format("结构:%s,【增模排产】,物料编码:%s,非首次增模且非临近月底,尝试排产天数:%s,最短排产天数:%s,还原尝试排产量:%s,退出！",contextDTO.getStructureName(),mpFinalVo.getMaterialCode(),incMouldContext.getUsedProductionMap().size(),shortestProductionDays,incMouldContext.getUsedProductionMap())).append(ApsConstant.DIVISION);
                //还原已排产量
                resetUsedProductionQty(mpFinalVo, incMouldContext);
                newPlanQty = 0;
                return resetPlanQty(contextDTO,newOnLineDay,structureDeadLine,oriNewPlanQty,newPlanQty,incMouldContext.getHasProductionQty(),mpFinalVo,bakMpFinalVo);
            }
            ...
            /*if (newPlanQty < minProdQty){
                //若剩余的计划量 < 最小投产量 或 <= 最短排产量，忽略
                contextDTO.getLogDetail().append(String.format("结构:%s,【增模排产】,物料编码:%s,剩余排产量:%s,最小投产量:%s,最短排产量:%s,在新起模具时,剩余排产量低于最小投产量或最短排产量,退出！",contextDTO.getStructureName(),mpFinalVo.getMaterialCode(),newPlanQty,minProdQty,shortestProductionQty+dailyQty)).append(ApsConstant.DIVISION);
                newPlanQty = 0;
                return resetPlanQty(contextDTO,newOnLineDay,structureDeadLine,oriNewPlanQty,newPlanQty,incMouldContext.getHasProductionQty(),mpFinalVo,bakMpFinalVo);
            }*/
            incMouldContext.setBFirstAddMould(true);
     ...
     * @param contextDTO 周程滚动上下文
     * @param newOnLineDay 新的上机日期
     * @param newEndDay 新的截止日
     * @param newPlanQty 新的计划量
     * @param mpFinalVo 当前定稿记录
     */
    private synchronized int delayIncMouldProductionByAlign(List<FactoryMonthPlanFinalAdjustVo> mpProdFinalList,
                                                            MpRollAdjustContextDTO contextDTO,
                                                            Integer newOnLineDay, Integer newEndDay, Integer oriEndDay, Integer newPlanQty, FactoryMonthPlanFinalAdjustVo mpFinalVo, FactoryMonthPlanFinalAdjustVo bakMpFinalVo) {
        String dayField;
        int dayValue;
        ...
        IncMouldContext incMouldContext = new IncMouldContext();
        incMouldContext.setBFirstAddMould(true);
        incMouldContext.setHasProductionQty(getProductionQty(newOnLineDay,newEndDay,bakMpFinalVo));
        while (newPlanQty > 0){
            //已有排产标识，防止中间断开
            //按硫化机维度，记录前日排产位置
            incMouldContext.setBeforeProductionPosition(0);
            //按硫化机维度，记录已排产天数及排产量
            incMouldContext.setUsedProductionMap(new HashMap<>());
            contextDTO.getLogDetail().append(String.format("结构:%s,【延后增模排产】,物料编码:%s,尝试增模具数:%s！",contextDTO.getStructureName(),mpFinalVo.getMaterialCode(),startMould)).append(ApsConstant.DIVISION);
            for (int i = newOnLineDay; i<= newEndDay; i++){
                //SKU的模具数限制：SKU的模具数<=SKU活块的数量
                blockQty = getNewTypeBlockQty(contextDTO,mpFinalVo,i);
                contextDTO.getLogDetail().append(String.format("结构:%s,【延后增模排产】,物料编码:%s,排产日:%s,获取到新的活块数:%s！",contextDTO.getStructureName(),mpFinalVo.getMaterialCode(),i,blockQty)).append(ApsConstant.DIVISION);
                if (startMould > blockQty){
                    contextDTO.getLogDetail().append(String.format("结构:%s,【延后增模排产】,物料编码:%s,排产日:%s,SKU增模后的模具数:%s 大于SKU活块的数量:%s！",contextDTO.getStructureName(),mpFinalVo.getMaterialCode(),i,startMould,blockQty)).append(ApsConstant.DIVISION);
                ...
                if (!checkSecOnline(mpFinalVo,newOnLineDay, i, contextDTO.getParamMap()) &&
                        !hasPlanByDay(mpFinalVo, getPreDayCloseDay(contextDTO.getDailyCapacityLimitVoMap(),i -1))){
                    contextDTO.getLogDetail().append(String.format("结构:%s,【延后增模排产】,物料编码:%s,排产日:%s,不符二次上机条件,退出！", contextDTO.getStructureName(),mpFinalVo.getMaterialCode(),i)).append(ApsConstant.DIVISION);
                    newPlanQty += revertPreDayQty(contextDTO,mpFinalVo,i,incMouldContext);
                    continue;
                }
                //注：这里不能做预检查，会导致可以拼的SKU被忽略掉
                ...
                if(dayVulcanizationQty == null){
                    //若获取到首日量是空值,表示主花纹向下，当日不让增模或换活字块
                    contextDTO.getLogDetail().append(String.format("结构:%s,【延后增模排产】,物料编码:%s,排产日:%s,获取到的首日计划量为空,表示当前排产日主花纹下不允许换活块！",contextDTO.getStructureName(),mpFinalVo.getMaterialCode(),i)).append(ApsConstant.DIVISION);
                    if (incMouldContext.getUsedProductionMap().size() >1 && getBeforeProductionQty(incMouldContext) > 0 && YesOrNoEnum.YES.getCode().equals(dailyCapacityLimitVoMap.get(i).getDayOpenCloseFlag())){
                        break;
                    }
                    newPlanQty += revertPreDayQty(contextDTO,mpFinalVo,i,incMouldContext);
                ...
                //检查是否自动补量
                boolean isAutoReplenishment = checkAutoReplenishment(contextDTO.getParamMap(),i,mpFinalVo);
                contextDTO.getLogDetail().append(String.format("结构:%s,【延后增模排产】,物料编码:%s,排产日:%s,自动补量标识:%s！",contextDTO.getStructureName(),mpFinalVo.getMaterialCode(),i,isAutoReplenishment ? "是":"否")).append(ApsConstant.DIVISION);
                if (!isAutoReplenishment){
                    //若剩余计划量 < 日硫化量，则按剩余计划量累加
                    dayVulcanizationQty = newPlanQty < dayVulcanizationQty ? newPlanQty : dayVulcanizationQty;
                }
            ...
            if (incMouldContext.getUsedProductionMap().size() == 0){
                contextDTO.getLogDetail().append(String.format("结构:%s,【延后增模排产】,物料编码:%s,第:%s台硫化机,没有排产量,退出！",contextDTO.getStructureName(),mpFinalVo.getMaterialCode(),startMould/2)).append(ApsConstant.DIVISION);
                return resetPlanQty(contextDTO,newOnLineDay,newEndDay,oriNewPlanQty,newPlanQty,incMouldContext.getHasProductionQty(),mpFinalVo,bakMpFinalVo);
            }
            ...
            if (startMould/2 >= 2 && incMouldContext.getUsedProductionMap().size() < shortestProductionDays &&
                    incMouldContext.getUsedProductionMap().get(maxDays) == null){
                contextDTO.getLogDetail().append(String.format("结构:%s,【延后增模排产】,物料编码:%s,非首次增模且非临近月底,尝试排产天数:%s,最短排产天数:%s,还原尝试排产量:%s,退出！",contextDTO.getStructureName(),mpFinalVo.getMaterialCode(),incMouldContext.getUsedProductionMap().size(),shortestProductionDays,incMouldContext.getUsedProductionMap())).append(ApsConstant.DIVISION);
                //还原已排产量
                resetUsedProductionQty(mpFinalVo, incMouldContext);
                newPlanQty = 0;
                return resetPlanQty(contextDTO,newOnLineDay,newEndDay,oriNewPlanQty,newPlanQty,incMouldContext.getHasProductionQty(),mpFinalVo,bakMpFinalVo);
     ...
     * 还原已排产量
     * @param mpFinalVo
     * @param incMouldContext
     */
    private static void resetUsedProductionQty(FactoryMonthPlanFinalAdjustVo mpFinalVo, IncMouldContext incMouldContext) {
        HashMap<Integer,Integer> usedProductionMap = incMouldContext.getUsedProductionMap();
        if (PubUtil.isEmpty(usedProductionMap)){
            return;
        }
        for (Map.Entry<Integer, Integer> entry : incMouldContext.getUsedProductionMap().entrySet()) {
            String preDayField = FactoryConstant.DAY_FIELD + entry.getKey();
            int preDayValue = (Integer) mpFinalVo.getFieldValueByFieldName(preDayField) - entry.getValue();
     ...
     * @return
     */
    private Integer getBeforeProductionQty(IncMouldContext incMouldContext){
        Integer beforeProductionQty = incMouldContext.getUsedProductionMap().get(incMouldContext.getBeforeProductionPosition());
        return beforeProductionQty == null ? 0 : beforeProductionQty;
    }
    /**
     * 获取最少排产量
     * @param shortestProductionDays
     * @param dailyQty
     * @return
     */
    private Integer getShortestProductionQty(Integer shortestProductionDays,Integer dailyQty){
        if (shortestProductionDays == null){
            return 0;
        }
        shortestProductionDays -= 1;
        if (shortestProductionDays > 0){
            return shortestProductionDays * dailyQty;
        }
        return 0;
    }
    /**
     * 返回计划量，若排的计划量比原有的计划量还小，则还原其原来的计划量
     * @param newOnLineDay 新的上机日
     * @param structureDeadLine 结构收尾日
     * @param oriNewPlanQty 首次计划量
     * @param newPlanQty 递减的计划量
     * @param hasProductionQty 原已排计划量
     * @param mpFinalVo 定稿记录Vo
     * @param bakMpFinalVo 备份定稿记录Vo
     * @return 计划量
     */
    private int resetPlanQty(MpRollAdjustContextDTO contextDTO, int newOnLineDay, int structureDeadLine, int oriNewPlanQty, int newPlanQty, int hasProductionQty,
                             FactoryMonthPlanFinalAdjustVo mpFinalVo, FactoryMonthPlanFinalAdjustVo bakMpFinalVo){
        int realProductionQty = getProductionQty(newOnLineDay,structureDeadLine,mpFinalVo);
        if (realProductionQty < hasProductionQty && bakMpFinalVo != null){
            BeanUtils.copyProperties(bakMpFinalVo,mpFinalVo);
            contextDTO.getLogDetail().append(String.format("结构:%s,【增模排产】,物料编码:%s,模拟排产量比原计划量小,恢复原计划量！",contextDTO.getStructureName(),mpFinalVo.getMaterialCode())).append(ApsConstant.DIVISION);
            return oriNewPlanQty;
        }
        return newPlanQty < 0 ? 0:newPlanQty;
    }
    /**
    ...
    private int revertPreDayQty(MpRollAdjustContextDTO contextDTO,FactoryMonthPlanFinalAdjustVo mpFinalVo, int currentDay, IncMouldContext incMouldContext){
        int beforeProductionQty = getBeforeProductionQty(incMouldContext);
        if (incMouldContext.getUsedProductionMap().size() == 1 && beforeProductionQty > 0 &&
            ...
            mpFinalVo.setFieldValueByFieldName(preDayField,preDayValue==0 ? null : preDayValue);
            contextDTO.getLogDetail().append(String.format("结构:%s,【增模排产】,物料编码:%s,排产日:%s,单机台已排产天数: %s, 还原值:%s, 前日还原后的排产量: %s！", contextDTO.getStructureName(),mpFinalVo.getMaterialCode(),currentDay,incMouldContext.getUsedProductionMap().size(),beforeProductionQty,preDayValue)).append(ApsConstant.DIVISION);
            incMouldContext.setBeforeProductionPosition(0);
            incMouldContext.setUsedProductionMap(new HashMap<>());
            incMouldContext.setBFirstAddMould(true);
    ...
    private int getDiffQtyForSpecMaterial(MpRollAdjustContextDTO contextDTO, FactoryMonthPlanFinalAdjustVo mpFinalVo,
        ...
        contextDTO.getLogDetail().append(String.format("结构:%s,【续作/增模排产】,物料编码:%s,排产日:%s,特殊标识：是,特殊结构原总计划量:%s,其他SKU总计划量:%s,当前SKU已排计划量:%s,允许再排计划量:%s！",contextDTO.getStructureName(),mpFinalVo.getMaterialCode(),iDay,contextDTO.getSpecStructureTotalQty(),otherTotalQty,curTotalQty,diffQty)).append(ApsConstant.DIVISION);
        return diffQty;
    }
    ...
    public int getMouldByDay(MpAdjustDailyCapacityLimit adjustDailyCapacityLimitObj,Map<String,Object> paramMap,Integer iDay, FactoryMonthPlanFinalAdjustVo mpFinalVo,MpDailyCapacityLimitVo dailyCapacityLimitVo){
        ...
        if (adjustDailyCapacityLimitObj.isDecMould(mpFinalVo,dayField,day1Field,day2Field,iDay)){
            ...
            //增模
            int[]addMouldArr = adjustDailyCapacityLimitObj.getAddMouldMachines(mpFinalVo,dailyLhQty,paramMap,dayField,day1Field,day2Field,null);
    ...
    /**
     * 生产对齐 - 获取的排产计划量（实单量+自带的搭配量）
     * @param contextDTO 周程滚动上下文
     * @param mpFinalVo 定稿Vo
     * @return 新的排产计划量
     */
    private int getNewPlanQty(MpRollAdjustContextDTO contextDTO,FactoryMonthPlanFinalAdjustVo mpFinalVo,int lockNextDay){
        String dayField,matchDayField;
        // 锁定日之后的实单每日排产量;
        int iRealQty = 0;
        for (int i = lockNextDay; i<=contextDTO.getStructureDeadLine();i++){
        ...
        //实单：待调整量+ 锁定日之后的每日排产量
        iRealQty +=  mpFinalVo.getPendingQty();
        //实单+搭配量
        return  iRealQty + (isConvertionProd(mpFinalVo) ? mpFinalVo.getConventionProductionQty():mpFinalVo.getCycleProductionQty());
    }
    ...
    private void doStructureInWithNewSku(MpRollAdjustContextDTO contextDTO,
        ...
        // 重算在机SKU的产能信息
        reCalcAdjustDailyCapacityLimit(contextDTO,mpProdFinalList,new MpAdjustDailyCapacityLimit());
     ...
     * @param contextDTO 周程滚动上下文
     * @param mpProdFinalList 定稿列表
     * @param adjustStructOutVo 结构调整记录
     * @param lockNextDay 锁定次日
     * @param newOnLineDay 新的上机日
     */
    private boolean doStructureOutWithNewSkuForOne(MpRollAdjustContextDTO contextDTO, List<FactoryMonthPlanFinalAdjustVo> mpProdFinalList, MpAdjustStructureOut adjustStructOutVo, int lockNextDay, int newOnLineDay) {
        FactoryMonthPlanFinalAdjustVo mpFinalVo = createMpFinalAdjustVo(contextDTO, adjustStructOutVo);
        //2.2、将新增的SKU纳入定稿列表(因在模拟排产时需要实时判断模数，后面没有排上，再移除)
        mpProdFinalList.add(mpFinalVo);
        contextDTO.getFactoryMonthPlanProdFinalList().add(mpFinalVo);

        //2.3、计算新需要排产的计划量 = 实单量，其中，实单量：待调整量
        Integer newPlanQty = adjustStructOutVo.getConfirmAdjustQty();
    ...
    private FactoryMonthPlanFinalAdjustVo createMpFinalAdjustVo(MpRollAdjustContextDTO contextDTO, MpAdjustStructureIn adjustStructInVo) {
        ...
        mpFinalVo.setEmbryoCode(adjustStructInVo.getEmbryoCode());
    ...
    private FactoryMonthPlanFinalAdjustVo createMpFinalAdjustVo(MpRollAdjustContextDTO contextDTO, MpAdjustStructureOut adjustStructOutVo) {
        ...
        mpFinalVo.setEmbryoCode(adjustStructOutVo.getEmbryoCode());
