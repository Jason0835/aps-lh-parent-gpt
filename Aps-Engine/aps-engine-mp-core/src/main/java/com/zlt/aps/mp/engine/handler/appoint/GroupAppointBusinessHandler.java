...
public class GroupAppointBusinessHandler {
     ...
     */
    public void getContinueSkuAdjustInfo(Context context, List<CxMachineAllocationPlanHelper> continueAllocationList) {
     ...
     */
    private void calculateContinueSkuForceConclusion(Context context, AppointGroupReduceMachineInfo appointOfflineInfo) {
        ...
        if(originContinueCxMachineInfo.size() == BigDecimal.ONE.intValue()){
            ...
            if(!onlyAllocationInfo.getCxMachineCode().equals(onlyCxMachine.getCxMachineInfo().getCxMachineCode())){
                return;
            }
            onlyCxMachine.setRealEarliestChangeGroupDay(context);
            Integer newEndDay = onlyCxMachine.getRealEarliestChangeGroupDay();
        ...
        forceConclusionList.sort(Comparator.comparing(OnlineGroupAppointCxMachineInfo::getRealEarliestChangeGroupDay));
    }
