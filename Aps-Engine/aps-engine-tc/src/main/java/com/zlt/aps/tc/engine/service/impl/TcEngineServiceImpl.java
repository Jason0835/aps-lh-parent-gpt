...
import com.zlt.aps.tc.engine.vo.TcTotalPlanQtyVo;
...
public class TcEngineServiceImpl implements TcEngineService {
    ...
    public void autoTcSchedule(String scheduleDate) {
        ...
        for (TcScheduleResultVo scheduleVo : scheduleList) {
            ...
            scheduleVo.setStockQty(planStockMap.getOrDefault(scheduleVo.getSidewallCode(), 0D));  //16点预计库存
            ...
            this.computeTcPlanQty(scheduleVo, totalPlanQtyMap, lossRateMap, getDouble(paramsMap.get(EngineConstants.LOSS_RATE)));  //计算胎侧中班和夜班计划量
...
//    }
     ...
     */
    private void equalShare(String batchNo, List<TcScheduleResultVo> scheduleList, String equalShareThreshold) {
        ...
        autoScheduleLogService.insertTcScheduleLog(batchNo, "", "单规格排产数量达到设定值时，中夜班数量对半分", logSplit("班计划量均分阈值:" + equalShareThreshold,
    ...
    private void newComputeSupplyTime(TcScheduleResultVo scheduleVo,  Double stockQty) {
        ...
        autoScheduleLogService.insertTcScheduleLog(scheduleVo.getBatchNo(), scheduleVo.getOrderNo(), "计算库存供应时长前数据",
                logSplit("具体算法：从1班开始判断，预计库存-1班的计划大于等于0时，供应时长+8小时；预计库存-1班计划-2班计划大于等于0时，供应时长+16小时；预计库存-1班计划-2班计划-3班计划小于0，供应时长=16个小时+（((预计库存-1班计划-2班计划)/3班计划)*8）；以此类推到第5班",
                        "物料编号：" + scheduleVo.getSidewallCode() + "，16点预计库存：" + stockQty + "，对应成型一班的计划量：" + cxClass1Plan + "，对应成型二班的计划量：" + cxClass2Plan + "，对应成型三班的计划量：" + cxClass3Plan + "，对应成型次日一班的计划量：" + cxClass4Plan + "，对应成型次日二班的计划量：" + cxClass5Plan));
     ...
     */
    private void computeTcPlanQty(TcScheduleResultVo scheduleVo, Map<String, TcTotalPlanQtyVo> totalPlanQtyMap, Map<String, Double> lossMap, double paramLossRate) {
...
//        double unitConsume = BigDecimalUtil.div(scheduleVo.getUnitConsume(), 1000D, 3);
        ...
        if(StringUtils.isNotBlank(machineId) && !machineId.contains(",")) {
            //计划量要加上耗损量
            lossRate = tcEngineLossService.getLossRate(scheduleVo.getSidewallCode(), scheduleVo.getMachineId(), lossMap, paramLossRate);
            dayPlanQty = BigDecimalUtil.add(dayPlanQty, BigDecimalUtil.mul(dayPlanQty, lossRate));
            nightPlanQty = BigDecimalUtil.add(nightPlanQty, BigDecimalUtil.mul(nightPlanQty, lossRate));
        }
