...
public class TcEngineStockServiceImpl implements TcEngineStockService {
    ...
    public Map<String, Double> getPlanStockMap(String batchNo, String scheduleDate, Double stockLossRate) {
        ...
        autoScheduleLogService.insertTcScheduleLog(batchNo, "", "①16点预计库存 = 11点半部件库存 - 11点到16点半部件消耗", "11点半部件库存量：" + toJSONString(list));
        if (list.isEmpty()) {
            log.error("胎侧库存查询为空");
            return stockMap;
        }

        //开始计算胎侧16点预计库存 = 11点半部件库存 - 11点到16点半部件消耗
        Map<String, Double> consumMap = this.getTcConsume(batchNo, scheduleDate); // 11点到16点半部件计划消耗量
        ...
        autoScheduleLogService.insertTcScheduleLog(batchNo, "", "④16点预计库存 = 11点半部件库存 - 11点到16点半部件消耗",
    ...
    private Map<String, Double> getTcConsume(String batchNo, String scheduleDate) {
        ...
        List<TcStockConsumeVo> list = tcEngineStockMapper.listCxPlanAndConsume(scheduleDate);  //查询胎侧胶对应的成型三班计划消耗量，以及8-12点实际消耗量
        autoScheduleLogService.insertTcScheduleLog(batchNo, "", "②16点预计库存 = 11点半部件库存 - 11点到16点半部件消耗",
                "半制品对应成型(8点-16点)计划量的消耗量cxClass3PlanConsume，以及制品对应成型(8点-12点)完成量的消耗量cxFinishConsume：" + toJSONString(list));  //添加日志
        if (list.isEmpty()) {
            log.error("计算12点到16点胎侧计划消耗量为空");
            return result;
        }
        //开始计算11点到16点半部件消耗量
