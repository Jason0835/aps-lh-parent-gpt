...
@Slf4j
@Service
public class DjEngineStockServiceImpl implements DjEngineStockService {
    ...
    public Map<String, Double> getPlanStockMap(String batchNo, String scheduleDate, Double stockLossRate) {
        ...
        List<DjStockVo> list = djEngineStockMapper.listDjStock(scheduleDate);  //询指定日期的垫胶库存量
        ...
        //开始计算垫胶预计库存 = 7点半部件库存 + 昨日半制品早班计划量 - 今天成型早班(成型一班)
        Map<String, Double> consumMap = this.getNcConsume(batchNo, scheduleDate); // 今天成型早班(成型一班)-昨日半制品早班计划量
        for (DjStockVo stockVo : list) {
            double consum = consumMap.getOrDefault(stockVo.getPaddingCode(), 0D);  //消耗量
//            consum = (consum < 0 ? 0 : consum);
            ...
            stockMap.put(stockVo.getPaddingCode(), planStock);
        }
    ...
    private Map<String, Double> getNcConsume(String batchNo, String scheduleDate) {
        ...
        List<DjStockConsumeVo> list = djEngineStockMapper.listCxPlanAndConsume(scheduleDate);  //查询垫胶胶对应的成型三班计划消耗量，以及8-12点实际消耗量
        ...
        List<DjStockConsumeVo> consumeVos = djEngineStockMapper.listLastDayMidPlan(scheduleDate);
        ...
        //开始计算半部件消耗量
        for (DjStockConsumeVo consumeVo : list) {
            Double cxClass1PlanConsume = consumeVo.getCxClass1PlanConsume();
            if (Objects.isNull(cxClass1PlanConsume)) {
                cxClass1PlanConsume = 0D;
            }
            result.put(consumeVo.getPaddingCode(), cxClass1PlanConsume);
        }
        for (DjStockConsumeVo consumeVo : consumeVos) {
            String treadCode = consumeVo.getPaddingCode();
            ...
            if (result.containsKey(treadCode)) {
                ...
                result.put(consumeVo.getPaddingCode(), planConsume);
            } else {
