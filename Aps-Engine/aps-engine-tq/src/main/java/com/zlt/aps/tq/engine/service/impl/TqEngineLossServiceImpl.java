...
import com.zlt.aps.tq.engine.mapper.TqEngineLossMapper;
...
@Slf4j
@Service
public class TqEngineLossServiceImpl implements TqEngineLossService {

    @Resource
    private TqEngineLossMapper tqEngineLossMapper;

    /**
     * 把损耗率list转成map
     *
     * @return key：机台id + ‘#’ + 胎圈代码，例如20#HT3568-377P
     */
    public Map<String, Double> getLossRateMap() {
        Map<String, Double> lossMap = new HashMap<>();
        List<TqLossVo> list = tqEngineLossMapper.listLossRate();
        for (TqLossVo lossVo : list) {
    ...
    /**
     * 获得损耗率（从损耗率表获取对应的损耗率，获取顺序：机台+物料编号 > 胎圈代码 > 机台 >工序参数配置）
     *
     * @param beadCode  胎圈代码
     ...
     * @return
     */
    public double getLossRate(String beadCode, String machineIds, Map<String, Double> lossMap, double paramLossRate) {
        beadCode = (StringUtils.isBlank(beadCode) ? "" : beadCode);
        if (StringUtils.isBlank(machineIds)) {
            //排程没有机台信息
            double lossRate = lossMap.getOrDefault("#" + beadCode, paramLossRate);
            ...
            for (int i = 0; i < machineIdArry.length; i++) {
                String key1 = machineIdArry[i] + "#" + beadCode;
                String key2 = "#" + beadCode;
