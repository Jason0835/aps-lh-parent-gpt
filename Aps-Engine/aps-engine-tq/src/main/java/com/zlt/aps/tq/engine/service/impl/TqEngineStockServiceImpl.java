...
public class TqEngineStockServiceImpl implements TqEngineStockService {
    ...
    public Map<String, Double> getPlanStockMap(String batchNo, String scheduleDate, Double stockLossRate) {
        ...
        autoScheduleLogService.insertTqScheduleLog(batchNo, "", "①16点预计库存 = 11点半部件库存 - 11点到16点半部件消耗", "11点半部件库存量：" + toJSONString(list));
        if (list.isEmpty()) {
            log.error("胎圈库存查询为空");
            return stockMap;
        }

        //开始计算胎圈16点预计库存 = 11点半部件库存 - 11点到16点半部件消耗
        Map<String, Double> consumMap = this.getTqConsume(batchNo, scheduleDate); // 11点到16点半部件计划消耗量
     ...
     * @return
     */
    private Map<String, Double> getTqConsume(String batchNo, String scheduleDate) {
        Map<String, Double> result = new HashMap<>();
        List<TqStockConsumeVo> list = tqEngineStockMapper.listCxPlanAndConsume(scheduleDate);  //查询胎圈胶对应的成型三班计划消耗量，以及8-12点实际消耗量
        autoScheduleLogService.insertTqScheduleLog(batchNo, "", "②16点预计库存 = 11点半部件库存 - 11点到16点半部件消耗",
                "半制品对应成型(8点-16点)计划量的消耗量cxClass3PlanConsume，以及制品对应成型(8点-12点)完成量的消耗量cxFinishConsume：" + toJSONString(list));  //添加日志
        if (list.isEmpty()) {
            log.error("计算12点到16点胎圈计划消耗量为空");
            return result;
        }
        //开始计算11点到16点半部件消耗量
