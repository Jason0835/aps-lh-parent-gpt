...
public class TmScheduleTemplateImpl extends AbsTmScheduleTemplate {
    ...
    /** 步骤过程日志摘要构建组件。 */
    private final TmScheduleProcessLogBuilder processLogBuilder = new TmScheduleProcessLogBuilder();
    ...
    private void runStep(TmScheduleContext context, TmScheduleStepEnum stepEnum, Runnable runnable) {
        try {
            ...
            if (processLogger != null) {
                processLogger.logStepStart(context, stepEnum.getDesc(),
                        processLogBuilder.buildStepSummary(context, stepEnum, true));
            }
            runnable.run();
            boolean completedInsideService = TmScheduleStepEnum.SNAPSHOT_BUILD == stepEnum
            ...
            if (!completedInsideService) {
                ...
                if (processLogger != null) {
                    processLogger.logStepEnd(context, stepEnum.getDesc(),
                            processLogBuilder.buildStepSummary(context, stepEnum, false));
                }
    ...
    private void appendStepCalculationDetail(TmScheduleContext context, TmScheduleStepEnum stepEnum) {
        ...
        switch (stepEnum) {
            ...
            case MACHINE_ASSIGN:
                this.appendFullStepDetail(context, stepEnum);
                // 实际班次计划量只有机台分配完成后才能确定，库存日志延后到此处生成。
    ...
    private void appendShiftMachineCalculationDetail(TmScheduleContext context) {
        Map<Integer, List<TmTaskDraft>> shiftTaskMap = this.sortedLogTaskStream(context)
                .filter(Objects::nonNull)
                .collect(Collectors.groupingBy(task -> task.getShiftOrder() == null ? 1 : task.getShiftOrder(),
        ...
        shiftTaskMap.forEach((shiftOrder, shiftTaskList) -> {
            ...
            // 工装预校验和账本结算按全局工装账本序号排序；缺少序号时沿用机台展示顺序。
            ...
            shiftTaskList.forEach(task -> {
                ...
                if (!task.isUnassigned() && StrUtil.isNotBlank(task.getMachineCode())) {
                    context.appendShiftProcessLog(shiftOrder, ScheduleProcessLogSection.MACHINE_SELECTION,
                            "机台确认：胎面代码={0}，机台={1}",
                            task.getTreadCode(), task.getMachineCode());
                }
            });

            // 选机后执行损耗、开产阈值、工装预校验和最终计划量定稿。
            ...
            // 实际承接任务后的工装账本结算及使用摘要最后输出。
            shiftTaskList.stream()
                    .sorted(this.buildToolLedgerProcessLogTaskComparator(displayOrderMap))
                    .filter(task -> !task.isUnassigned())
                    .forEach(task -> context.appendShiftProcessLog(shiftOrder,
                            ScheduleProcessLogSection.TOOL_LEDGER_SETTLEMENT,
    ...
    /**
     * 在机台分配完成后按班次记录库存滚动日志，确保日志使用实际落入各班次的计划量。
     *
     * @param context 胎面排程上下文
     */
     ...
     */
    private Stream<TmTaskDraft> sortedLogTaskStream(TmScheduleContext context) {
     ...
     * @return 过程日志任务排序器
     */
    private Comparator<TmTaskDraft> buildLogTaskComparator() {
        return Comparator.comparing(TmTaskDraft::getBaseSortIndex,
                ...
                .thenComparing(TmTaskDraft::getBusinessKey, Comparator.nullsLast(Comparator.naturalOrder()));
    }
    ...
    private String buildOpeningStockFormula(TmScheduleContext context, TmTaskDraft task) {
        ...
        if (previousTask != null) {
            ...
            BigDecimal previousPlanQty = this.resolveActualShiftPlanQty(context, task.getTreadCode(),
                    previousTask.getShiftOrder());
            ...
            BigDecimal displayOpeningStockQty = rawOpeningStockQty.max(BigDecimal.ZERO);
            return displayOpeningStockQty.toPlainString() + "=上一班班初库存"
    ...
    /**
     * 汇总指定胎面在目标班次任务链中的实际计划量，仅用于过程日志展示。
     *
     * <p>任务草稿中的计划量是机台分配前的名义计划量，发生顺延或拆分时不能代表目标班次的实际计划量。
     * 任务链不存在对应节点时按零处理，不回退名义计划量。</p>
     *
     * @param context    胎面排程上下文
     * @param treadCode  胎面代码
     * @param shiftOrder 目标班次顺序
     * @return 目标班次实际计划量
     */
    private BigDecimal resolveActualShiftPlanQty(TmScheduleContext context, String treadCode, Integer shiftOrder) {
        if (context == null || context.getTaskChainGroup() == null || StrUtil.isBlank(treadCode)
                || shiftOrder == null) {
        ...
        return context.getTaskChainGroup().values().stream()
                ...
                .filter(Objects::nonNull)
                .filter(node -> Objects.equals(shiftOrder, node.getShiftOrder()))
                .filter(node -> node.getTask() != null
                        && Objects.equals(treadCode, node.getTask().getTreadCode()))
                .map(node -> this.nvl(node.getPlanQty()))
     ...
     */
    private void appendFullStepDetail(TmScheduleContext context, TmScheduleStepEnum stepEnum) {
        if (TmScheduleStepEnum.BOOTSTRAP == stepEnum) {
        ...
        if (TmScheduleStepEnum.MACHINE_ASSIGN == stepEnum) {
            this.sortedLogTaskStream(context)
                    ...
                    .forEach(task -> context.appendShiftFullProcessTrace(task.getShiftOrder(),
                            ScheduleProcessLogSection.TOOL_LEDGER_SETTLEMENT,
                            this.buildToolUsageFullEvent(context, task)));
        }
        if (TmScheduleStepEnum.PLAN_CALC == stepEnum) {
            ...
            for (int index = 0; index < context.getTaskDraftList().size(); index++) {
                ...
                context.appendShiftFullProcessTrace(task.getShiftOrder(), ScheduleProcessLogSection.PLAN_QTY_CALCULATION,
                        ...
                        "库存供应时长=" + this.displaySupplyHours(task.getSupplyHours()) + "，班次="
                                + task.getShiftOrder() + "，胎面=" + task.getTreadCode() + "。",
     ...
     * @param context 排程上下文
     */
    private void appendBootstrapFullDetail(TmScheduleContext context) {
        context.appendFullProcessTrace(new ScheduleProcessTraceEvent(
                TmScheduleStepEnum.BOOTSTRAP.getDesc(), "批次级", "批次初始化",
                "自动排程请求和胎面基础资料加载服务。",
                ...
                "按工厂和排程日期装载有效参数、成型需求、施工资料、库存、班次和可用机台。",
        ...
        context.getParamMap().entrySet().stream().sorted(Map.Entry.comparingByKey()).forEach(entry -> {
            TmParamValue value = entry.getValue();
            context.appendFullProcessTrace(new ScheduleProcessTraceEvent(
                    TmScheduleStepEnum.BOOTSTRAP.getDesc(), "批次级", "有效参数快照",
                    "胎面参数表或代码缺省值（" + entry.getKey() + "）。",
     ...
     * @param stepEnum 当前步骤
     */
    private void appendUnrenderedRuleTrace(TmScheduleContext context, TmScheduleStepEnum stepEnum) {
        context.getRuleTraceMap().entrySet().stream().sorted(Map.Entry.comparingByKey()).forEach(entry -> {
            TmRuleTrace trace = entry.getValue();
            if (trace == null) {
                return;
            }
            int startIndex = context.getProcessRuleTraceCursorMap().getOrDefault(entry.getKey(), 0);
            List<TmRuleTraceItem> ruleHits = trace.getRuleHits();
            for (int index = startIndex; index < ruleHits.size(); index++) {
                TmRuleTraceItem item = ruleHits.get(index);
                String ruleName = Arrays.stream(TmScheduleRuleCodeEnum.values())
                        .filter(rule -> rule.getCode().equals(item.getRuleCode()))
                        .map(TmScheduleRuleCodeEnum::getDesc).findFirst().orElse("补充业务规则");
                String resultName = Arrays.stream(TmScheduleRuleResultEnum.values())
     ...
     * @return 中文结果去向
     */
    private String resolveRuleDestination(TmScheduleStepEnum stepEnum) {
        switch (stepEnum) {
            case BOOTSTRAP:
                return "进入库存预测和计划量计算。";
            case INVENTORY_PREDICT:
    ...
    private void appendFinalPlanCalculationFullDetail(TmScheduleContext context, TmTaskDraft task) {
        context.appendShiftFullProcessTrace(task.getShiftOrder(), ScheduleProcessLogSection.TOOL_LIMIT,
     ...
     * @return 过程日志分区
     */
    private ScheduleProcessLogSection resolveStepProcessLogSection(TmScheduleStepEnum stepEnum) {
        if (TmScheduleStepEnum.INVENTORY_PREDICT == stepEnum) {
    ...
    private String buildPlanFormula(TmScheduleContext context, TmTaskDraft task) {
        ...
        return "计划量计算：胎面代码=" + task.getTreadCode()
                ...
                + "，是否收尾=" + TmYesNoEnum.YES.getCode().equals(task.getTailFlag())
     ...
     * @return 工装限制过程事件
     */
    private ScheduleProcessTraceEvent buildToolUsageFullEvent(TmScheduleContext context, TmTaskDraft task) {
        return new ScheduleProcessTraceEvent(
                TmScheduleStepEnum.MACHINE_ASSIGN.getDesc(), task.getBusinessKey(), "工装限制",
                "工装总量、当前任务计划量、当班需求量和有效卷曲长度。",
                "胎面=" + task.getTreadCode() + "，计划量=" + this.nvl(task.getPlanQty())
    ...
    private void appendMachineCandidateDetail(TmScheduleContext context, TmTaskDraft task) {
        ...
        if (ScheduleProcessLogLevel.FULL == context.getProcessLogLevel()) {
            candidateList.forEach(candidate -> {
                ...
                context.appendShiftFullProcessTrace(task.getShiftOrder(), ScheduleProcessLogSection.MACHINE_SELECTION,
                        new ScheduleProcessTraceEvent(
                    ...
                    "按配置顺序执行启用的硬约束；关闭的规则跳过，任一启用规则拒绝后停止该机台后续过滤。",
        ...
        if (!filteredMachineMap.isEmpty()) {
            context.appendShiftProcessLog(task.getShiftOrder(), ScheduleProcessLogSection.MACHINE_SELECTION,
                    "机台筛选：胎面代码={0}，已过滤机台={1}", task.getTreadCode(), filteredMachineMap);
        }
        candidateList.stream().filter(candidate -> !candidate.isFiltered())
                .forEach(candidate -> context.appendShiftProcessLog(task.getShiftOrder(),
                        ScheduleProcessLogSection.MACHINE_SELECTION, this.buildScoreDetail(task, candidate)));
        if (ScheduleProcessLogLevel.FULL == context.getProcessLogLevel()) {
            context.appendShiftFullProcessTrace(task.getShiftOrder(), ScheduleProcessLogSection.MACHINE_SELECTION,
                    new ScheduleProcessTraceEvent(
                    ...
                    "最终机台=" + StrUtil.blankToDefault(task.getMachineCode(), "未选中") + "，班次="
                            + task.getShiftOrder() + "。",
                    StrUtil.isBlank(task.getMachineCode()) ? "进入顺延或未排处理。" : "进入工装预校验、计划量定稿、产能扣减和工装账本结算。"
            ));
        }
    }
