...
public class TmEngineServiceImpl implements TmEngineService {
    ...
    public void autoTmSchedule(String scheduleDate) {
        ...
        for (TmScheduleResultVo scheduleVo : scheduleList) {
            ...
            scheduleVo.setStockQty(planStockMap.getOrDefault(scheduleVo.getTreadCode(), 0D));  //16点预计库存
            ...
            this.computeTmPlanQty(scheduleVo, totalPlanQtyMap, lossRateMap, getDouble(paramsMap.get(EngineConstants.LOSS_RATE)));  //计算胎面中班和夜班计划量
...
//    }
     ...
     * @param scheduleList
     */
    private void setProduceOrder(List<TmScheduleResultVo> scheduleList) {
        //根据机台+胶料进行分组
        Map<String, List<TmScheduleResultVo>> groupMap = scheduleList.stream().collect(Collectors.groupingBy(v -> v.getGlueCode() + v.getMachineId()));
        scheduleList.clear();
        ...
        for(List<TmScheduleResultVo> list : groupMap.values()) {
            ...
            //根据库存供应时长升序排序
            list = list.stream().sorted(Comparator.comparing(TmScheduleResultVo::getSupplyTime)).collect(Collectors.toList());
            for(TmScheduleResultVo scheduleVo : list) {
                Double dayPlanQty = scheduleVo.getDayPlanQty();
                Double nightPlanQty = scheduleVo.getNightPlanQty();
                if(dayPlanQty > 0) {
...
//            log.error("月计划汇总数据为空，物料编号为：", scheduleResultVo.getTreadCode());
    ...
    private void newComputeSupplyTime(TmScheduleResultVo scheduleVo,  Double stockQty) {
        ...
        autoScheduleLogService.insertTmScheduleLog(scheduleVo.getBatchNo(), scheduleVo.getOrderNo(), "计算库存供应时长前数据",
                logSplit("具体算法：从1班开始判断，预计库存-1班的计划大于等于0时，供应时长+8小时；预计库存-1班计划-2班计划大于等于0时，供应时长+16小时；预计库存-1班计划-2班计划-3班计划小于0，供应时长=16个小时+（((预计库存-1班计划-2班计划)/3班计划)*8）；以此类推到第5班",
                        "物料编号：" + scheduleVo.getTreadCode() + ",16点预计库存：" + stockQty + "，对应成型一班的计划量：" + cxClass1Plan + "，对应成型二班的计划量：" + cxClass2Plan + "，对应成型三班的计划量：" + cxClass3Plan + "，对应成型次日一班的计划量：" + cxClass4Plan + "，对应成型次日二班的计划量：" + cxClass5Plan));
     ...
     */
    private boolean oneComputeSupplyTime(TmScheduleResultVo scheduleVo,Double remnantStock, Double classPlan) {
        Double supplyTime = scheduleVo.getSupplyTime();
     ...
     */
    private void computeTmPlanQty(TmScheduleResultVo scheduleVo, Map<String, TmTotalPlanQtyVo> totalPlanQtyMap, Map<String, Double> lossMap, double paramLossRate) {
...
//        double unitConsume = BigDecimalUtil.div(scheduleVo.getUnitConsume(), 1000D, 3);
        ...
        if(StringUtils.isNotBlank(machineId) && !machineId.contains(",")) {
            //计划量要加上耗损量
            lossRate = tmEngineLossService.getLossRate(scheduleVo.getTreadCode(), scheduleVo.getMachineId(), lossMap, paramLossRate);
            dayPlanQty = BigDecimalUtil.add(dayPlanQty, BigDecimalUtil.mul(dayPlanQty, lossRate));
            nightPlanQty = BigDecimalUtil.add(nightPlanQty, BigDecimalUtil.mul(nightPlanQty, lossRate));
        }
