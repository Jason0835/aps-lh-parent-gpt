...
public class TmMachineAssignService implements ITmMachineAssignService {
    ...
    /** 机台分配任务固定排序协作器 */
    private final TmMachineAssignmentTaskOrderSelector taskOrderSelector = new TmMachineAssignmentTaskOrderSelector();
    ...
    public void assign(TmScheduleContext context) {
        ...
        for (Map.Entry<Integer, List<TmTaskDraft>> entry : shiftTaskMap.entrySet()) {
            ...
            this.taskOrderSelector.sort(remainingTaskList);
            int machineAssignmentSequence = 0;
            while (CollUtil.isNotEmpty(remainingTaskList)) {
                machineAssignmentSequence++;
                TmTaskDraft task = this.selectNextTaskByBaseSortIndex(remainingTaskList, context,
                        machineAssignmentSequence);
                remainingTaskList.remove(0);
    ...
    private void assignSingleTask(TmTaskDraft task, TmScheduleContext context) {
        ...
        TmMachineCandidate presetCandidate = this.findCandidateByMachineCode(
        ...
        this.addAssignTrace(context, task, TmScheduleRuleResultEnum.PASS, task.getMachineCode(), null, null);
        this.bindSmallGlueMachine(context, task, task.getMachineCode(), null,
                TmScheduleConstants.PRESET_MACHINE_BIND_SOURCE);
        ...
        ITmMachineScoreStrategy scoreStrategy = strategyRegistry.getMachineScoreStrategy(this.resolveParamValue(context,
                TmScheduleConstants.PARAM_MACHINE_SCORE_STRATEGY, TmScheduleStrategyEnum.DEFAULT.getCode()));
     ...
     */
    private boolean isMachineShiftOpen(TmTaskDraft task, TmScheduleContext context,
                                       TmMachineCandidate candidate) {
        if (task == null || context == null || candidate == null || candidate.getOpenShiftCodes() == null) {
            return false;
        }
        TmShiftTimeWindow shiftTimeWindow = context.getShiftTimeWindowMap().get(task.getShiftOrder());
    ...
    /**
     * 按来源排序序号选择本班次下一条实际机台分配任务。
     *
     * <p>机台候选评分只决定当前任务进入分配后的目标机台，不得反向改变任务进入分配的顺序。
     * 因此同一班次严格按 {@code planCalcOrderIndex/baseSortIndex} 升序尝试，排序序号相同时按业务键稳定排序。
     * 当前任务后续即使发生候选过滤失败、工装不足、产能不足或顺延，也会在
     * {@link #assignSingleTask(TmTaskDraft, TmScheduleContext)} 返回后才处理下一条任务。</p>
     *
     * @param remainingTaskList 当前班次尚未分配的实际任务；来源解释任务不在此列表中
     * @param context 胎面排程上下文
     * @param machineAssignmentSequence 本班次机台资源尝试序号，从1开始
     * @return 本轮应优先尝试的任务
     */
    private TmTaskDraft selectNextTaskByBaseSortIndex(List<TmTaskDraft> remainingTaskList,
                                                      TmScheduleContext context,
                                                      int machineAssignmentSequence) {
        TmTaskDraft selectedTask = this.taskOrderSelector.selectFirst(remainingTaskList);
        selectedTask.setMachineAssignmentSequence(machineAssignmentSequence);
        Map<String, Object> sortEvidence = new LinkedHashMap<>();
        sortEvidence.put("phase", "MACHINE_ASSIGN");
        sortEvidence.put("strategyCode", "BASE_SORT_INDEX_FIRST");
        sortEvidence.put("shiftOrder", selectedTask.getShiftOrder());
        sortEvidence.put("planCalcOrderIndex", selectedTask.getPlanCalcOrderIndex());
        sortEvidence.put("baseSortIndex", selectedTask.getBaseSortIndex());
        sortEvidence.put("machineAssignmentSequence", machineAssignmentSequence);
        sortEvidence.put("remainingTaskCountBeforeSelection", remainingTaskList.size());
        sortEvidence.put("presetMachine", !selectedTask.isUnassigned());
        sortEvidence.put("sortPriority", "PLAN_CALC_ORDER_ASC,BASE_SORT_INDEX_ASC,BUSINESS_KEY_ASC");
        sortEvidence.put("selectionDecision", "排序优先选择");
        traceOf(context, selectedTask).addRuleHit(TmScheduleRuleCodeEnum.TASK_SORT,
                TmScheduleRuleResultEnum.PASS, sortEvidence);
        log.info("[TM_MACHINE_ASSIGN_TASK_ORDER] batchNo={}, traceId={}, factoryCode={}, scheduleDate={}, shiftOrder={}, machineAssignmentSequence={}, planCalcOrderIndex={}, baseSortIndex={}, selectedBusinessKey={}, selectedTreadCode={}, selectedGlueCode={}, strategyCode={}, selectionDecision={}, remainingTaskCount={}",
                context.getBatchNo(), context.getTraceId(), context.getFactoryCode(), this.formatScheduleDate(context),
                this.normalizeShiftOrder(selectedTask.getShiftOrder()), machineAssignmentSequence,
                selectedTask.getPlanCalcOrderIndex(),
                selectedTask.getBaseSortIndex(), selectedTask.getBusinessKey(), selectedTask.getTreadCode(),
                selectedTask.getGlueCode(), "BASE_SORT_INDEX_FIRST", "排序优先选择", remainingTaskList.size());
        return selectedTask;
    }
     ...
     * 使用基础排序序号和业务键稳定选择任务。
     *
     * @param remainingTaskList 当前班次尚未分配的实际任务
     * @return 排序最靠前的任务
     */
    TmTaskDraft selectTaskByBaseSortIndex(List<TmTaskDraft> remainingTaskList) {
        return this.taskOrderSelector.selectMinimum(remainingTaskList);
    }
     ...
     * @return 本轮开产班次优先任务
     */
    TmTaskDraft selectStartupSupplyFirstTask(List<TmTaskDraft> remainingTaskList,
    ...
    private String resolveParamValue(TmScheduleContext context, String paramCode, String defaultValue) {
        return TmScheduleContextValueUtils.readParam(context, paramCode, defaultValue, true);
    }
     ...
     * @param candidate 候选机台
     */
    private void applyEffectivePredecessor(TmTaskDraft task, TmScheduleContext context, TmMachineCandidate candidate) {
        ...
        candidate.getEvidence().put("predecessorTreadCode", predecessor.getTreadCode());
     ...
     */
    private TmTaskPredecessor resolveEffectivePredecessor(TmScheduleContext context, String machineCode,
                                                          Integer shiftOrder) {
        Integer normalizedShiftOrder = this.normalizeShiftOrder(shiftOrder);
        TmTaskDraft currentShiftTail = this.findChainTailTask(context, machineCode, normalizedShiftOrder);
        ...
        for (int previousShiftOrder = normalizedShiftOrder - 1; previousShiftOrder >= 1; previousShiftOrder--) {
            TmTaskDraft previousShiftTail = this.findChainTailTask(context, machineCode, previousShiftOrder);
     ...
     */
    private TmTaskDraft findChainTailTask(TmScheduleContext context, String machineCode, Integer shiftOrder) {
        ...
        ScheduleTaskNode<TmTaskDraft> tailNode = nodeList.get(nodeList.size() - 1);
     ...
     * @return 前置任务快照
     */
    private TmTaskPredecessor buildPredecessorFromTask(String machineCode, Integer shiftOrder, TmTaskDraft task) {
        TmTaskPredecessor predecessor = new TmTaskPredecessor();
        predecessor.setMachineCode(machineCode);
        predecessor.setTreadCode(task.getTreadCode());
     ...
     * @return 机台前置任务摘要
     */
    private Map<String, String> summarizeMachinePredecessors(TmScheduleContext context, Integer shiftOrder) {
        if (CollUtil.isEmpty(context.getMachineCandidateList())) {
            return Collections.emptyMap();
        }
        Map<String, String> summary = new LinkedHashMap<>();
        for (TmMachineCandidate candidate : context.getMachineCandidateList()) {
            TmTaskPredecessor predecessor = this.resolveEffectivePredecessor(context, candidate.getMachineCode(), shiftOrder);
            if (predecessor == null) {
                summary.put(candidate.getMachineCode(), null);
                continue;
            }
            summary.put(candidate.getMachineCode(), predecessor.getTreadCode() + "/" + predecessor.getGlueCode()
    ...
    private Integer normalizeShiftOrder(Integer shiftOrder) {
        ...
        return Math.min(shiftOrder, TmScheduleConstants.TM_MAX_SHIFT_ORDER);
    }
    ...
    private void assignByFilterAndScore(TmTaskDraft task, TmScheduleContext context) {
        ...
        if (CollUtil.isEmpty(candidateList)) {
            ...
            context.getCandidateTraceMap().put(task.getBusinessKey(), Collections.emptyList());
            addAssignTrace(context, task, TmScheduleRuleResultEnum.REJECT, null,
                    TmUnplannedReasonEnum.NO_AVAILABLE_MACHINE.getCode(),
            ...
            this.logMachineAssignSummary(context, task, Collections.emptyList(), Collections.emptyList(), null,
                    TmScheduleRuleResultEnum.REJECT, TmUnplannedReasonEnum.NO_AVAILABLE_MACHINE);
        ...
        for (TmMachineCandidate candidate : candidates) {
            ...
            if (ruleResult.isPassed()) {
                ...
                addFilterTrace(context, task, candidate, TmScheduleRuleResultEnum.PASS, null, null);
            } else {
                addFilterTrace(context, task, candidate, TmScheduleRuleResultEnum.REJECT,
        ...
        if (passedCandidates.isEmpty()) {
            if (this.isCapacityOrShiftBlockedCarryover(task, context, filterRule)) {
                boolean machineShiftBlocked = candidates.stream().anyMatch(candidate ->
                        TmMachineFilterReasonEnum.MACHINE_SHIFT_NOT_OPEN.getCode()
            ...
            this.logMachineAssignSummary(context, task, candidates, passedCandidates, null,
                    TmScheduleRuleResultEnum.REJECT, unplannedReason);
            context.getCandidateTraceMap().put(task.getBusinessKey(), candidates);
        ...
        this.logMachineAssignSummary(context, task, candidates, passedCandidates, bestCandidate,
                TmScheduleRuleResultEnum.PASS, null);
     ...
     */
    private boolean isCapacityOrShiftBlockedCarryover(TmTaskDraft task, TmScheduleContext context,
     ...
     *
     * <p>除机台开机班次外，胶料等约束也可能随班次变化，因此不能仅检查开机编码集合。</p>
     *
     * @param task 当前待排任务
     * @param context 排程上下文
     * @param filterRule 机台过滤规则
     ...
     */
    private boolean hasFutureStaticCandidate(TmTaskDraft task, TmScheduleContext context,
                                             ITmMachineFilterRule filterRule, Integer currentShiftOrder) {
        Integer originalShiftOrder = task.getShiftOrder();
        try {
            for (int futureShiftOrder = currentShiftOrder + 1;
                 futureShiftOrder <= TmScheduleConstants.TM_MAX_SHIFT_ORDER; futureShiftOrder++) {
                TmShiftTimeWindow shiftTimeWindow = context.getShiftTimeWindowMap().get(futureShiftOrder);
                if (shiftTimeWindow == null || StrUtil.isBlank(shiftTimeWindow.getShiftCode())) {
                    continue;
                }
                task.setShiftOrder(futureShiftOrder);
                List<TmMachineCandidate> futureCandidates = this.copyCandidates(context.getMachineCandidateList());
                this.prepareCandidatesForTask(task, context, futureCandidates);
                TmMachineRuleContext futureRuleContext = new TmMachineRuleContext();
    ...
    private void appendTaskWithCapacityOverflow(TmTaskDraft task, TmMachineCandidate selectedCandidate,
                                                ...
                                                TmScheduleContext context, String taskSource) {
        ...
        log.info("[TM_MACHINE_ASSIGN_FINAL] batchNo={}, traceId={}, factoryCode={}, scheduleDate={}, businessKey={}, treadCode={}, shiftOrder={}, selectedMachineCode={}, planQtyBeforeToolLimit={}, finalPlanQty={}, toolLimitAdjustQty={}, toolOverflowQty={}, availableToolQty={}, assignStatus={}",
                ...
                StrUtil.isNotBlank(task.getUnplannedReasonCode())
                        ? TmMachineAssignStatusEnum.UNPLANNED.getCode()
                        : TmMachineAssignStatusEnum.ASSIGNED.getCode());
        ...
        if (currentShiftPlanQty.compareTo(BigDecimal.ZERO) > 0) {
            ...
            if (assignedQty.compareTo(BigDecimal.ZERO) > 0) {
                ...
                this.addAssignTrace(context, task, TmScheduleRuleResultEnum.PASS,
     ...
     * @param context           胎面排程上下文
     */
    private void fillCurrentShiftIdleCapacity(Integer currentShiftOrder, Map<Integer, List<TmTaskDraft>> shiftTaskMap,
                                             TmScheduleContext context) {
        ...
        while (hasAssigned) {
            hasAssigned = false;
            for (TmMachineCandidate machineCandidate : context.getMachineCandidateList()) {
                if (machineCandidate == null || StrUtil.isBlank(machineCandidate.getMachineCode())) {
                    continue;
                }
                TmTaskDraft sourceTask = this.selectFutureEarlyFillTask(targetShiftOrder, shiftTaskMap, context,
                ...
                TmMachineCandidate runtimeCandidate = this.resolveEarlyFillRuntimeCandidate(sourceTask,
                ...
                TmTaskDraft earlyFillTask = this.copyFutureEarlyFillTask(sourceTask, targetShiftOrder, assignedQty,
            ...
            this.addAssignTrace(context, earlyFillTask, TmScheduleRuleResultEnum.PASS,
                    runtimeCandidate.getMachineCode(), null, null);
                ...
                log.info("[TM_FUTURE_SHIFT_EARLY_FILL] batchNo={}, traceId={}, factoryCode={}, scheduleDate={}, sourceBusinessKey={}, targetBusinessKey={}, sourceShiftOrder={}, targetShiftOrder={}, machineCode={}, assignedQty={}, sourceRemainQty={}",
                        context.getBatchNo(), context.getTraceId(), context.getFactoryCode(), this.formatScheduleDate(context),
     ...
     */
    private TmTaskDraft selectFutureEarlyFillTask(Integer targetShiftOrder, Map<Integer, List<TmTaskDraft>> shiftTaskMap,
                                                  TmScheduleContext context, ITmMachineFilterRule filterRule,
                                                  ITmMachineScoreStrategy scoreStrategy, String machineCode,
                                                  int earlyFillIndex) {
        TmTaskDraft bestTask = null;
        ...
        for (Map.Entry<Integer, List<TmTaskDraft>> entry : shiftTaskMap.entrySet()) {
            ...
            for (int taskIndex = 0; taskIndex < entry.getValue().size(); taskIndex++) {
                TmTaskDraft sourceTask = entry.getValue().get(taskIndex);
     ...
     */
    private TmMachineCandidate resolveEarlyFillRuntimeCandidate(TmTaskDraft sourceTask, Integer targetShiftOrder,
                                                               Integer sourceShiftOrder, TmScheduleContext context,
                                                               ITmMachineFilterRule filterRule,
                                                               ITmMachineScoreStrategy scoreStrategy,
                                                               String machineCode, int earlyFillIndex) {
        TmTaskDraft probeTask = this.copyFutureEarlyFillTask(sourceTask, targetShiftOrder, nvl(sourceTask.getPlanQty()),
                sourceShiftOrder, earlyFillIndex, machineCode + "_PROBE");
        List<TmMachineCandidate> candidates = this.buildPassedAndScoredCandidates(probeTask, context,
                filterRule, scoreStrategy);
        TmMachineCandidate candidate = this.findCandidateByMachineCode(candidates, machineCode);
        if (candidate == null) {
            return null;
        }
     ...
     * @param context     胎面排程上下文
     */
    private void deductFutureTaskPlan(TmTaskDraft sourceTask, BigDecimal deductedQty,
                                      Map<Integer, List<TmTaskDraft>> shiftTaskMap, TmScheduleContext context) {
        ...
        for (List<TmTaskDraft> taskList : shiftTaskMap.values()) {
            if (taskList != null) {
     ...
     * @return 提前补产任务副本
     */
    private TmTaskDraft copyFutureEarlyFillTask(TmTaskDraft sourceTask, Integer targetShiftOrder, BigDecimal planQty,
                                                Integer sourceShiftOrder, int earlyFillIndex, String machineCode) {
        TmTaskDraft target = this.copyOverflowTask(sourceTask, targetShiftOrder, planQty, sourceShiftOrder,
     ...
     * @return 业务键后缀
     */
    private String buildFutureEarlyFillBusinessKeySuffix(TmTaskDraft sourceTask, Integer sourceShiftOrder,
        ...
        if (StrUtil.isBlank(sourceOrderToken)) {
            sourceOrderToken = TmScheduleConstants.UNKNOWN_CODE;
        }
     ...
     */
    private void addFutureEarlyFillTrace(TmScheduleContext context, TmTaskDraft targetTask, TmTaskDraft sourceTask,
        ...
        traceOf(context, targetTask).addRuleHit(TmScheduleRuleCodeEnum.FUTURE_SHIFT_EARLY_FILL,
    ...
    private void appendCarryoverQty(TmTaskDraft sourceTask, TmMachineCandidate selectedCandidate,
        ...
        for (int shiftOrder = sourceShiftOrder + 1;
            ...
            List<TmMachineCandidate> shiftCandidates = this.resolveShiftAssignableCandidates(sourceTask, context,
                    filterRule, scoreStrategy, selectedCandidate, shiftOrder, remainingQty,
            ...
            for (TmMachineCandidate candidate : shiftCandidates) {
                ...
                this.addAssignTrace(context, mergeTarget, TmScheduleRuleResultEnum.PASS,
            ...
            this.addAssignTrace(context, overflowTask, TmScheduleRuleResultEnum.PASS,
                    runtimeCandidate.getMachineCode(), null, null);
     ...
     */
    private void logMachineAssignSummary(TmScheduleContext context, TmTaskDraft task,
                                         List<TmMachineCandidate> candidates,
                                         List<TmMachineCandidate> passedCandidates,
        ...
        log.info("[TM_MACHINE_ASSIGN_SUMMARY] batchNo={}, traceId={}, factoryCode={}, scheduleDate={}, businessKey={}, treadCode={}, shiftOrder={}, result={}, candidateCount={}, passedCount={}, rejectedCount={}, rejectReasonSummary={}, selectedMachineCode={}, selectedScore={}, scoreItems={}, planQty={}, remainCapacity={}, assignedQty={}, overflowQty={}, reasonCode={}, reasonDesc={}",
    ...
    /**
     * 保存来源班次候选机台失败和顺延判定证据，供解释表还原顺延原因。
     *
     * @param context                胎面排程上下文
     * @param sourceTask             来源任务
     * @param candidates             来源班次候选机台
     * @param carryoverSourceType    顺延来源类型
     * @param carryoverQty           待顺延计划量
     */
    private void addSourceCarryoverFailureTrace(TmScheduleContext context, TmTaskDraft sourceTask,
                                                List<TmMachineCandidate> candidates, String carryoverSourceType,
                                                BigDecimal carryoverQty) {
        Map<String, Object> evidence = new LinkedHashMap<>();
        evidence.put("sourceTask", sourceTask == null ? null : sourceTask.getBusinessKey());
        evidence.put("sourceShiftOrder", sourceTask == null ? null : sourceTask.getShiftOrder());
        evidence.put("sourcePlanQty", sourceTask == null ? null : sourceTask.getPlanQty());
        evidence.put("planCalcOrderIndex", sourceTask == null ? null : sourceTask.getPlanCalcOrderIndex());
        evidence.put("baseSortIndex", sourceTask == null ? null : sourceTask.getBaseSortIndex());
        evidence.put("machineAssignmentSequence", sourceTask == null
                ? null : sourceTask.getMachineAssignmentSequence());
        evidence.put("carryoverQty", carryoverQty);
        evidence.put("sourceType", carryoverSourceType);
        evidence.put("sourceReasonCode", carryoverSourceType);
        evidence.put("sourceReasonDesc", this.resolveCarryoverReasonDesc(carryoverSourceType));
        evidence.put("candidateCount", candidates == null ? 0 : candidates.size());
        evidence.put("passedCandidateCount", 0);
        evidence.put("rejectReasonSummary", this.summarizeRejectReasons(candidates));
        evidence.put("candidateDetails", this.buildCarryoverCandidateDetails(candidates));
        evidence.put("nextStep", "从来源班次下一班开始重新过滤、评分并尝试承接");
        TmScheduleRuleCodeEnum ruleCode = TmScheduleRuleCodeEnum.MACHINE_SHIFT_BLOCKED_CARRYOVER.getCode()
                .equals(carryoverSourceType)
                ? TmScheduleRuleCodeEnum.MACHINE_SHIFT_BLOCKED_CARRYOVER
                : TmScheduleRuleCodeEnum.CAPACITY_BLOCKED_CARRYOVER;
        traceOf(context, sourceTask).addRuleHit(ruleCode, TmScheduleRuleResultEnum.REJECT, evidence);
    }

    /**
     * 构建候选机台失败明细，避免只保存原因汇总而丢失单机台证据。
     *
     * @param candidates 候选机台
     * @return 候选机台结构化失败明细
     */
    private List<Map<String, Object>> buildCarryoverCandidateDetails(List<TmMachineCandidate> candidates) {
        if (CollUtil.isEmpty(candidates)) {
            return Collections.emptyList();
        }
        return candidates.stream().filter(Objects::nonNull).map(candidate -> {
            Map<String, Object> detail = new LinkedHashMap<>();
            detail.put("machineCode", candidate.getMachineCode());
            detail.put("filtered", candidate.isFiltered());
            detail.put("filterReasonCode", candidate.getFilterReasonCode());
            detail.put("filterReasonDesc", candidate.getFilterReasonDesc());
            detail.put("remainCapacity", candidate.getRemainCapacity());
            detail.put("maxCapacity", candidate.getMaxCapacity());
            detail.put("evidence", candidate.getFilterEvidence());
            return detail;
        }).collect(Collectors.toList());
    }
     ...
     * @return 顺延原因说明
     */
    private String resolveCarryoverReasonDesc(String carryoverSourceType) {
        if (TmScheduleRuleCodeEnum.MACHINE_SHIFT_BLOCKED_CARRYOVER.getCode().equals(carryoverSourceType)) {
            return "来源班次机台未开班或当前班不可生产，顺延到后续班次继续尝试";
        }
        return "来源班次候选机台均因当前班剩余产能不足被过滤，顺延到后续班次继续尝试";
    }
     ...
     */
    private void logCarryoverSummary(TmScheduleContext context, TmTaskDraft targetTask, TmTaskDraft sourceTask,
        ...
        log.info("[TM_MACHINE_CARRYOVER] batchNo={}, traceId={}, factoryCode={}, scheduleDate={}, sourceBusinessKey={}, targetBusinessKey={}, sourceType={}, sourceShiftOrder={}, targetShiftOrder={}, targetMachineCode={}, carryoverQty={}, capacityOverflowQty={}, toolOverflowQty={}, beforeMergePlanQty={}, afterMergePlanQty={}, remainingQty={}, merged={}, reasonCode={}",
                context == null ? null : context.getBatchNo(), context == null ? null : context.getTraceId(),
                context == null ? null : context.getFactoryCode(), formatScheduleDate(context),
        ...
        if (context != null && sourceTask != null && sourceShiftOrder != null) {
            context.appendShiftProcessLog(sourceShiftOrder, ScheduleProcessLogSection.MACHINE_SELECTION,
                    "机台过滤与选择：胎面代码={0}，顺延原因={1}。",
                    sourceTask.getTreadCode(), this.resolveCarryoverReasonDesc(sourceType));
        }
    }
     ...
     * @return 原因编码到数量的映射
     */
    private Map<String, Long> summarizeRejectReasons(List<TmMachineCandidate> candidates) {
        if (CollUtil.isEmpty(candidates)) {
            return Collections.emptyMap();
        }
    ...
    private List<TmMachineCandidate> resolveShiftAssignableCandidates(TmTaskDraft sourceTask, TmScheduleContext context,
        ...
        TmTaskDraft probeTask = this.copyOverflowTask(sourceTask, shiftOrder, remainingQty, sourceShiftOrder,
                overflowIndex, TmScheduleConstants.OVERFLOW_PROBE_SUFFIX);
     ...
     * @param context 排程上下文
     */
    private void finalizeSelectedTaskPlan(TmTaskDraft task, TmMachineCandidate selectedCandidate,
                                          TmScheduleContext context, String taskSource) {
        ...
        SchedulePlanQtyAdjustmentResult planQtyAdjustmentResult = this.constraintCalculator.calculatePlanQtyAfterLoss(
                ...
                TmScheduleConstants.DECIMAL_CALCULATION_SCALE);
        ...
        if (currentAvailableToolQty != null && curlLength.compareTo(BigDecimal.ZERO) > 0) {
            ...
            toolLimitAdjustQty = finalPlanQty.subtract(planQtyBeforeToolLimit)
                    .setScale(TmScheduleConstants.DECIMAL_CALCULATION_SCALE,
                            java.math.RoundingMode.HALF_UP);
        ...
        task.setLossMatchLevel(matchResult == null
                ...
                ? TmLossMatchLevelEnum.PARAM_DEFAULT.getCode() : TmLossMatchLevelEnum.NONE.getCode())
                : TmLossMatchLevelEnum.LEGACY_TASK.getCode())
        ...
        if (startupPlanQtyLimit != null) {
            ...
            traceOf(context, task).addRuleHit(TmScheduleRuleCodeEnum.STARTUP_THRESHOLD_ADJUST,
                    startupThresholdAdjusted ? TmScheduleRuleResultEnum.PASS : TmScheduleRuleResultEnum.SKIP,
                    startupEvidence);
        }
     ...
     */
    BigDecimal applyStartupFinalPlanQtyLimit(TmScheduleContext context, TmTaskDraft task,
    ...
    private BigDecimal resolveStartupThreshold(TmScheduleContext context) {
        String value = this.resolveParamValue(context, TmScheduleConstants.PARAM_OPEN_SHIFT_THRESHOLD,
     ...
     */
    private BigDecimal resolveCurrentAvailableToolQty(TmScheduleContext context, TmTaskDraft task) {
        if (context != null && context.getCurrentAvailableToolQty() != null) {
            return context.getCurrentAvailableToolQty().setScale(TmScheduleConstants.DECIMAL_CALCULATION_SCALE,
                    java.math.RoundingMode.HALF_UP);
        }
        if (task != null && task.getAvailableToolQty() != null) {
            return task.getAvailableToolQty().setScale(TmScheduleConstants.DECIMAL_CALCULATION_SCALE,
                    java.math.RoundingMode.HALF_UP);
        }
        return null;
    }
    ...
    private BigDecimal limitPlanQtyByCurrentTool(TmTaskDraft task, TmScheduleContext context,
        ...
        if (currentAvailableToolQty == null) {
            context.appendDeferredToolPrecheckProcessLog(processLogTaskBusinessKey, logShiftOrder,
     ...
     * @param taskSource 任务来源
     */
    private void settleAssignedTaskToolState(TmTaskDraft task, TmScheduleContext context, String taskSource) {
        BigDecimal currentAvailableToolQty = this.resolveCurrentAvailableToolQty(context, task);
        if (currentAvailableToolQty == null) {
            return;
        }
        ...
        context.appendDeferredToolLedgerSettlementProcessLog(task.getBusinessKey(), task.getShiftOrder(),
     ...
     */
    private void recordToolLedgerSnapshot(TmScheduleContext context, TmTaskDraft task,
     ...
     * @param context 排程上下文
     */
    private void applyCarryoverMergeToolState(TmTaskDraft mergeTarget, BigDecimal carryoverQty, TmScheduleContext context) {
        ...
        context.appendDeferredToolLedgerSettlementProcessLog(mergeTarget.getBusinessKey(), mergeTarget.getShiftOrder(),
     ...
     * @return 命中来源说明
     */
    private String buildLossMatchSource(TmTaskDraft task, TmMachineCandidate selectedCandidate,
                                        TmLossRuleMatchResult matchResult, String lossMatchLevel) {
        ...
        if (matchResult == null || matchResult.getMatchedRule() == null) {
            return "machineCode=" + machineCode + ",treadCode=" + task.getTreadCode()
                    + ",lossMatchLevel=" + lossMatchLevel;
        }
        TmLossRule matchedRule = matchResult.getMatchedRule();
        return "machineCode=" + machineCode + ",treadCode=" + task.getTreadCode()
                + ",ruleMachineCode=" + matchedRule.getMachineCode()
                + ",ruleTreadCode=" + matchedRule.getTreadCode()
    ...
    private TmTaskDraft copyOverflowTask(TmTaskDraft source, Integer shiftOrder, BigDecimal planQty,
        ...
        target.setPlanCalcOrderIndex(source.getPlanCalcOrderIndex());
    ...
    private String resolveCarryoverSourceType(BigDecimal capacityOverflowQty, BigDecimal toolOverflowQty) {
        ...
        if (capacityOverflow && toolOverflow) {
            return TmScheduleConstants.CARRYOVER_SOURCE_CAPACITY_LIMIT + ","
                    + TmScheduleConstants.CARRYOVER_SOURCE_TOOL_LIMIT;
        }
        if (toolOverflow) {
            return TmScheduleConstants.CARRYOVER_SOURCE_TOOL_LIMIT;
        }
        return TmScheduleConstants.CARRYOVER_SOURCE_CAPACITY_LIMIT;
    }
    ...
    private void addCarryoverTrace(TmScheduleContext context, TmTaskDraft targetTask, TmTaskDraft sourceTask,
        ...
        evidence.put("planCalcOrderIndex", targetTask == null ? null : targetTask.getPlanCalcOrderIndex());
        evidence.put("baseSortIndex", targetTask == null ? null : targetTask.getBaseSortIndex());
        evidence.put("machineAssignmentSequence", targetTask == null
                ? null : targetTask.getMachineAssignmentSequence());
        evidence.put("merged", merged);
        evidence.put("beforeMergePlanQty", beforeMergeQty);
        evidence.put("afterMergePlanQty", afterMergeQty);
        TmScheduleRuleCodeEnum ruleCode = TmScheduleRuleCodeEnum.CAPACITY_BLOCKED_CARRYOVER.getCode().equals(sourceType)
        ...
        if (sourceTask != null && sourceTask != targetTask) {
            Map<String, Object> sourceEvidence = new LinkedHashMap<>(evidence);
            sourceEvidence.put("sourceEvidenceType", "CARRYOVER_TARGET_ASSIGNMENT");
            traceOf(context, sourceTask).addRuleHit(ruleCode,
     ...
     */
    private void appendCapacityDeductProcessLog(TmScheduleContext context, TmTaskDraft task,
                                                TmMachineCandidate candidate, BigDecimal beforeAssignQty,
    ...
    private void addCapacitySplitTrace(TmScheduleContext context, TmTaskDraft task, BigDecimal beforeAssignQty,
        ...
        evidence.put("splitType", sameShift ? TmScheduleConstants.SPLIT_TYPE_SAME_SHIFT_MATCHED_MACHINE
                : TmScheduleConstants.SPLIT_TYPE_NEXT_SHIFT_MATCHED_MACHINE);
        evidence.put("beforeAssignQty", beforeAssignQty);
        evidence.put("assignedQty", assignedQty);
        evidence.put("overflowQty", overflowQty);
        evidence.put("remainCapacity", remainCapacity);
        traceOf(context, task).addRuleHit(TmScheduleRuleCodeEnum.CAPACITY_OVERFLOW_SPLIT,
                overflowQty.compareTo(BigDecimal.ZERO) > 0
                        ? TmScheduleRuleResultEnum.SPLIT : TmScheduleRuleResultEnum.PASS,
                evidence);
    ...
    private void addNewSpecAdvanceResultTrace(TmScheduleContext context, TmTaskDraft task, BigDecimal assignedQty,
        ...
        evidence.put("assignedQty", assignedQty);
        evidence.put("overflowQty", overflowQty);
        traceOf(context, task).addRuleHit(TmScheduleRuleCodeEnum.NEW_SPEC_ADVANCE_RESULT,
                advancedWindowHit ? TmScheduleRuleResultEnum.PASS : TmScheduleRuleResultEnum.ROLLING,
                evidence);
    }
    ...
    private void addCapacityUnplannedTrace(TmScheduleContext context, TmTaskDraft task, BigDecimal remainingQty) {
        ...
        traceOf(context, task).addRuleHit(TmScheduleRuleCodeEnum.CAPACITY_OVERFLOW_UNPLANNED,
    ...
    private void addToolLimitUnplannedTrace(TmScheduleContext context, TmTaskDraft task,
        ...
        traceOf(context, task).addRuleHit(TmScheduleRuleCodeEnum.TOOL_LIMIT_UNPLANNED,
                TmScheduleRuleResultEnum.REJECT, evidence);
    }
    ...
    private String resolveSmallGlueSwitchReason(String boundMachineCode, List<TmMachineCandidate> candidates) {
        ...
        if (boundCandidate == null) {
            return TmScheduleConstants.SMALL_GLUE_BOUND_MACHINE_NOT_FOUND;
        }
        ...
        if (nvl(boundCandidate.getRemainCapacity()).compareTo(BigDecimal.ZERO) <= 0) {
            return TmMachineFilterReasonEnum.NO_REMAIN_CAPACITY.getCode();
        }
        return null;
    }
    ...
    private void addSmallGlueContinuousTrace(TmScheduleContext context, TmTaskDraft task, String originalMachineCode,
        ...
        Map<String, Object> evidence = new LinkedHashMap<>();
        evidence.put("paramCode", TmScheduleConstants.PARAM_SMALL_GLUE_CODES);
        ...
        traceOf(context, task).addRuleHit(TmScheduleRuleCodeEnum.SMALL_GLUE_CONTINUOUS,
                selectedMachineCode == null
                        ? TmScheduleRuleResultEnum.REJECT : TmScheduleRuleResultEnum.PASS,
                evidence);
    }
    ...
    private void addFilterTrace(TmScheduleContext context, TmTaskDraft task, TmMachineCandidate candidate,
                                TmScheduleRuleResultEnum result, String reasonCode, String reasonDesc) {
        Map<String, Object> evidence = new LinkedHashMap<>();
        ...
        traceOf(context, task).addRuleHit(TmScheduleRuleCodeEnum.MACHINE_FILTER, result, evidence);
    }
    ...
    private void addScoreTrace(TmScheduleContext context, TmTaskDraft task, TmMachineCandidate candidate,
        ...
        traceOf(context, task).addRuleHit(TmScheduleRuleCodeEnum.MACHINE_SCORE,
                TmScheduleRuleResultEnum.PASS, evidence);
    }
     ...
     * @param reasonDesc          原因描述
     */
    private void addAssignTrace(TmScheduleContext context, TmTaskDraft task, TmScheduleRuleResultEnum result,
                                String selectedMachineCode, String reasonCode, String reasonDesc) {
        Map<String, Object> evidence = new LinkedHashMap<>();
        evidence.put("selectedMachineCode", selectedMachineCode);
        evidence.put("reasonCode", reasonCode);
        evidence.put("reasonDesc", reasonDesc);
        traceOf(context, task).addRuleHit(TmScheduleRuleCodeEnum.MACHINE_ASSIGN, result, evidence);
    }
    ...
    private TmUnplannedReasonEnum resolveUnplannedReasonFromCandidates(List<TmMachineCandidate> candidates) {
        ...
        if (reasonCodes.size() == 1) {
            ...
            if (TmMachineFilterReasonEnum.NO_REMAIN_CAPACITY.getCode().equals(reasonCode)) {
                return TmUnplannedReasonEnum.CAPACITY_NOT_ENOUGH;
            }
            if (TmMachineFilterReasonEnum.MOUTH_PLATE_NOT_MATCH.getCode().equals(reasonCode)) {
                return TmUnplannedReasonEnum.MOUTH_PLATE_NOT_MATCH;
            }
            if (TmMachineFilterReasonEnum.GLUE_MACHINE_NOT_MATCH.getCode().equals(reasonCode)) {
    ...
    private void prepareCandidatesForTask(TmTaskDraft task, TmScheduleContext context,
        ...
        for (TmMachineCandidate candidate : candidates) {
            ...
            log.debug("[TM_MACHINE_CAPACITY_DETAIL] treadCode={}, machineCode={}, shiftOrder={}, maxCapacity={}, maintenanceHours={}, totalMaintenanceHours={}, machineSpeed={}, assignedPlanQty={}, previousGlueCode={}, currentGlueCode={}, glueChangeCapacityDeductParam={}, currentGlueSwitchCapacityDeduct={}, existingSwitchCapacityDeduct={}, currentSwitchCapacityDeduct={}, remainCapacity={}",
                    ...
                    this.resolveParamValue(context, TmScheduleConstants.PARAM_GLUE_CHANGE_CAPACITY_DEDUCT,
     ...
     * @return true 表示匹配
     */
    private boolean isMouthPlateMatched(TmTaskDraft task, TmScheduleContext context,
                                        TmMachineCandidate candidate) {
     ...
     * @return true 表示匹配
     */
    private boolean isGlueMachineMatched(TmTaskDraft task, TmScheduleContext context,
                                         TmMachineCandidate candidate) {
    ...
    /**
     * 计算合并到既有同胎面任务时的剩余产能，合并数量不会新增切换损失。
     *
     * @param task         待合并任务草稿
     * @param context      排程上下文
     * @param candidate    候选机台
     ...
     */
    private BigDecimal resolveRemainCapacityWithoutNewSwitch(TmTaskDraft task, TmScheduleContext context,
                                                              TmMachineCandidate candidate,
        ...
        return TmSwitchCapacityCalculator.calculateRemainCapacity(maxCapacity, maintenanceDeduct, assignedPlanQty,
                existingSwitchDeduct, BigDecimal.ZERO);
    }
     ...
     */
    private BigDecimal resolvePrependRemainCapacity(TmTaskDraft task, TmScheduleContext context,
        ...
        BigDecimal assignedPlanQty = this.resolveAssignedPlanQty(context, candidate.getMachineCode(),
                task.getShiftOrder());
        List<TmTaskDraft> orderedTaskList = new ArrayList<>();
        orderedTaskList.add(task);
        ScheduleTaskLinkedList<TmTaskDraft> chain = context.getTaskChain(candidate.getMachineCode(),
                task.getShiftOrder());
        ...
        TmTaskPredecessor predecessor = this.resolvePreviousShiftPredecessor(context, candidate.getMachineCode(),
                task.getShiftOrder());
        BigDecimal totalSwitchHours = BigDecimal.ZERO;
        BigDecimal totalSwitchDeduct = BigDecimal.ZERO;
        for (TmTaskDraft currentTask : orderedTaskList) {
            ...
            BigDecimal specSwitchHours = predecessor != null
                    && !Objects.equals(predecessor.getTreadCode(), currentTask.getTreadCode())
                ? this.resolveSwitchParamHours(context, TmScheduleConstants.PARAM_SPEC_CHANGE_MINUTES)
                : BigDecimal.ZERO;
            ...
            BigDecimal glueSwitchHours = TmSwitchCapacityCalculator.convertCapacityDeductToHours(
                    glueSwitchCapacityDeduct, currentSpeed);
        ...
        return TmSwitchCapacityCalculator.calculateRemainCapacity(maxCapacity, maintenanceDeduct, assignedPlanQty,
                BigDecimal.ZERO, totalSwitchDeduct);
    }
    ...
    private BigDecimal resolveRemainCapacity(TmTaskDraft task, TmScheduleContext context,
        ...
        return TmSwitchCapacityCalculator.calculateRemainCapacity(maxCapacity, maintenanceDeduct, assignedPlanQty,
                existingSwitchDeduct, currentSwitchDeduct);
    }
     ...
     * @return 当前候选切换总小时数
     */
    private BigDecimal resolveCurrentSwitchHours(TmTaskDraft task, TmScheduleContext context,
                                                 TmMachineCandidate candidate) {
        TmTaskPredecessor predecessor = this.resolveEffectivePredecessor(context, candidate.getMachineCode(),
                task.getShiftOrder());
        ...
        task.setPreviousGlueSwitchCapacityDeduct(BigDecimal.ZERO);
        if (predecessor != null) {
            if (!Objects.equals(predecessor.getTreadCode(), task.getTreadCode())) {
            specSwitchHours = this.resolveSwitchParamHours(context, TmScheduleConstants.PARAM_SPEC_CHANGE_MINUTES);
            }
            if (this.isGlueSwitch(predecessor.getGlueCode(), task.getGlueCode())) {
                ...
                glueSwitchHours = TmSwitchCapacityCalculator.convertCapacityDeductToHours(glueSwitchCapacityDeduct,
     ...
     * @return 已发生切换折算量
     */
    private BigDecimal resolveExistingSwitchDeduct(TmScheduleContext context, String machineCode, Integer shiftOrder) {
        ScheduleTaskLinkedList<TmTaskDraft> chain = context.getTaskChain(machineCode, shiftOrder);
        if (chain == null || CollUtil.isEmpty(chain.toList())) {
            return BigDecimal.ZERO;
        }
     ...
     */
    private BigDecimal resolveGlueSwitchCapacityDeduct(TmScheduleContext context) {
        String value = this.resolveParamValue(context, TmScheduleConstants.PARAM_GLUE_CHANGE_CAPACITY_DEDUCT,
                TmScheduleConstants.DEFAULT_GLUE_CHANGE_CAPACITY_DEDUCT);
        try {
            BigDecimal deduct = new BigDecimal(value);
            if (deduct.compareTo(BigDecimal.ZERO) < 0) {
                log.warn("[TM_SWITCH_PARAM] batchNo={}, traceId={}, paramCode={}, paramValue={}, reason=NEGATIVE_VALUE",
                        context.getBatchNo(), context.getTraceId(),
                        TmScheduleConstants.PARAM_GLUE_CHANGE_CAPACITY_DEDUCT, value);
                return BigDecimal.ZERO;
            }
            return deduct;
        } catch (NumberFormatException exception) {
            log.warn("[TM_SWITCH_PARAM] batchNo={}, traceId={}, paramCode={}, paramValue={}, defaultValue={}, reason=INVALID_NUMBER",
                    context.getBatchNo(), context.getTraceId(),
                    TmScheduleConstants.PARAM_GLUE_CHANGE_CAPACITY_DEDUCT, value,
                    TmScheduleConstants.DEFAULT_GLUE_CHANGE_CAPACITY_DEDUCT, exception);
            return new BigDecimal(TmScheduleConstants.DEFAULT_GLUE_CHANGE_CAPACITY_DEDUCT);
        }
    }
     ...
     * @return 累计切换小时数
     */
    private BigDecimal resolveExistingSwitchHours(TmScheduleContext context, String machineCode, Integer shiftOrder) {
        ScheduleTaskLinkedList<TmTaskDraft> chain = context.getTaskChain(machineCode, shiftOrder);
        if (chain == null || CollUtil.isEmpty(chain.toList())) {
            return BigDecimal.ZERO;
        }
    ...
    private BigDecimal resolveSwitchParamHours(TmScheduleContext context, String paramCode) {
        ...
        return switchMinutes.divide(BigDecimal.valueOf(TmScheduleConstants.MINUTES_PER_HOUR),
                TmScheduleConstants.DECIMAL_CALCULATION_SCALE, java.math.RoundingMode.HALF_UP);
        } catch (NumberFormatException exception) {
     ...
     * @return 百分比口径的非负损耗率
     */
    BigDecimal resolveFallbackLossRate(TmScheduleContext context, TmTaskDraft task) {
        if (task != null && task.getLossRate() != null) {
            return this.nvl(task.getLossRate()).max(BigDecimal.ZERO);
        }
        String value = this.resolveParamValue(context, TmScheduleConstants.PARAM_DEFAULT_LOSS_RATE,
                TmScheduleConstants.DEFAULT_LOSS_RATE);
        try {
            return new BigDecimal(value).max(BigDecimal.ZERO);
        } catch (NumberFormatException exception) {
            return new BigDecimal(TmScheduleConstants.DEFAULT_LOSS_RATE);
        }
    }
     ...
     */
    private boolean hasConfiguredLossRate(TmScheduleContext context) {
        if (context == null || context.getParamMap() == null) {
            return false;
        }
        TmParamValue paramValue = context.getParamMap().get(TmScheduleConstants.PARAM_DEFAULT_LOSS_RATE);
        return paramValue != null && TmParamValueSourceEnum.TABLE.getCode().equals(paramValue.getSource())
