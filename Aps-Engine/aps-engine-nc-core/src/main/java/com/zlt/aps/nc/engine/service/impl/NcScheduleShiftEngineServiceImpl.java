...
import com.zlt.aps.nc.engine.mapper.NcEngineDayFinishQtyMapper;
 ...
 */
@Component
public class NcScheduleShiftEngineServiceImpl implements INcScheduleShiftEngineService {

    @Resource
    private NcEngineScheduleResultMapper djEngineScheduleResultMapper;

    @Resource
    private NcEngineMachineMapper djEngineMachineMapper;

    @Resource
    private NcEngineDayFinishQtyMapper djEngineDayFinishQtyMapper;

    @Resource
    private NcEngineSpecifyMachineMapper djEngineSpecifyMachineMapper;

    @Resource
    private NcEngineShiftConfigMapper djEngineShiftConfigMapper;
    ...
    private boolean validateProductionOrder(String factoryCode, Date scheduleDate,
        ...
        // 查询当前机台各班次已完成量 > 0 的规格
        List<NcDayFinishQty> finishQtyList = djEngineDayFinishQtyMapper.selectList(
                new LambdaQueryWrapper<NcDayFinishQty>()
                        .eq(NcDayFinishQty::getFactoryCode, factoryCode)
                        .eq(NcDayFinishQty::getScheduleDate, scheduleDate));
        ...
        List<NcScheduleResult> machineResults = djEngineScheduleResultMapper.selectList(
        ...
        Set<String> finishedOrderNos = finishQtyList.stream()
                .map(NcDayFinishQty::getOrderNo)
                .filter(Objects::nonNull)
                .collect(Collectors.toSet());

        for (NcScheduleResult result : machineResults) {
    ...
    public ShiftValidateResult validateInsertConstraint(String factoryCode, Date scheduleDate,
        ...
        for (NcScheduleResult sr : machineResults) {
        ...
        if (earliestClass > NcEngineConstants.SHIFT_COUNT) {
    ...
    public CapacityValidateResult validateCapacity(String machineCode, int classIndex, int targetSeq,
        ...
        for (NcScheduleResult sr : currentResults) {
            ...
            if (planQty != null && planQty.compareTo(BigDecimal.ZERO) > 0) {
                ...
                if (seq != null && seq >= targetSeq) {
                    overflowSpecs.add(sr.getLiningName());
                }
            }
        }
     ...
     */
    @Override
    public List<NcScheduleResult> processInsertAndCascade(ShiftContext context) {
        ...
        List<NcScheduleResult> machineResults = context.getScheduleResults().stream()
        ...
        // 3. 从目标班次开始，逐班检查产能溢出
        for (int i = targetClass; i <= NcEngineConstants.SHIFT_COUNT; i++) {
            BigDecimal quota = getMachineQuota(machineCode);
            if (quota == null) {
            ...
            List<NcScheduleResult> toShift = new ArrayList<>();
            List<NcScheduleResult> remainingInClass = new ArrayList<>(sortedBySeq);

            // 从末端开始移除
            while (!remainingInClass.isEmpty() && overflow.compareTo(BigDecimal.ZERO) > 0) {
                NcScheduleResult last = remainingInClass.remove(remainingInClass.size() - 1);
            ...
            if (i < NcEngineConstants.SHIFT_COUNT) {
                // 顺延到下一个班次
                int nextClass = i + 1;
                // 反转保持相对顺序
                Collections.reverse(toShift);

                for (NcScheduleResult sr : toShift) {
                    BigDecimal planQty = getPlanQtyByIndex(sr, i);
                ...
                for (NcScheduleResult sr : toShift) {
                    ...
                    // 当前班次剩余总量
                    BigDecimal currentTotal = BigDecimal.ZERO;
                    for (NcScheduleResult r : remainingInClass) {
     ...
     */
    @Override
    public void reorganizeAfterReduce(List<NcScheduleResult> machineResults, int classIndex) {
     ...
     */
    @Override
    public void reorganizeAfterReduce(List<NcScheduleResult> machineResults, int classIndex, Integer skipSeq) {
        List<NcScheduleResult> withSeq = machineResults.stream()
                .filter(r -> getSequenceByIndex(r, classIndex) != null)
                .sorted(Comparator.comparingInt(r -> getSequenceByIndex(r, classIndex)))
                .collect(Collectors.toList());

        int newSeq = 1;
        for (NcScheduleResult sr : withSeq) {
     ...
     */
    @Override
    public Integer getSequenceByIndex(NcScheduleResult sr, int classIndex) {
        String fieldName = String.format(NcEngineConstants.CLASS_SEQUENCE_FIELD, classIndex);
     ...
     */
    @Override
    public void setSequenceByIndex(NcScheduleResult sr, int classIndex, Integer seq) {
        String fieldName = String.format(NcEngineConstants.CLASS_SEQUENCE_FIELD, classIndex);
     ...
     */
    @Override
    public BigDecimal getPlanQtyByIndex(NcScheduleResult sr, int classIndex) {
     ...
     */
    @Override
    public void setPlanQtyByIndex(NcScheduleResult sr, int classIndex, BigDecimal qty) {
        String fieldName = String.format(NcEngineConstants.CLASS_PLAN_QTY_FIELD, classIndex);
     ...
     */
    @Override
    public String getAnalysisByIndex(NcScheduleResult sr, int classIndex) {
        String fieldName = String.format(NcEngineConstants.CLASS_ANALYSIS_FIELD, classIndex);
     ...
     */
    @Override
    public void setAnalysisByIndex(NcScheduleResult sr, int classIndex, String analysis) {
        String fieldName = String.format(NcEngineConstants.CLASS_ANALYSIS_FIELD, classIndex);
     ...
     * 根据映射，获取对应真实班次的完成量
     */
    private BigDecimal getFinishQtyByRealShift(NcDayFinishQty finishQty, String realShiftClass) {
        if (finishQty == null) {
    ...
    private BigDecimal calculateClassFinishQty(String machineCode, int classIndex,
        ...
        Set<String> orderNos = currentResults.stream()
                .filter(r -> machineCode.equals(r.getMachineCode()))
                .map(NcScheduleResult::getOrderNo)
                .filter(Objects::nonNull)
                .collect(Collectors.toSet());
        if (orderNos.isEmpty()) {
            return BigDecimal.ZERO;
        ...
        List<NcDayFinishQty> finishQtyList = djEngineDayFinishQtyMapper.selectList(
                new LambdaQueryWrapper<NcDayFinishQty>()
                        .eq(NcDayFinishQty::getFactoryCode, factoryCode)
                        .eq(NcDayFinishQty::getScheduleDate, scheduleDate)
                        .in(NcDayFinishQty::getOrderNo, orderNos));
        ...
        for (NcDayFinishQty fq : finishQtyList) {
     ...
     * <p>
     * 从 {@link NcShiftConfig} 表中查询当前工厂开班班次配置，按 shiftOrder 排序，
    ...
    private String[] buildShiftClassMap(String factoryCode) {
        List<NcShiftConfig> shifts = djEngineShiftConfigMapper.selectList(
                new LambdaQueryWrapper<NcShiftConfig>()
                        .eq(NcShiftConfig::getFactoryCode, factoryCode)
        ...
        String[] map = new String[NcEngineConstants.SHIFT_COUNT];
        for (int i = 0; i < NcEngineConstants.SHIFT_COUNT; i++) {
    ...
    private BigDecimal getMachineQuota(String machineCode) {
        List<NcMachineInfo> list = djEngineMachineMapper.selectList(
                new LambdaQueryWrapper<NcMachineInfo>()
                        .eq(NcMachineInfo::getMachineCode, machineCode));
