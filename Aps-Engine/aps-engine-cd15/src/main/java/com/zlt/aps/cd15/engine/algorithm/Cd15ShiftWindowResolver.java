...
public class Cd15ShiftWindowResolver {
    ...
    /**
     * 解析启用班次并按排程日、当天顺序、全局顺序和 CLASS 字段稳定排序。
     *
     * @param scheduleDate 排程日期，配置第一天从该日期前一天开始
     * @param configs 班次配置
     * @return 有序班次描述
     */
    public List<Cd15ShiftDescriptor> resolve(LocalDate scheduleDate, List<Cd15ShiftConfig> configs) {
        ...
        List<Cd15ShiftConfig> enabledConfigs = configs.stream()
                ...
                .sorted(Comparator.comparingInt(this::scheduleDay)
                        ...
                        .thenComparing(config -> this.normalizeClassField(config.getClassField())))
    ...
    /** 将单条配置解析为绝对时间窗口。 */
    private Cd15ShiftDescriptor resolveOne(LocalDate scheduleDate, Cd15ShiftConfig config) {
        this.validate(config);
        String shiftCode = config.getShiftCode().trim();
        String classField = this.normalizeClassField(config.getClassField());
        int classIndex = this.classIndex(classField);
        LocalTime startTime = this.parseTime(config.getStartTime(), classField, "开始时间");
        LocalTime endTime = this.parseTime(config.getEndTime(), classField, "结束时间");
        ...
        LocalDate displayDate = Integer.valueOf(1).equals(config.getIsCrossDay()) ? endDate : startDate;
        String shiftName = this.trim(config.getShiftName());
        return Cd15ShiftDescriptor.builder()
                .shiftCode(shiftCode)
                .shiftName(shiftName)
                .shiftDisplayName(this.shiftDisplayName(shiftName, classField, displayDate))
    ...
    private void validate(Cd15ShiftConfig config) {
        ...
        if (!ThreeShiftEnum.isValidCode(shiftCode)) {
            throw new IllegalArgumentException("班次编码必须为标准三班编码01、02、03: " + shiftCode);
        }
        String classField = this.normalizeClassField(config.getClassField());
        if (!CLASS_FIELD_PATTERN.matcher(classField).matches()) {
            throw new IllegalArgumentException("班次结果字段只能取CLASS1至CLASS8: " + config.getClassField());
        }
        if (!StringUtils.hasText(config.getStartTime()) || !StringUtils.hasText(config.getEndTime())) {
            throw new IllegalArgumentException("启用班次的起止时间不能为空: " + classField);
        }
        if (config.getScheduleDay() == null || config.getScheduleDay() <= 0
                || config.getDayShiftOrder() == null || config.getDayShiftOrder() <= 0
                || config.getShiftOrder() == null || config.getShiftOrder() <= 0) {
            throw new IllegalArgumentException("启用班次的排程天数、当天顺序和班次顺序必须为正整数: " + classField);
        }
        if (!Integer.valueOf(0).equals(config.getIsCrossDay())
                && !Integer.valueOf(1).equals(config.getIsCrossDay())) {
            throw new IllegalArgumentException("启用班次的跨天标识只能取0或1: " + classField);
        }
        if (config.getShiftHours() != null && config.getShiftHours() <= 0) {
            throw new IllegalArgumentException("启用班次的班次时长必须为正整数: " + classField);
        }
    }

    /** 同一结果 CLASS 只能配置一个启用窗口。 */
    private void validateDuplicateClassField(List<Cd15ShiftConfig> configs) {
        ...
        if (duplicate.isPresent()) {
            throw new IllegalArgumentException("启用班次结果字段重复: " + duplicate.get());
        }
    }

    /** 解析 HH:mm:ss 时间。 */
    private LocalTime parseTime(String value, String classField, String fieldName) {
        try {
            return LocalTime.parse(value.trim(), TIME_FORMATTER);
        } catch (DateTimeException exception) {
            throw new IllegalArgumentException(classField + fieldName + "格式必须为HH:mm:ss", exception);
        }
    }

    /** 生成配置名称加业务日期的展示文案。 */
    private String shiftDisplayName(String shiftName, String classField, LocalDate displayDate) {
    ...
    private int classIndex(String classField) {
        Matcher matcher = CLASS_FIELD_PATTERN.matcher(classField);
        if (!matcher.matches()) {
            return 0;
        }
