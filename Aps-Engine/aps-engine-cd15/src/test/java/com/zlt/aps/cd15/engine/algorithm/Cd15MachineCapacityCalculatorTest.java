 ...
 * 斜裁机台班产能增量试算测试。
 */
public class Cd15MachineCapacityCalculatorTest {

    private final Cd15MachineCapacityCalculator calculator = new Cd15MachineCapacityCalculator();
    ...
    public void shouldDeductMaintenanceAndSpecChangeTime() {
        Cd15MachineCapacityTrial result = calculator.calculateInitial(
    ...
    public void sameSpecShouldNotDeductChangeTime() {
        Cd15MachineCapacityTrial result = calculator.calculateWithRemainingSeconds(
                new BigDecimal("2400"), 8, 3600,
                tail("R1", "A"), tail("R1", "A"),
    ...
    private com.zlt.aps.cd15.engine.model.Cd15MachineTailState tail(String roll, String spec) {
        return com.zlt.aps.cd15.engine.model.Cd15MachineTailState.builder()
                .bigRollCode(roll).steelStripCode(spec).build();
    }
}
