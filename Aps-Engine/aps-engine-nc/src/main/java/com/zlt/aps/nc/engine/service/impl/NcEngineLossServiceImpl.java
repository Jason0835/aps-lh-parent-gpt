...
@Slf4j
@Service
public class NcEngineLossServiceImpl implements NcEngineLossService {
     ...
     * 把损耗率list转成map
     *
     * @return key：机台id + ‘#’ + 内衬代码，例如20#HT3568-377P
     */
    public Map<String, Double> getLossRateMap() {
        Map<String, Double> lossMap = new HashMap<>();
        List<NcLossVo> list = ncEngineLossMapper.listLossRate();
        for (NcLossVo lossVo : list) {
     ...
     * @return
     */
    public double getLossRate(String liningCode, String machineIds, Map<String, Double> lossMap, double paramLossRate) {
        liningCode = (StringUtils.isBlank(liningCode) ? "" : liningCode);
        if (StringUtils.isBlank(machineIds)) {
            //排程没有机台信息
            double lossRate = lossMap.getOrDefault("#" + liningCode, paramLossRate);
            ...
            for (int i = 0; i < machineIdArry.length; i++) {
                String key1 = machineIdArry[i] + "#" + liningCode;
                String key2 = "#" + liningCode;
