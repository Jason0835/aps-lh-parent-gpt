  ...
  <basic-container>
    ...
    >
      <template slot="header">
        <el-button type="primary" @click="handleExport">{{$t("common.button.export")}}</el-button>
      </template>
    </page-table>
  </basic-container>
</template>
  ...
  data() {
    ...
    return {
      ...
      columns: [
          ...
          render: ({ row }) => {
            return (
              <dict-tag
                ...
                options={[
                  { label: "硫化", value: "0", raw: {} },
                  { label: "成型", value: "1", raw: {} },
                  { label: "胎面", value: "2", raw: {} },
                  { label: "半部件	", value: "-1", raw: {} },
                ]}
              />
            );
          },
        ...
        {
          label: this.$t("ui.data.column.reportStatistics.actualProFinishRateLow"),
          prop: "actualProFinishRateLow",
          minWidth: 100,
          sortable: "custom",
        },
        {
          label: this.$t("ui.data.column.reportStatistics.actualProFinishRateMid"),
          prop: "actualProFinishRateMid",
          minWidth: 100,
          sortable: "custom",
        },
        {
          label: this.$t("ui.data.column.reportStatistics.actualProFinishRateHigh"),
          ...
          render: (form) => {
            return (
              <dict-select
                ...
                options={[
                  { label: this.$t("ui.data.column.reportStatistics.everyDay"), value: 1 },
                  { label: this.$t("ui.data.column.reportStatistics.summary"), value: 2 },
                ]}
              />
            );
          },
