      ...
      <el-row>
        <el-col :span="24">
          <el-card class="table-box-card" shadow="never">
            ...
            <div>
              <t-form ref="valiFormRef" :model="valiForm" :show-message="false">
                <page-table
...
import { addSubData } from "@/api/lh/lhApsMoldAdjustPlan";
export default {
  components: { infoForm },
  inject: ["parentDict"],
  data() {
      ...
      formColumns: [
        {
          ...
          span: 12,
          // render: (form) => {
          //   return <el-select class="w100" v-model={form.workingProcedure} />;
          // },
        },
        {
  ...
  methods: {
    ...
    async save(params) {
      let query = {
        // planDate: string,
        // lhMachineCode: string,
        // lhMachineName: string,
        ...params,
        // apsMoldAdjustPlanList: this.data,
      };
      try {
        this.loading = true;
        const data = await addSubData(query);
