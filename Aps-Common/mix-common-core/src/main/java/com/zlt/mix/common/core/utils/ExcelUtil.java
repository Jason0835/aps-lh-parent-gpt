...
public class ExcelUtil<T> {
    ...
    /**
     * init方法重载，可从调用端定义好fields传入进行初始化
     * @param fields 待构建的栏位信息列表，结构：[[字段1的反射对象，字段1的excel注解对象], [字段2的反射对象，字段2的excel注解对象]...]
     */
    public void init(List<T> list, String sheetName, Type type, List<Object[]> fields) {
        if (list == null) {
            list = new ArrayList<T>();
        }
        this.list = list;
    ...
    /**
     *  解析Excel的第column列
     * @param InputStream,headRowNum,column
     * @return List<String>
     * @throws Exception
     */
    ...
    public void exportExcel(HttpServletResponse response, List<T> list, String sheetName, String fileName) throws IOException {
        ...
        // 解决导出文件名中文乱码
        response.setHeader("Content-Disposition", "attachment;filename=" + new String(fileName.getBytes("UTF-8"), "ISO-8859-1").concat(".xlsx"));
    ...
    public void importTemplateExcel(HttpServletResponse response, String sheetName) throws IOException {
        response.setCharacterEncoding("UTF-8");
        response.setHeader("content-Type", "application/vnd.ms-excel");
        response.setHeader("Content-Disposition", "attachment;filename=" + new String(sheetName.getBytes("UTF-8"), "ISO-8859-1").concat(".xlsx"));
        response.setHeader("Access-Control-Expose-Headers", "Content-Disposition");
    ...
    public void createExcelWithDict(Sheet sheet1, Sheet dictSheet, List<String[]> downDataList, List<Integer> downDataLocations) {
        ...
        for (int i = 0; i < downDataLocations.size(); i++) {
            ...
            Excel excel = (Excel) os[1];
            boolean dictTypeToExcelEnable=excel.dictTypeToExcelEnable();
            if(dictTypeToExcelEnable){
     ...
     * @throws Exception
     */
    public static String convertByDictOrCheckbox(String propertyValue, String dictType) throws Exception {
    ...
    public static String convertByDict(String propertyValue, String dictType) throws Exception {
        String dictDataStr = dictDataCach.get().get(dictType + propertyValue);
        if (StringUtils.isEmpty(dictDataStr)) {
            List<SysDictData> dictDatas = getDictCache(dictType);
            SysDictData dictData = null;
            for (SysDictData sysDictData : dictDatas) {
                if (propertyValue.equals(sysDictData.getDictValue())) {
                    dictData = sysDictData;
                    break;
                }
            }
            if (StringUtils.isNotNull(dictData)) {
                dictDataStr = dictData.getDictLabel();
                dictDataCach.get().put(dictType + propertyValue, dictDataStr);
            } else {
     ...
     * @throws Exception
     */
    public static String convertByDictValueOrCheckbox(String propertyValue, String dictType) throws Exception {
        StringBuffer cellValue = new StringBuffer();
        if (propertyValue.contains(",")) {
            String[] ss = propertyValue.split(",");
            for (String s : ss) {
     ...
     * @throws Exception
     */
    public static String convertByDictValue(String propertyValue, String dictType) throws Exception {
        String dictDataStr = dictDataCach.get().get(dictType + propertyValue + SecurityUtils.getUserLang());
        if (StringUtils.isEmpty(dictDataStr)) {
            List<SysDictData> dictDatas = getDictCache(dictType);
            SysDictData dictData = null;
            for (SysDictData sysDictData : dictDatas) {
                if (propertyValue.equals(sysDictData.getDictLabel())) {
                    dictData = sysDictData;
                    break;
                }
            }
            if (StringUtils.isNotNull(dictData)) {
                dictDataStr = dictData.getDictValue();
                dictDataCach.get().put(dictType + propertyValue + SecurityUtils.getUserLang(), dictDataStr);
            }
        }
        return dictDataStr;
    }

    public static ThreadLocal<Map<String, String>> convertByDictValue2(ThreadLocal<Map<String, String>> dictDataCach, String dictType) throws Exception {
        String dictDataStr = dictDataCach.get().get(dictType + "hasValue");
        if (StringUtils.isEmpty(dictDataStr)) {
            List<SysDictData> dictDatas = getDictCache(dictType);
            for (SysDictData sysDictData : dictDatas) {
                dictDataCach.get().put(dictType + SecurityUtils.getUserLang() + sysDictData.getDictValue(), sysDictData.getDictValue());
            }
