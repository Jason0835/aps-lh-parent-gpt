...
import com.zlt.mix.common.engine.mapper.SyncDataLogsMapper;
...
public class SyncDataLogsServiceImpl implements SyncDataLogsService {
	...
	public SyncDataLogs getSyncDataResult(String dataVersion) {
        if ("0".equals(checkSyncResult)) { // 是否检查接口返回状态，不检查则直接返回
            ...
            logs.setStatus(ZltConstant.IS_RELEASE);
            return logs;
        }
		// 扫描截止时间：30秒后
	...
	private SyncDataLogs checkLogStatus(String dataVersion, SyncDataLogs logs, Date endTime) {
		if (logs != null) {
			if (ZltConstant.IS_RELEASE.equals(logs.getStatus())
					|| ZltConstant.FAILURE_RELEASE.equals(logs.getStatus())) {
				// 异常情况，需要处理异常信息
				if (ZltConstant.FAILURE_RELEASE.equals(logs.getStatus())) {
		...
		if (currentTime.compareTo(endTime) > 0) {
			...
			logs.setStatus(ZltConstant.TIMEOUT_FAILURE);
			return logs;
		}
		// 等待3秒后重新扫描
		try {
