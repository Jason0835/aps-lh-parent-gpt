...
CREATE TABLE `T_TM_SCHEDULE_RESULT_EXPLAIN` (
  ...
  `is_delete` tinyint NOT NULL DEFAULT '0' COMMENT '是否删除，0否，1是',
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_tm_schedule_result_explain_result` (`result_id`, `is_delete`),
...
VALUES
('', 'TM_ALGORITHM_SWITCH', '胎面需求算法开关', '1', '1', '^[12]$', '胎面需求算法开关只能维护1或2', 'TM_SCHEDULE', 'NUMBER', '1', '1表示成型三班最大计划量算法，2表示下个班成型计划算法', 'system'),
('', 'TM_MIN_STOCK_CLASS', '胎面库存最低保证班数', '1', '1', '^[0-9]+(\\.[0-9]+)?$', '胎面库存最低保证班数必须为非负数字', 'TM_SCHEDULE', 'NUMBER', '1', '计算需排产量时使用的库存最低保证班数', 'system'),
('', 'TM_MIN_START_QTY', '胎面最小起排量', '300', '300', '^[0-9]+(\\.[0-9]+)?$', '胎面最小起排量必须为非负数字', 'TM_SCHEDULE', 'NUMBER', '1', '需排产量未达到该值时本班不排产', 'system'),
('', 'TM_VEHICLE_RATE', '胎面工装整车率', '1', '1', '^[0-9]+(\\.[0-9]+)?$', '胎面工装整车率必须为非负数字', 'TM_SCHEDULE', 'NUMBER', '1', '工装可用数量计算使用的整车率', 'system'),
('', 'TM_OPEN_SHIFT_THRESHOLD', '胎面开班排产阈值', '1', '1', '^[0-9]+(\\.[0-9]+)?$', '胎面开班排产阈值必须为非负数字', 'TM_SCHEDULE', 'NUMBER', '1', '开产班次按库存供应成型时长切换规格时使用', 'system'),
('', 'TM_MAX_CLASS_QTY', '胎面最大班产限制', '5500', '5500', '^[0-9]+(\\.[0-9]+)?$', '胎面最大班产限制必须为非负数字', 'TM_SCHEDULE', 'NUMBER', '1', '单班最多可排产米数', 'system'),
('', 'TM_TOOL_TOTAL_QTY', '胎面工装总数', NULL, NULL, '^[0-9]+(\\.[0-9]+)?$', '胎面工装总数必须为非负数字', 'TM_SCHEDULE', 'NUMBER', '1', '工装限制排产量时使用，按现场实际数量维护', 'system'),
('', 'TM_SPEC_CHANGE_MINUTES', '胎面规格切换时长', '0', '0', '^[0-9]+(\\.[0-9]+)?$', '胎面规格切换时长必须为非负数字', 'TM_SCHEDULE', 'NUMBER', '1', '计算机台剩余产能和预计开始时间时使用，单位分钟', 'system'),
('', 'TM_GLUE_CHANGE_MINUTES', '胎面胶料切换时长', '0', '0', '^[0-9]+(\\.[0-9]+)?$', '胎面胶料切换时长必须为非负数字', 'TM_SCHEDULE', 'NUMBER', '1', '计算机台剩余产能时使用，单位分钟', 'system'),
('', 'TM_PROCESS_STANDING_HOURS', '胎面工艺停放时长', '0', '0', '^[0-9]+(\\.[0-9]+)?$', '胎面工艺停放时长必须为非负数字', 'TM_SCHEDULE', 'NUMBER', '1', '计算最晚开始生产时间时使用，单位小时', 'system');
