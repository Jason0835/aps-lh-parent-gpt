...
public class StringUtils extends org.apache.commons.lang3.StringUtils {
    ...
    /***
     * 从json读出语言包的名称，如果没有，则用默认名称
     * @param json
     * @param locale
     * @param defaultName
     * @return
     */
    public static String getLocaleName(String json, Locale locale, String defaultName) {

        List<Map> lists = JSONArray.parseArray(json, Map.class);
        if (StringUtils.isNull(lists) || lists.isEmpty()) {
            return defaultName;
        }

        //先把符合语言的找出来
        List<Map> locales = lists.stream().filter(
                item -> (StringUtils.isNotNull(item.get(locale.toString()))
                )
        ...
        return locales.get(0).get(locale.toString()).toString();
    }
}